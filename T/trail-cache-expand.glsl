#version 330 core
// af_scale-cache-code-acid-crown (Expanding Edition)
// Solid live frame with a soft, expanding, and breathing audio-reactive trail.
#define EFFECT_ID 24
...
vec2 dynamic_trail_transform(vec2 uv, vec2 center, int index, int total_layers, float bass, float treble, float time) {
    ...
    // EXPANDING SCALE: Older frames are pushed outward.
    // A staggered sine wave adds an organic breathing pulse through the cache layers.
    float breath = sin(time * 2.5 - float(index) * 0.4) * 0.06;
    float expand_factor = 1.0 + (bass * 1.8) + breath; 
    float current_scale = mix(1.0, expand_factor, progress);
...
void main() {
    ...
    // while the trail softly glows and expands from behind it.
