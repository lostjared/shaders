#version 330 core
// psyche-bubble-cache-fractal-pearl
...
vec3 psyche_field(vec2 point, float bass, float middle, float treble, float age) {
    ...
    vec2 pearl = recursive_fold(point * (1.1 + bass * 0.2), time + age * 5.0);
    float fracture = sin(length(pearl) * 13.0 + atan(pearl.y, pearl.x) * 4.0 - time * 3.0);
    float nacre = exp(-abs(length(pearl) - 0.55) * 8.0);
    return vec3(fracture, nacre * (0.5 + 0.5 * fracture), length(pearl) * 0.3 + age + time * 0.04);
}
...
vec2 psyche_warp(vec2 uv, vec2 point, vec3 signal, float bass, float middle,
    ...
    vec2 pearl = recursive_fold(point * (0.9 + age * 0.2), time_f);
    uv = pearl * (0.08 + age * 0.025) + 0.5;
...
float layer_gate(vec2 point, vec3 signal, float progress, int index) {
    ...
    return 0.25 + 0.75 * smoothstep(0.12, 0.7, signal.y);
}
