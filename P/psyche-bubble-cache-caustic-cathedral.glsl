#version 330 core
// psyche-bubble-cache-caustic-cathedral
...
vec3 psyche_field(vec2 point, float bass, float middle, float treble, float age) {
    ...
    float nave = exp(-abs(point.x) * (7.0 + middle * 7.0)) *
                 cos(point.y * (21.0 + treble * 9.0) - time * 2.0);
    float arches = cos(sqrt(abs(point.x) + 0.006) * 18.0 - point.y * 8.0 + age * 9.0);
    return vec3(arches, max(nave, 0.0) + pow(max(arches, 0.0), 6.0) * 0.4,
                point.y * 0.7 - time * 0.08 + age);
}
...
vec2 psyche_warp(vec2 uv, vec2 point, vec3 signal, float bass, float middle,
    ...
    uv.x += sin(point.y * 12.0 + age * 8.0) * 0.018 * depth;
    uv.y += signal.x * (0.025 + middle * 0.05) * sign(point.x);
...
float layer_gate(vec2 point, vec3 signal, float progress, int index) {
    ...
    return 0.25 + 0.75 * exp(-abs(point.x - (progress - 0.5) * 0.7) * 12.0);
}
