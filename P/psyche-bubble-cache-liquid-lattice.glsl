#version 330 core
// psyche-bubble-cache-liquid-lattice
...
vec3 psyche_field(vec2 point, float bass, float middle, float treble, float age) {
    ...
    float horizontal = sin(point.x * 17.0 + sin(point.y * 7.0 - time) * (4.0 + bass * 3.0));
    float vertical = cos(point.y * 16.0 + sin(point.x * 6.0 + time) * (4.0 + treble * 3.0));
    float knots = pow(1.0 - abs(horizontal * vertical), 5.0);
    return vec3(horizontal + vertical, knots, point.x + point.y + age + middle);
}
...
vec2 psyche_warp(vec2 uv, vec2 point, vec3 signal, float bass, float middle,
    ...
    uv += vec2(sin(point.y * 15.0 + signal.x), cos(point.x * 15.0 - signal.x)) *
          (0.015 + middle * 0.035) * depth;
...
float layer_gate(vec2 point, vec3 signal, float progress, int index) {
    ...
    return 0.28 + 0.72 * pow(max(signal.y, 0.0), 0.65);
}
