#version 330 core
// psyche-bubble-cache-orbital-chain
...
vec3 psyche_field(vec2 point, float bass, float middle, float treble, float age) {
    ...
    vec2 orbit = vec2(cos(time * 0.61 + age * TAU), sin(time * 0.47 + age * TAU)) *
                 (0.18 + age * 0.25);
    float bead = sin(length(point - orbit) * (38.0 + bass * 18.0) - time * 7.0);
    float chain = pow(max(cos(angle * 9.0 - time * 1.7 + age * 12.0), 0.0), 7.0);
    return vec3(bead, chain * exp(-abs(radius - 0.33) * 7.0), angle / TAU + age + middle);
}
...
vec2 psyche_warp(vec2 uv, vec2 point, vec3 signal, float bass, float middle,
    ...
    vec2 orbit_center = vec2(cos(time_f * 0.61 + age * TAU),
                             sin(time_f * 0.47 + age * TAU)) * 0.16;
    vec2 local = point - orbit_center;
    local = rotate_2d(signal.x * 0.09 + age * 0.7) * local;
    uv = (local + orbit_center) / vec2(iResolution.x / max(iResolution.y, 1.0), 1.0) + 0.5;
...
float layer_gate(vec2 point, vec3 signal, float progress, int index) {
    ...
    return 0.2 + 0.8 * exp(-abs(sin(angle * 4.5 - progress * TAU)) * 9.0);
}
