...
vec3 psyche_field(vec2 point, float bass, float middle, float treble, float age) {
    ...
    vec2 cell = hex_cell(rotate_2d(time * 0.04 + age * 0.4) * point *
                         (8.0 + treble * 3.0));
    float edge = exp(-abs(length(cell) - 0.35) * 24.0);
    float pulse = sin(length(cell) * (32.0 + bass * 18.0) - time * 5.0);
    return vec3(pulse, edge, atan(cell.y, cell.x) / TAU + age + middle);
}
...
vec2 psyche_warp(vec2 uv, vec2 point, vec3 signal, float bass, float middle,
    ...
    vec2 cell = hex_cell(point * (8.0 + treble * 3.0));
    uv += rotate_2d(age * 1.2) * cell * signal.x * 0.045;
