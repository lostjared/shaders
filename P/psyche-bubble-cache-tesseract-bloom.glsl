#version 330 core
// psyche-bubble-cache-tesseract-bloom
...
vec3 psyche_field(vec2 point, float bass, float middle, float treble, float age) {
    ...
    vec2 cube = abs(rotate_2d(PI * 0.25 + age) * point);
    float outer = max(cube.x, cube.y);
    float inner = min(cube.x, cube.y);
    float planes = sin((outer + inner * 0.6) * (35.0 + bass * 16.0) - time * 6.0);
    float edges = exp(-abs(outer - (0.18 + age * 0.32)) * 18.0);
    return vec3(planes, edges, outer - inner + age + treble);
}
...
vec2 psyche_warp(vec2 uv, vec2 point, vec3 signal, float bass, float middle,
    ...
    vec2 cube = rotate_2d(PI * 0.25 + age) * point;
    cube = sign(cube) * pow(abs(cube), vec2(0.72 + bass * 0.12));
    uv = cube / vec2(iResolution.x / max(iResolution.y, 1.0), 1.0) + 0.5;
...
float layer_gate(vec2 point, vec3 signal, float progress, int index) {
    ...
    return 0.23 + 0.77 * exp(-abs(max(abs(point.x), abs(point.y)) - mix(0.12, 0.65, progress)) * 15.0);
}
