#version 330 core
// psyche-bubble-cache-gravity-well
...
vec3 psyche_field(vec2 point, float bass, float middle, float treble, float age) {
    ...
    float gravity = sin(log(radius + 0.025) * (17.0 + bass * 7.0) + time * 5.0 - age * 10.0);
    float photon_ring = exp(-abs(radius - 0.23 - middle * 0.06) * 24.0);
    return vec3(gravity, photon_ring * (0.5 + 0.5 * gravity) + exp(-radius * 8.0),
                -log(radius + 0.03) * 0.25 + angle / TAU + age);
}
...
vec2 psyche_warp(vec2 uv, vec2 point, vec3 signal, float bass, float middle,
    ...
    uv += radial * signal.x * (0.018 + bass * 0.055) / (0.22 + radius);
    uv += tangent * (0.02 + age * 0.08) / (0.3 + radius);
...
float layer_gate(vec2 point, vec3 signal, float progress, int index) {
    ...
    return 0.2 + 0.8 * exp(-abs(radius - 0.23 - progress * 0.18) * 20.0);
}
