#version 330 core
// psyche-bubble-cache-cellular-foam
...
vec3 psyche_field(vec2 point, float bass, float middle, float treble, float age) {
    ...
    vec2 cell = voronoi_cell(point * (6.0 + treble * 4.0));
    float distance_to_seed = length(cell);
    float foam = sin(distance_to_seed * 27.0 - time * 3.5 - bass * 5.0);
    float membrane = exp(-abs(distance_to_seed - 0.38) * 20.0);
    return vec3(foam, membrane, distance_to_seed * 2.0 + age + time * 0.05);
}
...
vec2 psyche_warp(vec2 uv, vec2 point, vec3 signal, float bass, float middle,
    ...
    vec2 cell = voronoi_cell(point * (6.0 + treble * 4.0));
    uv += normalize(cell + vec2(0.0001)) * signal.x * 0.045 * depth;
...
float layer_gate(vec2 point, vec3 signal, float progress, int index) {
    ...
    return 0.3 + 0.7 * smoothstep(0.18, 0.82, signal.y);
}
