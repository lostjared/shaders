...
vec3 spectrum(float w) {
    vec3 c;
    if (w < 0.2)
        c = mix(vec3(1, 0, 1), vec3(0, 0, 1), w * 5.0); // Violet to Blue
    else if (w < 0.4)
        c = mix(vec3(0, 0, 1), vec3(0, 1, 0), (w - 0.2) * 5.0); // Blue to Green
    else if (w < 0.6)
        c = mix(vec3(0, 1, 0), vec3(1, 1, 0), (w - 0.4) * 5.0); // Green to Yellow
    else if (w < 0.8)
        c = mix(vec3(1, 1, 0), vec3(1, 0, 0), (w - 0.6) * 5.0); // Yellow to Red
    else
        c = vec3(1, 0, 0);
    return c;
}
