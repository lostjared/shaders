#version 330 core
// psyche-bubble-cache-nautilus-echo
...
vec3 psyche_field(vec2 point, float bass, float middle, float treble, float age) {
    ...
    float spiral = sin(log(radius + 0.035) * (14.0 + bass * 8.0) -
                       angle * (5.0 + floor(treble * 4.0)) + time * 4.0 + age * 11.0);
    float ribs = pow(abs(cos(angle * 11.0 - radius * 8.0)), 9.0) * exp(-radius * 1.4);
    return vec3(spiral, ribs, angle / TAU - log(radius + 0.05) * 0.2 + age);
}
...
vec2 psyche_warp(vec2 uv, vec2 point, vec3 signal, float bass, float middle,
    ...
    float twist = signal.x * 0.08 + bass * 0.12 + age * 0.55;
    vec2 spun = rotate_2d(twist / (0.35 + radius)) * point;
...
float layer_gate(vec2 point, vec3 signal, float progress, int index) {
    ...
    return 0.22 + 0.78 * smoothstep(0.2, 0.85, abs(signal.x));
}
