...
vec3 psyche_field(vec2 point, float bass, float middle, float treble, float age) {
    ...
    float column = floor((point.x + 0.8) * (11.0 + treble * 8.0));
    float speed = 0.5 + hash_21(vec2(column, 3.7)) * 1.7;
    float drop = fract(point.y * 2.3 + time * speed + age * 1.9);
    float pearl = exp(-abs(drop - 0.5) * (18.0 + bass * 10.0));
    return vec3(sin(drop * TAU + column), pearl, column * 0.07 + drop + age);
}
...
vec2 psyche_warp(vec2 uv, vec2 point, vec3 signal, float bass, float middle,
    ...
    float column = floor((point.x + 0.8) * (11.0 + treble * 8.0));
    uv.y += (hash_21(vec2(column, 1.0)) - 0.5) * 0.12 * depth;
    uv.x += signal.x * 0.012;
...
float layer_gate(vec2 point, vec3 signal, float progress, int index) {
    ...
    return 0.2 + 0.8 * step(0.6, fract(point.y * 2.3 + progress + time_f * 0.4));
}
