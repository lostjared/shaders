...
// Temporal inversion energy field: dipole choir.
...
vec3 energyField(vec2 p, float age, vec4 band) {
    float t = clockTime();
    vec2 poleA = vec2(cos(t * 0.37 + age), sin(t * 0.29 + age * 0.7)) * 0.34;
    vec2 poleB = -poleA;
    vec2 da = p - poleA;
    vec2 db = p - poleB;
    float ra = length(da) + 0.055;
    float rb = length(db) + 0.055;
    float potential = (1.0 + band.x) / ra - (1.0 + band.y) / rb;
    float energy = 0.5 + 0.5 * sin(potential * 2.6 + t - age * 0.8);
    vec2 flow = normalize(da) / ra - normalize(db) / rb;
    return vec3(energy, flow * 0.18);
}
