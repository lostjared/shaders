...
// Cache-driven perceptual energy field: afterimage choir.
...
const float HUE_OFFSET = 0.34;
const float DISTORTION = 0.76;
const float PERSISTENCE = 0.94;
...
vec3 perceptualField(vec2 p, float age, vec4 band) {
    float t = clockTime();
    vec2 q = p;
    q.x += sin(q.y * 4.0 + t * 0.23 - age * 0.3) * 0.09;
    q.y += cos(q.x * 5.0 - t * 0.19 + age * 0.27) * 0.08;
    float voiceA = sin(q.x * 9.0 + q.y * 5.0 + t * 0.38 - age * 0.8);
    float voiceB = sin(q.x * 6.0 - q.y * 11.0 - t * 0.31 + age * 0.6);
    float voiceC = cos(length(q) * 18.0 + t * 0.25 - age);
    float chord = (voiceA + voiceB + voiceC) / 3.0;
    float energy = 0.5 + 0.5 * chord;
    vec2 flow = vec2(voiceB - voiceC, voiceC - voiceA) * 0.28;
