...
// History-manipulated in-the-sky effect: celestial cathedral.
...
vec3 skyField(vec2 p, float age, vec4 band) {
    float t = clockTime();
    float archX = sqrt(p.x * p.x + 0.004);
    float arches = cos(archX * (12.0 + band.y * 10.0) - p.y * 5.0 + age * 0.4);
    float windows = cos(p.y * (17.0 + band.z * 16.0) + t * 0.55);
    float vault = exp(-abs(arches * 0.72 + windows * 0.28) * 4.0);
    vec2 flow = vec2(-sin(archX * 12.0), cos(p.y * 17.0)) * vault;
    return vec3(vault, flow);
}
