...
// Cache-driven perceptual energy field: scintillating aura.
...
const float HUE_OFFSET = 0.44;
const float DISTORTION = 1.16;
const float PERSISTENCE = 0.82;
...
vec3 perceptualField(vec2 p, float age, vec4 band) {
    ...
    float polygon = cos(angle * (18.0 + floor(band.z * 8.0))) * 0.035;
    float frontRadius = 0.18 + 0.34 * (0.5 + 0.5 * sin(t * 0.14 - age * 0.09));
    float front = abs(radius - frontRadius - polygon);
    float aura = exp(-front * (55.0 + band.w * 80.0));
    float zigzag = 0.5 + 0.5 * sin(angle * 36.0 + radius * 70.0 - t * 1.2 + age);
    float crescent = smoothstep(-0.7, 0.35, cos(angle - t * 0.12));
    vec2 flow = normalize(p + vec2(0.001)) * aura;
    return vec3(aura * zigzag * crescent, flow);
}
