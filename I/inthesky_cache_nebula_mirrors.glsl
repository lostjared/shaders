...
// History-manipulated in-the-sky effect: nebula mirrors.
...
const float SEGMENTS = 22.0;
const float HUE_OFFSET = 0.74;
const float INTENSITY = 1.52;
...
vec3 skyField(vec2 p, float age, vec4 band) {
    float t = clockTime();
    vec2 q = rotate2(p, t * 0.035 + age * 0.02);
    float nebulaA = fbm(q * 2.2 + vec2(t * 0.04, -t * 0.03));
    float nebulaB = fbm(abs(q) * 4.8 - vec2(t * 0.05, t * 0.02));
    float dust = pow(clamp(nebulaA * 0.65 + nebulaB * 0.52, 0.0, 1.0), 2.4);
    vec2 starGrid = floor((q + age * 0.01) * 28.0);
    float stars = pow(hash21(starGrid), 24.0) * (0.5 + band.w);
    float energy = dust + stars;
    vec2 flow = vec2(nebulaA - nebulaB, nebulaB - 0.5);
