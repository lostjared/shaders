...
// Temporal inversion energy field: laser caustics.
...
vec3 energyField(vec2 p, float age, vec4 band) {
    ...
    for (int fold = 0; fold < 5; ++fold) {
        float fi = float(fold);
        q = abs(q) / max(dot(q, q), 0.12) - vec2(0.72 + band.x * 0.08, 0.48);
        q = rotate2(q, 0.31 + fi * 0.17 + age * 0.01);
        accumulation += exp(-abs(q.x + q.y) * (5.0 + fi));
    }
    float energy = accumulation / 5.0;
    vec2 flow = normalize(q + vec2(0.001)) * energy;
