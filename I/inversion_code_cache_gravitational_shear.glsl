...
// Temporal inversion energy field: gravitational shear.
...
vec3 energyField(vec2 p, float age, vec4 band) {
    ...
    float x2 = p.x * p.x;
    float y2 = p.y * p.y;
    float saddle = (x2 - y2) * (4.0 + band.x * 5.0);
    float crossField = p.x * p.y * (9.0 + band.y * 7.0);
    float sheet = sin(saddle - t * 0.7 + age * 0.3) * cos(crossField + t * 0.5 - age * 0.2);
    float energy = pow(abs(sheet), 2.2);
    vec2 flow = normalize(vec2(p.x * (1.0 + band.x), -p.y * (1.0 + band.y)) + vec2(0.001));
    return vec3(energy, flow * (0.4 + energy));
}
