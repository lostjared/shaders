...
// Temporal inversion energy field: harmonic glyphs.
...
vec3 energyField(vec2 p, float age, vec4 band) {
    ...
    float petals = 5.0 + floor(band.z * 8.0);
    float glyphA = cos(angle * petals) * (0.28 + band.x * 0.12);
    float glyphB = sin(angle * (petals + 2.0) + t * 0.5) * 0.12;
    float stroke = abs(radius - 0.38 - glyphA - glyphB);
    float energy = exp(-stroke * (45.0 + band.w * 75.0));
    vec2 flow = vec2(-sin(angle), cos(angle)) * energy;
