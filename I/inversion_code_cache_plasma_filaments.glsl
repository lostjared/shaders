...
// Temporal inversion energy field: plasma filaments.
...
vec3 energyField(vec2 p, float age, vec4 band) {
    float t = clockTime();
    float filamentA = sin(p.x * 8.0 + sin(p.y * 5.0 - t) * (3.0 + band.x * 6.0));
    float filamentB = sin(p.y * 11.0 + cos(p.x * 4.0 + t * 0.8) * (2.0 + band.y * 7.0));
    float filamentC = cos((p.x + p.y) * 13.0 - t * 1.7 + age * 0.5);
    float junction = abs(filamentA + filamentB + filamentC) / 3.0;
    float energy = pow(junction, 3.0);
    vec2 flow = vec2(filamentB - filamentC, filamentC - filamentA);
    return vec3(energy, flow * 0.4);
}
