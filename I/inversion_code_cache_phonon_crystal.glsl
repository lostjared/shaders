...
// Temporal inversion energy field: phonon crystal.
...
vec3 energyField(vec2 p, float age, vec4 band) {
    ...
    vec2 k1 = vec2(0.5, 0.8660254);
    vec2 k2 = vec2(-0.5, 0.8660254);
    float frequency = 15.0 + band.y * 24.0;
    float w0 = cos(dot(p, k0) * frequency - t * 1.3 + age);
    float w1 = cos(dot(p, k1) * frequency + t * 0.9 - age * 0.7);
    float w2 = cos(dot(p, k2) * frequency - t * 0.7 + age * 0.4);
    float mode = (w0 + w1 + w2) / 3.0;
    vec2 flow = k0 * w0 + k1 * w1 + k2 * w2;
    return vec3(mode * mode, flow * 0.5);
}
