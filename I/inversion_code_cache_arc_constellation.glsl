...
// Temporal inversion energy field: arc constellation.
...
vec3 energyField(vec2 p, float age, vec4 band) {
    float t = clockTime();
    vec2 a = vec2(sin(t * 0.31 + age), cos(t * 0.27 + age * 0.4)) * 0.52;
    vec2 b = vec2(cos(t * 0.23 - age), sin(t * 0.35 + age * 0.6)) * 0.52;
    vec2 c = vec2(sin(t * 0.19 + 2.1), cos(t * 0.33 - 1.4)) * 0.38;
    float d0 = segmentDistance(p, a, b);
    float d1 = segmentDistance(p, b, c);
    float d2 = segmentDistance(p, c, a);
    float width = 35.0 + band.z * 90.0;
    float arcs = exp(-min(d0, min(d1, d2)) * width);
    float pulse = 0.65 + 0.35 * sin((d0 + d1 + d2) * 30.0 - t * 4.0 + age);
    vec2 flow = normalize((a + b + c) / 3.0 - p + vec2(0.001));
    return vec3(arcs * pulse, flow * arcs);
}
