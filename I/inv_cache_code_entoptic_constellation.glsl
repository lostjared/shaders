...
// Cache-driven perceptual energy field: entoptic constellation.
...
const float HUE_OFFSET = 0.04;
const float DISTORTION = 1.10;
const float PERSISTENCE = 0.88;
...
vec3 perceptualField(vec2 p, float age, vec4 band) {
    float t = clockTime();
    vec2 grid = p * (12.0 + band.w * 10.0);
    vec2 id = floor(grid);
    vec2 local = fract(grid) - 0.5;
    vec2 seed = hash22(id + age * 3.1);
    vec2 drift =
        vec2(sin(t * (0.4 + seed.x) + seed.y * TAU), cos(t * (0.3 + seed.y) + seed.x * TAU)) * 0.23;
    vec2 particle = seed - 0.5 + drift;
    float spark = exp(-length(local - particle) * (20.0 + band.z * 35.0));
    float tail =
        exp(-abs(local.y - particle.y) * 16.0) * exp(-max(particle.x - local.x, 0.0) * 9.0) * 0.35;
    float pulse = 0.65 + 0.35 * sin(t * 2.0 + hash21(id) * TAU - age);
    return vec3((spark + tail) * pulse, normalize(drift + vec2(0.001)) * spark);
}
