...
// Temporal inversion energy field: hex current.
...
vec3 energyField(vec2 p, float age, vec4 band) {
    float t = clockTime();
    vec2 q = p * (6.0 + band.y * 4.0);
    q.x *= 0.57735027;
    q.y += mod(floor(q.x), 2.0) * 0.5;
    vec2 cell = fract(q) - 0.5;
    float edgeA = abs(cell.x);
    float edgeB = abs(dot(cell, normalize(vec2(0.5, 0.8660254))));
    float edgeC = abs(dot(cell, normalize(vec2(-0.5, 0.8660254))));
    float edge = max(edgeA, max(edgeB, edgeC));
    float wire = exp(-abs(edge - 0.43) * (38.0 + band.z * 50.0));
    float current = 0.6 + 0.4 * sin((cell.x + cell.y) * 18.0 - t * 3.0 + age);
    vec2 flow = normalize(vec2(-cell.y, cell.x) + vec2(0.001));
    return vec3(wire * current, flow * wire);
}
