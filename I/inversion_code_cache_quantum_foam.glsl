...
// Temporal inversion energy field: quantum foam.
...
vec3 energyField(vec2 p, float age, vec4 band) {
    float t = clockTime();
    vec2 coarseGrid = p * (4.0 + band.x * 3.0);
    vec2 fineGrid = p * (11.0 + band.y * 5.0);
    float coarse = 9.0;
    float fine = 9.0;
    vec2 coarseId = floor(coarseGrid);
    vec2 fineId = floor(fineGrid);
    for (int y = -1; y <= 1; ++y) {
        for (int x = -1; x <= 1; ++x) {
            vec2 neighbor = vec2(float(x), float(y));
            coarse =
                min(coarse, length(neighbor + hash22(coarseId + neighbor) - fract(coarseGrid)));
            fine = min(fine, length(neighbor + hash22(fineId + neighbor) - fract(fineGrid)));
        }
    }
    float foam = exp(-abs(coarse - fine * 0.45) * (18.0 + band.z * 28.0));
    vec2 flow = vec2(sin(coarse * 12.0 + t), cos(fine * 18.0 - t));
    return vec3(foam, flow * foam);
}
