...
// Temporal inversion energy field: resonance cavity.
...
vec3 energyField(vec2 p, float age, vec4 band) {
    float t = clockTime();
    float modeX = 3.0 + floor(band.x * 8.0);
    float modeY = 4.0 + floor(band.y * 9.0);
    float standingX = sin((p.x + 0.8) * PI * modeX + age * 0.2);
    float standingY = sin((p.y + 0.6) * PI * modeY - age * 0.17);
    float harmonic = cos((p.x - p.y) * PI * (2.0 + band.z * 5.0) - t * 1.4);
    float energy = abs(standingX * standingY * harmonic);
    vec2 flow = vec2(standingY * harmonic, -standingX * harmonic);
