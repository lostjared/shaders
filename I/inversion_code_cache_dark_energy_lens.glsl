...
// Temporal inversion energy field: dark energy lens.
...
vec3 energyField(vec2 p, float age, vec4 band) {
    float t = clockTime();
    vec2 center = vec2(sin(t * 0.17 + age) * 0.16, cos(t * 0.19 - age) * 0.13);
    vec2 q = p - center;
    float radius = length(q) + 0.025;
    float potential = log(radius) * (5.0 + band.x * 8.0);
    float shell = 0.5 + 0.5 * cos(potential - t * 0.8 + age);
    float shear = exp(-radius * (2.0 + band.y * 2.0));
    vec2 flow = normalize(q) * (-shear / radius);
    return vec3(shell * shear, flow * 0.12);
}
