...
// Temporal inversion energy field: photon mesh.
...
vec3 energyField(vec2 p, float age, vec4 band) {
    float t = clockTime();
    vec2 axisA = normalize(vec2(1.0, 0.37));
    vec2 axisB = normalize(vec2(-0.42, 1.0));
    vec2 axisC = normalize(vec2(0.76, -0.65));
    float a = cos(dot(p, axisA) * (24.0 + band.x * 25.0) - t * 2.0);
    float b = cos(dot(p, axisB) * (27.0 + band.y * 23.0) + t * 1.7);
    float c = cos(dot(p, axisC) * (31.0 + band.z * 21.0) - t * 1.3);
    float nodes = pow(abs(a * b * c), 0.65);
    vec2 flow = axisA * a + axisB * b + axisC * c;
    return vec3(nodes, flow * 0.4);
}
