...
// History-manipulated in-the-sky effect: solar pillar array.
...
const float INTENSITY = 1.68;
const float PERSISTENCE = 0.77;
...
vec3 skyField(vec2 p, float age, vec4 band) {
    ...
    float angle = atan(p.y, p.x);
    float rays = pow(abs(cos(angle * 24.0 + age * 0.08)), 18.0);
    float pillars = pow(abs(cos(p.x * (18.0 + band.z * 18.0))), 14.0);
    float corona =
        exp(-radius * (2.2 + band.x)) * (0.55 + 0.45 * sin(radius * 32.0 - t * 1.4 + age));
    float energy = rays * corona + pillars * exp(-abs(p.y) * 2.8) * 0.55;
    vec2 flow = normalize(p + vec2(0.001)) * corona + vec2(0.0, pillars * 0.4);
