...
// Cache-driven perceptual energy field: dreamlogic mosaic.
...
const float HUE_OFFSET = 0.84;
const float DISTORTION = 0.98;
...
vec3 perceptualField(vec2 p, float age, vec4 band) {
    float t = clockTime();
    vec2 warped = p + vec2(fbm(p * 2.0 + t * 0.035), fbm(p * 2.0 + 8.0 - t * 0.03)) * 0.23;
    vec2 grid = warped * (5.0 + band.y * 3.0);
    vec2 id = floor(grid);
    ...
    for (int y = -1; y <= 1; ++y) {
        for (int x = -1; x <= 1; ++x) {
            ...
            vec2 point = 0.5 + 0.4 * sin(TAU * hash22(id + neighbor) + t * 0.12 + age);
    ...
    float boundary = exp(-(second - nearest) * (18.0 + band.z * 34.0));
    float roomTone = 0.5 + 0.5 * sin(hash21(id) * TAU + t * 0.18 - age * 0.4);
    return vec3(boundary * 0.75 + roomTone * 0.25, normalize(direction + vec2(0.001)) * boundary);
}
