...
// Temporal inversion energy field: synaptic canopy.
...
vec3 energyField(vec2 p, float age, vec4 band) {
    float t = clockTime();
    vec2 grid = p * (4.5 + band.y * 3.0);
    vec2 id = floor(grid);
    vec2 local = fract(grid) - 0.5;
    vec2 node = hash22(id) - 0.5;
    float soma = exp(-length(local - node) * (18.0 + band.z * 20.0));
    float dendrite = 0.0;
    vec2 direction = vec2(0.0);
    for (int branch = 0; branch < 4; ++branch) {
        float a = hash21(id + float(branch)) * TAU + t * 0.07;
        vec2 target = vec2(cos(a), sin(a)) * 0.65;
        float d = segmentDistance(local, node, target);
        float signal = exp(-d * 32.0);
        dendrite = max(dendrite, signal);
        direction += target * signal;
    }
    float pulse = 0.65 + 0.35 * sin(t * 3.0 - age + hash21(id) * TAU);
    return vec3(max(soma, dendrite) * pulse, direction * 0.3);
}
