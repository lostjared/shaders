...
// Temporal inversion energy field: vacuum bloom.
...
vec3 energyField(vec2 p, float age, vec4 band) {
    ...
    float symmetry = 6.0 + floor(band.z * 6.0);
    float boundary = 0.34 + cos(angle * symmetry + age * 0.12) * (0.08 + band.x * 0.05);
    float shell = exp(-abs(radius - boundary) * (38.0 + band.w * 65.0));
    float interior = exp(-radius * 3.0) * (0.5 + 0.5 * cos(radius * 28.0 - t * 2.0));
    vec2 flow = normalize(p + vec2(0.001)) * (interior - shell);
    return vec3(shell + interior * 0.35, flow);
}
