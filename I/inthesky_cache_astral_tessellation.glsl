...
// History-manipulated in-the-sky effect: astral tessellation.
...
vec3 skyField(vec2 p, float age, vec4 band) {
    ...
    for (int y = -1; y <= 1; ++y) {
        for (int x = -1; x <= 1; ++x) {
            ...
            vec2 point = 0.5 + 0.38 * sin(TAU * hash22(id + neighbor) + t * 0.16 + age);
    ...
    float edge = exp(-(second - nearest) * (20.0 + band.z * 36.0));
    float star = exp(-nearest * (14.0 + band.w * 24.0));
    return vec3(edge * 0.75 + star * 0.35, normalize(direction + vec2(0.001)) * edge);
}
