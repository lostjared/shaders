...
// Cache-driven perceptual energy field: peripheral drift.
...
const float HUE_OFFSET = 0.24;
const float DISTORTION = 1.28;
...
vec3 perceptualField(vec2 p, float age, vec4 band) {
    ...
    float eccentricity = smoothstep(0.12, 0.82, radius);
    float sectors = 14.0 + floor(band.z * 10.0);
    float rotor = sin(angle * sectors + radius * 22.0 - t * 0.8 + age * 0.33);
    float counter = cos(angle * (sectors - 3.0) - radius * 27.0 + t * 0.62);
    float drift = rotor * counter;
    float energy = eccentricity * (0.45 + 0.55 * abs(drift));
    vec2 tangent = vec2(-sin(angle), cos(angle));
    return vec3(energy, tangent * drift * eccentricity);
}
