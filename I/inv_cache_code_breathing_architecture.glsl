...
// Cache-driven perceptual energy field: breathing architecture.
...
const float HUE_OFFSET = 0.54;
const float DISTORTION = 0.68;
const float PERSISTENCE = 0.90;
...
vec3 perceptualField(vec2 p, float age, vec4 band) {
    float t = clockTime();
    float breath = 1.0 + 0.08 * sin(t * 0.32 - age * 0.12) + band.x * 0.07;
    vec2 q = p * breath;
    q.x += sin(q.y * 3.0 + t * 0.21) * 0.06;
    q.y += sin(q.x * 2.5 - t * 0.18) * 0.05;
    vec2 room = abs(fract(q * vec2(5.0, 4.0) + 0.5) - 0.5);
    float wallX = exp(-abs(room.x - 0.42) * 42.0);
    float wallY = exp(-abs(room.y - 0.42) * 42.0);
    float perspective = exp(-abs(q.y - q.x * 0.38) * 18.0) + exp(-abs(q.y + q.x * 0.38) * 18.0);
    float energy = max(wallX, wallY) * 0.7 + perspective * 0.3;
    vec2 flow = normalize(vec2(sin(q.y * 3.0), cos(q.x * 2.5)) + vec2(0.001));
    return vec3(energy, flow * (breath - 1.0) * 5.0);
}
