...
// Cache-driven perceptual energy field: lucid field.
...
const float HUE_OFFSET = 0.94;
const float DISTORTION = 0.82;
...
vec3 perceptualField(vec2 p, float age, vec4 band) {
    float t = clockTime();
    vec2 q = p + vec2(fbm(p * 2.4 + t * 0.04), fbm(p * 2.4 + 5.0 - t * 0.035)) * 0.18;
    float layerA = fbm(q * 4.0 + age * 0.09);
    float layerB = fbm(q * 8.0 - age * 0.07);
    float membrane = exp(-abs(layerA - layerB * 0.72 - 0.18) * (12.0 + band.z * 18.0));
    float lucidPulse = 0.7 + 0.3 * cos(length(p) * 9.0 - t * 0.34 + age * 0.25);
    vec2 flow = vec2(sin(layerA * TAU + t * 0.2), cos(layerB * TAU - t * 0.17));
    return vec3(membrane * lucidPulse, flow * membrane * 0.45);
}
