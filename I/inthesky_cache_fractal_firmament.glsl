...
// History-manipulated in-the-sky effect: fractal firmament.
...
const float INTENSITY = 1.58;
const float PERSISTENCE = 0.76;
...
vec3 skyField(vec2 p, float age, vec4 band) {
    ...
    for (int iteration = 0; iteration < 5; ++iteration) {
        float fi = float(iteration);
        q = abs(q) / max(dot(q, q), 0.16) - vec2(0.68 + band.x * 0.07, 0.48);
        q = rotate2(q, 0.25 + fi * 0.14 + age * 0.012);
        trace += exp(-abs(length(q) - 0.72) * (7.0 + fi));
    }
    float firmament = trace / 5.0;
    vec2 flow = normalize(q + vec2(0.001)) * firmament;
    return vec3(firmament, flow);
}
