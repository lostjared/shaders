...
// History-manipulated in-the-sky effect: prismatic thunderhead.
...
vec3 skyField(vec2 p, float age, vec4 band) {
    float t = clockTime();
    vec2 q = p + vec2(fbm(p * 2.0 + t * 0.04), fbm(p * 2.0 + 7.0 - t * 0.05)) * 0.28;
    float cloudLow = fbm(q * 3.0 + vec2(0.0, -t * 0.09));
    float cloudHigh = fbm(q * 8.0 - vec2(t * 0.06, t * 0.16));
    float thunderEdge = exp(-abs(cloudLow - cloudHigh * 0.72 - 0.18) * (15.0 + band.z * 25.0));
    float charge = 0.5 + 0.5 * sin((q.x + q.y) * 18.0 - t * 3.0 + age);
    vec2 flow = vec2(cloudLow - cloudHigh, -0.22 - band.x * 0.3);
    return vec3(thunderEdge * (0.6 + charge), flow);
}
