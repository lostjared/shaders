...
// Temporal inversion energy field: ion veil.
...
vec3 energyField(vec2 p, float age, vec4 band) {
    float t = clockTime();
    vec2 q = p;
    q.x += sin(q.y * 3.0 - t * 0.35 + age * 0.3) * 0.24;
    float coarse = fbm(q * vec2(2.2, 5.0) + vec2(t * 0.08, -t * 0.18));
    float fine = fbm(q * vec2(7.0, 13.0) - vec2(t * 0.12, t * 0.3));
    float curtain = exp(-abs(sin(q.x * (7.0 + band.y * 9.0) + coarse * 5.0)) * 5.0);
    float energy = curtain * (0.55 + fine) * (0.7 + band.z);
    vec2 flow = vec2(coarse - fine, -0.4 - band.x);
