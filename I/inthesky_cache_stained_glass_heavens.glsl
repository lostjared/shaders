...
// History-manipulated in-the-sky effect: stained glass heavens.
...
const float SEGMENTS = 26.0;
const float HUE_OFFSET = 0.94;
const float INTENSITY = 1.46;
const float PERSISTENCE = 0.81;
...
vec3 skyField(vec2 p, float age, vec4 band) {
    float t = clockTime();
    vec2 q = rotate2(p, 0.12 * sin(t * 0.11) + age * 0.01);
    vec2 grid = q * (6.0 + band.y * 3.0);
    vec2 id = floor(grid);
    ...
    float lead = exp(-(second - nearest) * (24.0 + band.z * 40.0));
    float glass = 0.35 + 0.65 * sin(identity * TAU + t * 0.16 - age * 0.3);
    return vec3(lead * 0.8 + glass * 0.28, normalize(direction + vec2(0.001)) * lead);
}
