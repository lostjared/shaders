...
// Temporal inversion energy field: electrostatic dunes.
...
vec3 energyField(vec2 p, float age, vec4 band) {
    float t = clockTime();
    float duneA = sin(p.x * (6.0 + band.x * 8.0) + sin(p.y * 3.0 - t * 0.4) * 2.0);
    float duneB = cos(p.y * (8.0 + band.y * 9.0) + sin(p.x * 4.0 + t * 0.3) * 2.5);
    float charge = duneA - duneB;
    float terraces = 1.0 - smoothstep(0.04, 0.16, abs(fract(charge * 3.5) - 0.5));
    vec2 flow = normalize(vec2(duneB, -duneA) + vec2(0.001));
    return vec3(terraces * (0.65 + 0.35 * abs(charge)), flow);
}
