...
// Temporal inversion energy field: flux lattice.
...
vec3 energyField(vec2 p, float age, vec4 band) {
    float t = clockTime();
    float gx = sin(p.x * (11.0 + band.x * 24.0) + t * 0.7 - age * 0.4);
    float gy = sin(p.y * (13.0 + band.y * 20.0) - t * 0.6 + age * 0.5);
    float diagonal = cos((p.x + p.y) * 9.0 + band.z * 8.0 + age);
    float energy = abs(gx * gy) + 0.35 * abs(diagonal);
    vec2 flow = vec2(gy - diagonal, diagonal - gx);
