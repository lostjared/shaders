...
// Temporal inversion energy field: quasicrystal field.
...
vec3 energyField(vec2 p, float age, vec4 band) {
    ...
    for (int ray = 0; ray < 7; ++ray) {
        float direction = float(ray) * TAU / 7.0 + age * 0.017;
        vec2 axis = vec2(cos(direction), sin(direction));
        float wave = cos(dot(p, axis) * (17.0 + band.z * 22.0) + t * (0.4 + float(ray) * 0.07));
        sum += wave;
        flow += axis * wave;
    }
    float energy = pow(abs(sum / 7.0), 1.6);
    return vec3(energy, flow / 7.0);
}
