...
// Temporal inversion energy field: borromean energy.
...
vec3 energyField(vec2 p, float age, vec4 band) {
    float t = clockTime();
    vec2 c0 = vec2(-0.24, 0.12);
    vec2 c1 = vec2(0.24, 0.12);
    vec2 c2 = vec2(0.0, -0.24);
    float radius = 0.31 + band.x * 0.06;
    float ring0 = abs(length(p - c0) - radius);
    float ring1 = abs(length(p - c1) - radius);
    float ring2 = abs(length(p - c2) - radius);
    float braid = exp(-min(ring0, min(ring1, ring2)) * (45.0 + band.z * 70.0));
    float phase = sin((ring0 - ring1 + ring2) * 80.0 - t * 2.0 + age);
    vec2 flow = normalize(vec2(ring1 - ring0, ring2 - ring1) + vec2(0.001));
    return vec3(braid * (0.7 + 0.3 * phase), flow * braid);
}
