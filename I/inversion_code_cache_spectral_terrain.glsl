...
// Temporal inversion energy field: spectral terrain.
...
vec3 energyField(vec2 p, float age, vec4 band) {
    float t = clockTime();
    vec2 q = p + vec2(t * 0.025, -age * 0.07);
    float lowland = fbm(q * 2.4);
    float ridge = 1.0 - abs(2.0 * fbm(q * 5.0 + 4.2) - 1.0);
    float detail = fbm(q * 11.0 - t * 0.03);
    float terrain = lowland * 0.5 + ridge * 0.35 + detail * 0.15;
    float isobar = 1.0 - smoothstep(0.025, 0.10, abs(fract((terrain + band.y * 0.2) * 12.0) - 0.5));
    vec2 flow = vec2(cos(terrain * 17.0), sin(terrain * 19.0));
    return vec3(isobar, flow * terrain);
}
