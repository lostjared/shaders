...
// Temporal inversion energy field: charge topography.
...
vec3 energyField(vec2 p, float age, vec4 band) {
    float t = clockTime();
    float terrain = fbm(p * 3.2 + vec2(t * 0.04, -age * 0.11));
    terrain += fbm(rotate2(p, 0.7) * 6.4 - t * 0.03) * 0.45;
    float levels = abs(fract((terrain + band.x * 0.3) * (9.0 + band.z * 7.0)) - 0.5);
    float contour = 1.0 - smoothstep(0.03, 0.14, levels);
    vec2 flow = vec2(cos(terrain * TAU + t), sin(terrain * TAU - t));
    return vec3(contour, flow * (0.3 + terrain));
}
