...
// Temporal inversion energy field: tachyon ribbons.
...
vec3 energyField(vec2 p, float age, vec4 band) {
    float t = clockTime();
    float curveA = p.y - sin(p.x * 5.0 + t * 0.9 + age * 0.4) * (0.22 + band.x * 0.2);
    float curveB = p.x - cos(p.y * 6.0 - t * 0.7 + age * 0.3) * (0.24 + band.y * 0.18);
    float curveC = p.y + p.x * 0.45 - sin((p.x - p.y) * 4.0 + t) * 0.16;
    float ribbon = exp(-min(abs(curveA), min(abs(curveB), abs(curveC))) * (32.0 + band.z * 65.0));
    float packet = 0.55 + 0.45 * sin((p.x + p.y) * 25.0 - t * 7.0 + age);
    vec2 flow = normalize(vec2(1.0, cos(p.x * 5.0 + t)) + vec2(0.001));
    return vec3(ribbon * packet, flow * ribbon);
}
