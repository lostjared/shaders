...
// Temporal inversion energy field: moire tensor.
...
vec3 energyField(vec2 p, float age, vec4 band) {
    float t = clockTime();
    float angleA = 0.18 + band.x * 0.4 + age * 0.025;
    float angleB = -0.23 - band.y * 0.35 - age * 0.021;
    vec2 qa = rotate2(p, angleA);
    vec2 qb = rotate2(p, angleB);
    float linesA = sin(qa.x * (22.0 + band.z * 25.0) + t);
    float linesB = sin(qb.x * (24.0 + band.w * 27.0) - t * 0.9);
    float tensor = linesA * linesB;
    vec2 flow = vec2(linesB - linesA, linesA + linesB);
    return vec3(abs(tensor), flow * 0.35);
}
