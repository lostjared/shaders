...
// Cache-driven perceptual energy field: cortical form constants.
...
const float DISTORTION = 0.92;
const float PERSISTENCE = 0.84;
...
vec3 perceptualField(vec2 p, float age, vec4 band) {
    ...
    float lattice = cos(p.x * (15.0 + band.x * 12.0)) * cos(p.y * (17.0 + band.y * 13.0));
    float cobweb = cos(angle * (8.0 + floor(band.z * 6.0)) + log(radius + 0.08) * 9.0 - t * 0.45);
    float honeycomb = cos(p.x * 13.0) + cos(dot(p, vec2(0.5, 0.8660254)) * 13.0) +
                      cos(dot(p, vec2(-0.5, 0.8660254)) * 13.0);
    float blend = mix(lattice, cobweb, 0.5 + 0.5 * sin(t * 0.17 + age * 0.2));
    float energy = pow(abs(blend * 0.45 + honeycomb * 0.18), 1.5);
    vec2 flow = vec2(-sin(angle), cos(angle)) * cobweb * 0.35;
