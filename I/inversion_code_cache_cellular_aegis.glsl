...
// Temporal inversion energy field: cellular aegis.
...
vec3 energyField(vec2 p, float age, vec4 band) {
    ...
    for (int y = -1; y <= 1; ++y) {
        for (int x = -1; x <= 1; ++x) {
            ...
            vec2 point = 0.5 + 0.38 * sin(TAU * hash22(cellId + neighbor) + t * 0.25 + age);
    ...
    float membrane = exp(-(second - nearest) * (18.0 + band.z * 25.0));
    return vec3(membrane, normalize(nearestVector + vec2(0.001)) * membrane);
}
