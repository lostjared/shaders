...
// History-manipulated in-the-sky effect: horizon implosion.
...
vec3 skyField(vec2 p, float age, vec4 band) {
    float t = clockTime();
    float perspective = 1.0 / max(abs(p.y + 0.34), 0.06);
    float horizon =
        exp(-abs(p.y + 0.18 + sin(p.x * 4.0 + t * 0.24) * 0.035) * (45.0 + band.z * 55.0));
    float lanes = sin(p.x * perspective * (3.0 + band.y * 5.0) + age);
    float depthBands = cos(perspective * 2.4 - t * 1.1 + age * 0.45);
    float energy = horizon + abs(lanes * depthBands) * smoothstep(-0.1, 0.7, p.y) * 0.65;
    vec2 flow = vec2(lanes, -depthBands) * 0.35;
