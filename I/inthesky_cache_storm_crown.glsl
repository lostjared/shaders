...
// History-manipulated in-the-sky effect: storm crown.
...
const float INTENSITY = 1.72;
const float PERSISTENCE = 0.72;
...
vec3 skyField(vec2 p, float age, vec4 band) {
    ...
    float radius = length(p);
    float angle = atan(p.y, p.x);
    float crown = exp(-abs(radius - 0.38 - cos(angle * 14.0) * 0.07) * (36.0 + band.z * 60.0));
    float forks = pow(abs(sin(angle * 28.0 + radius * 45.0 - t * 2.2 + age)), 12.0);
    float cloud = fbm(p * 4.0 + vec2(t * 0.08, -t * 0.11));
    float energy = crown * (0.55 + forks) + pow(cloud, 4.0) * 0.25;
    vec2 flow = vec2(-sin(angle), cos(angle)) * (crown + forks * 0.2);
