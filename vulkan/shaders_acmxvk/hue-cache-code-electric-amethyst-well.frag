...
// hue-cache-code-electric-amethyst-well
// Block-scan glitch well with audio-triggered horizontal tearing.
...
const float RIGHT_TURN = 0.20;
const float RIGHT_SCALE = 0.64;
const float HUE_OFFSET = 2.84;
const int FEEDBACK_STYLE = 3;
...
void main() {
    ...
    float row = floor(tc.y * (48.0 + treble * 48.0));
    float row_hash = hash_21(vec2(row, floor(time_f * 7.0)));
    float tear_gate = step(0.72 - bass * 0.25, row_hash);
    float tear = (row_hash - 0.5) * tear_gate * (0.12 + bass * 0.12);
    effect_point.x += tear * aspect;
    effect_point.y = (floor(tc.y * (72.0 + middle * 40.0)) + 0.5) /
                     (72.0 + middle * 40.0) - center.y;
    field = tear_gate * 2.0 - 1.0;
    feature = tear_gate * step(0.5, fract(row_hash * 9.0));
