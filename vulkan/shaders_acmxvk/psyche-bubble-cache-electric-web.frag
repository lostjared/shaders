...
// psyche-bubble-cache-electric-web
...
vec3 psyche_field(vec2 point, float bass, float middle, float treble, float age) {
    ...
    float radial_web = sin(angle * 13.0 + sin(radius * 17.0 - time * 2.0) * 2.5);
    float electric = sin(point.x * 31.0 + fbm(point * 8.0 + time * 0.2) * 9.0);
    float filaments = pow(abs(radial_web * electric), 7.0);
    return vec3(radial_web + electric * 0.3, filaments, angle / TAU + fbm(point * 4.0) + age);
}
...
vec2 psyche_warp(vec2 uv, vec2 point, vec3 signal, float bass, float middle,
    ...
    uv += vec2(sin(point.y * 23.0 + signal.x * 3.0),
               sin(point.x * 29.0 - signal.x * 2.0)) * (0.012 + treble * 0.035) * depth;
...
float layer_gate(vec2 point, vec3 signal, float progress, int index) {
    ...
    return 0.2 + 0.8 * pow(max(signal.y, 0.0), 0.45);
}
