...
// hue-cache-code-amethyst-vortex
// Mouse-steered amethyst vortex with polar shear and recursive chroma.
...
const float RIGHT_TURN = 2.40;
const float RIGHT_SCALE = 0.35;
const float HUE_OFFSET = 0.08;
...
void main() {
    ...
    float swirl = radius * (12.0 + bass * 8.0) - time_f * 2.2;
    effect_point = rotate_2d(swirl * 0.16 + sin(swirl) * 0.22) * point;
    field = sin(angle * 6.0 - radius * 18.0 - time_f * 4.0);
    feature = pow(max(field, 0.0), 5.0) * exp(-radius * 1.8);
    effect_point += radial_direction * field * (0.025 + bass * 0.035);
