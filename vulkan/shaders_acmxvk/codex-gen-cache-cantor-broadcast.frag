...
// Codex cache concept 078: cantor broadcast.
...
vec3 palette(float phase) {
    return 0.5 + 0.5 * cos(TAU * (phase + 0.549 + vec3(0.04, 0.37, 0.69)));
}
...
vec2 concept_space(vec2 point, float age, vec3 old_audio,
    ...
    float steps = 8.0 + floor(high * 12.0);
    q.y = floor(q.y * steps + age * 3.0) / steps;
    q.x += floor((q.y + 1.0) * steps) * (old_mid - 0.2) * 0.008;
    return q;
}
