...
// psyche-bubble-cache-magnetic-twins
...
vec3 psyche_field(vec2 point, float bass, float middle, float treble, float age) {
    ...
    vec2 pole_a = vec2(-0.24 + sin(time * 0.23) * 0.08, 0.0);
    vec2 pole_b = vec2(0.24 + cos(time * 0.19) * 0.08, 0.0);
    float first = sin(length(point - pole_a) * (31.0 + bass * 17.0) - time * 6.0);
    float second = cos(length(point - pole_b) * (29.0 + treble * 14.0) - time * 5.0);
    return vec3(first - second, pow(abs(first * second), 5.0),
                first * 0.19 + second * 0.13 + age);
}
...
vec2 psyche_warp(vec2 uv, vec2 point, vec3 signal, float bass, float middle,
    ...
    vec2 pole = vec2(sign(point.x) * 0.23, 0.0);
    vec2 force = point - pole;
    uv += force / max(dot(force, force), 0.035) * signal.x * 0.0045 * depth;
...
float layer_gate(vec2 point, vec3 signal, float progress, int index) {
    ...
    return 0.25 + 0.75 * exp(-abs(length(point - vec2((progress - 0.5) * 0.55, 0.0)) - 0.2) * 12.0);
}
