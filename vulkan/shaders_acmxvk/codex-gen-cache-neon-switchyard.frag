...
// Codex cache concept 005: neon switchyard.
...
vec3 palette(float phase) {
    return 0.5 + 0.5 * cos(TAU * (phase + 0.548 + vec3(0.04, 0.37, 0.69)));
}
...
vec2 concept_space(vec2 point, float age, vec3 old_audio,
    ...
    float horizon = max(0.12, q.y + 0.62);
    q.x /= 0.72 + horizon * (0.45 + age * 0.2);
    q.y = 0.18 / horizon - 0.24 + age * old_bass * 0.08;
    return q;
}
...
vec3 blend_memory(vec3 memory, vec3 live, float field, float age, vec3 old_audio) {
    vec3 solar = 1.0 - exp(-memory * (1.2 + old_audio.x));
    return mix(memory.brg, solar, field * 0.72);
}
...
void main() {
    ...
    for (int index = 0; index < SIZE; ++index) {
        ...
        float weight = exp(-age * (1.5 + high)) * (0.52 + field * 0.7);
        accumulated += treated * weight;
        total_weight += weight;
    }

    vec3 result = accumulated / max(total_weight, 0.001);
    result += palette(atan(live_q.y, live_q.x) / TAU + live_field) * live_field * high * 0.16;
