...
// hue-cache-code-kaleido-spectrum-bowl
// Pixel mosaic bowl folded through a sixteen-sided spectrum mirror.
...
const float RIGHT_TURN = 2.10;
const float RIGHT_SCALE = 1.00;
const float HUE_OFFSET = 2.96;
...
void main() {
    ...
    float pixel_count = 20.0 + floor(middle * 44.0);
    vec2 pixel_uv = (floor(tc * pixel_count) + 0.5) / pixel_count;
    vec2 pixel_point = (pixel_uv - center) * vec2(aspect, 1.0);
    float bowl = max(0.20, 1.0 - dot(pixel_point, pixel_point) * 0.75);
    effect_point = kaleidoscope(pixel_point * bowl,
                                10.0 + floor(treble * 8.0));
    field = sin(length(effect_point) * 32.0 - time_f * 5.0);
    feature = pow(max(field, 0.0), 8.0);
