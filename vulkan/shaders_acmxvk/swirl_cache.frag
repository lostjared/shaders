...
layout(location = 0) in vec2 tc;  // The default texture coordinate passed from your vertex shader

// Our five input textures
layout(set = 0, binding = 0) uniform sampler2D samp;   // base
...
void main(void)
{
    ...
    vec4 one    = texture(history, vec3(uv1, float(CACHE_HISTORY_LAYER(0))));
    vec4 two    = texture(history, vec3(uv2, float(CACHE_HISTORY_LAYER(1))));
    vec4 three  = texture(history, vec3(uv3, float(CACHE_HISTORY_LAYER(2))));
    vec4 four   = texture(history, vec3(uv4, float(CACHE_HISTORY_LAYER(3))));
