...
// psyche-bubble-cache-yin-yang-vortex
...
vec3 psyche_field(vec2 point, float bass, float middle, float treble, float age) {
    ...
    vec2 lobe_a = point - vec2(0.0, 0.22);
    vec2 lobe_b = point + vec2(0.0, 0.22);
    float yin = sin(length(lobe_a) * (31.0 + bass * 15.0) - time * 5.0);
    float yang = cos(length(lobe_b) * (31.0 + treble * 15.0) + time * 5.0);
    float boundary = exp(-abs(point.x + 0.17 * sin(point.y * 8.0)) * 14.0);
    float color_phase = 0.5 +
                        sin(angle + radius * 4.0 - time * 0.35) * 0.22 +
                        cos(angle * 2.0 + age * TAU) * 0.1;
    return vec3(yin - yang, boundary + pow(abs(yin * yang), 7.0) * 0.3,
                color_phase + yin * 0.12 + age);
}
...
vec2 psyche_warp(vec2 uv, vec2 point, vec3 signal, float bass, float middle,
    ...
    float turn = 0.14 + age * 0.65;
    vec2 upper_center = vec2(0.0, 0.22);
    vec2 lower_center = vec2(0.0, -0.22);
    vec2 upper_spun = rotate_2d(turn + signal.x * 0.06) *
                      (point - upper_center) + upper_center;
    vec2 lower_spun = rotate_2d(-turn + signal.x * 0.06) *
                      (point - lower_center) + lower_center;
    float divider = point.y + sin(point.x * 7.0 + time_f * 0.4) * 0.055;
    float lobe_mix = smoothstep(-0.14, 0.14, divider);
    vec2 spun = mix(lower_spun, upper_spun, lobe_mix);
    uv = spun / vec2(iResolution.x / max(iResolution.y, 1.0), 1.0) + 0.5;
...
float layer_gate(vec2 point, vec3 signal, float progress, int index) {
    ...
    return 0.24 + 0.76 * smoothstep(0.0, 0.5, abs(point.y + sin(point.x * 7.0) * 0.08));
}
