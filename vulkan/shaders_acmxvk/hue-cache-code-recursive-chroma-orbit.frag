...
// hue-cache-code-recursive-chroma-orbit
// Multiple orbital gravity lenses separate color and cache trajectories.
...
const float RIGHT_TURN = 3.00;
const float RIGHT_SCALE = 0.70;
const float HUE_OFFSET = 2.72;
const int FEEDBACK_STYLE = 0;
...
void main() {
    ...
    vec2 orbit_a = vec2(cos(time_f * 0.7), sin(time_f * 0.7)) * 0.28;
    vec2 orbit_b = vec2(cos(-time_f * 0.5), sin(-time_f * 0.5)) * 0.38;
    vec2 delta_a = point - orbit_a;
    vec2 delta_b = point - orbit_b;
    effect_point = point - delta_a * 0.035 / (dot(delta_a, delta_a) + 0.025) -
                   delta_b * 0.025 / (dot(delta_b, delta_b) + 0.025);
    field = sin(length(delta_a) * 28.0) + cos(length(delta_b) * 24.0);
    feature = exp(-length(delta_a) * 12.0) + exp(-length(delta_b) * 12.0);
