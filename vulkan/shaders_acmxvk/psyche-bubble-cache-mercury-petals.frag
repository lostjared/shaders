...
// psyche-bubble-cache-mercury-petals
...
vec3 psyche_field(vec2 point, float bass, float middle, float treble, float age) {
    ...
    float petal = cos(angle * (7.0 + floor(treble * 5.0)) - radius * 14.0 + time * 2.0);
    float mercury = sin(radius * (27.0 + bass * 16.0) + petal * 4.0 - time * 5.0 + age * 12.0);
    return vec3(mercury, pow(max(petal, 0.0), 6.0) * exp(-radius * 1.4),
                petal * 0.2 + radius + age);
}
...
vec2 psyche_warp(vec2 uv, vec2 point, vec3 signal, float bass, float middle,
    ...
    float petal = cos(angle * (7.0 + floor(treble * 5.0)));
    uv += radial * petal * signal.x * 0.045 * depth;
    uv += tangent * middle * 0.03 * sin(radius * 18.0);
...
float layer_gate(vec2 point, vec3 signal, float progress, int index) {
    ...
    return 0.22 + 0.78 * pow(max(cos(angle * (7.0 + slot * 0.5)), 0.0), 5.0);
}
