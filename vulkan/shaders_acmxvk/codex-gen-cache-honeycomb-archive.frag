...
// Codex cache concept 006: honeycomb archive.
...
vec3 palette(float phase) {
    return 0.5 + 0.5 * cos(TAU * (phase + 0.685 + vec3(0.04, 0.37, 0.69)));
}
...
float concept_field(vec2 q, float age, float bass, float middle, float high) {
    vec2 h = hex_coord(q * (6.0 + middle * 3.0));
    return exp(-abs(max(abs(h.x), dot(abs(h), vec2(0.5, 0.866))) - 0.42) * (34.0 + high * 18.0));
}
