...
// psyche-bubble-cache-lava-lantern
...
vec3 psyche_field(vec2 point, float bass, float middle, float treble, float age) {
    ...
    float blob = fbm(point * 3.4 + vec2(sin(time * 0.27 + age * 4.0),
                                        cos(time * 0.21 - age * 5.0)));
    float lava = sin((point.y + blob * 0.7) * (13.0 + middle * 8.0) - time * 2.0);
    float lamp = smoothstep(0.54, 0.74, blob + bass * 0.15);
    return vec3(lava, lamp, blob + point.y * 0.3 + age);
}
...
vec2 psyche_warp(vec2 uv, vec2 point, vec3 signal, float bass, float middle,
    ...
    vec2 flow = vec2(fbm(point * 3.0 + 8.0), fbm(point * 3.0 - 5.0)) - 0.5;
    uv += flow * (0.045 + middle * 0.06) * depth;
    uv.y += signal.x * 0.018;
...
float layer_gate(vec2 point, vec3 signal, float progress, int index) {
    ...
    return 0.25 + 0.75 * smoothstep(0.35, 0.8, signal.y);
}
