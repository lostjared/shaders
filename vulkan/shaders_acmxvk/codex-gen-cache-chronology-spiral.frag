...
// Codex cache concept 049: chronology spiral.
...
vec3 palette(float phase) {
    return 0.5 + 0.5 * cos(TAU * (phase + 0.576 + vec3(0.04, 0.37, 0.69)));
}
...
vec2 concept_space(vec2 point, float age, vec3 old_audio,
    ...
    float radius = max(length(q), 0.002);
    float angle = atan(q.y, q.x) + log(radius + 0.08) * (1.4 + middle) - time_f * 0.14;
    radius = fract(log(radius + 0.08) * -0.55 + age * 0.18 + 1.0);
    q = vec2(cos(angle), sin(angle)) * radius * 0.72;
    return q;
}
