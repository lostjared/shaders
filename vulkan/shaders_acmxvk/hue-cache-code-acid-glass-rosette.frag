...
// hue-cache-code-acid-glass-rosette
// Voronoi broken-glass refraction arranged into an acid rosette.
...
const float RIGHT_TURN = 1.10;
const float RIGHT_SCALE = 0.42;
const float HUE_OFFSET = 0.92;
const int FEEDBACK_STYLE = 1;
...
void main() {
    ...
    vec2 glass_point = kaleidoscope(point, 7.0 + floor(treble * 4.0));
    vec2 glass = voronoi_edges(glass_point * (7.0 + middle * 4.0));
    float crack = smoothstep(0.08, 0.0, glass.y - glass.x);
    vec2 cell_direction = vec2(noise_21(floor(glass_point * 8.0)),
                               noise_21(floor(glass_point * 8.0) + 4.7)) - 0.5;
    effect_point = point + cell_direction * (0.05 + bass * 0.04) * (1.0 - crack);
    field = crack * 2.0 - 1.0;
    feature = crack;
