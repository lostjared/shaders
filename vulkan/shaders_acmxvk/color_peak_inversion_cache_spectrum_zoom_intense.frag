...
// EXTREME historical FFT flow - PROMINENT RIGHT CLICK TEST
...
void main() {
    ...
    // Right-click hue shift and zoom control (EXAGGERATED)
    ...
    if (iMouse.w > 0.0) {
        ...
        // Massive multipliers on the horizontal axis
        userHueShift.r = (mouseUV.x - 0.5) * 8.0;
        userHueShift.g = -(mouseUV.x - 0.5) * 6.0;
        userHueShift.b = (mouseUV.x - 0.5) * 5.0;
        
        // Massive multiplier on the vertical axis (+/- 0.8 scale per layer)
    ...
    for (int i = 0; i < 8; i++) {
        ...
        // Calculate zoom with the exaggerated user shift
        float rawZoom = 0.95 + 0.02 * sin(time_f * 0.5) - (h_bass * 0.12) + userZoomShift; 
        
        // Safe clamp to prevent pow() from failing on negative values during extreme zoom out
        ...
        // Apply massive hue shift overdrive scaled by layer generation
