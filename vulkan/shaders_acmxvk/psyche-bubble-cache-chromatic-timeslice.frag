...
// psyche-bubble-cache-chromatic-timeslice
...
vec3 psyche_field(vec2 point, float bass, float middle, float treble, float age) {
    ...
    float slice = floor((point.y + 0.6) * (13.0 + treble * 8.0));
    float phase = hash_21(vec2(slice, floor(time * 2.0))) * TAU;
    float scan = sin(point.x * (19.0 + bass * 12.0) + phase + age * 17.0);
    float seam = exp(-abs(fract((point.y + 0.6) * 14.0) - 0.5) * 18.0);
    return vec3(scan, seam * (0.4 + 0.6 * abs(scan)), phase / TAU + age);
}
...
vec2 psyche_warp(vec2 uv, vec2 point, vec3 signal, float bass, float middle,
    ...
    float row = floor((point.y + 0.7) * 14.0);
    uv.x += (hash_21(vec2(row, floor(time_f * 2.0))) - 0.5) *
            (0.08 + bass * 0.12) * depth;
...
float layer_gate(vec2 point, vec3 signal, float progress, int index) {
    ...
    return 0.18 + 0.82 * step(0.5, fract((point.y + 0.7) * 14.0 + slot * 0.37));
}
