...
// psyche-bubble-cache-shockwave-crown
...
vec3 psyche_field(vec2 point, float bass, float middle, float treble, float age) {
    ...
    float shock = sin(radius * (37.0 + bass * 24.0) - time * 9.0 - age * 15.0);
    float crown = pow(abs(cos(angle * (12.0 + floor(treble * 6.0)) + time)), 12.0);
    return vec3(shock, crown * exp(-radius * 1.9) + pow(max(shock, 0.0), 9.0) * 0.35,
                radius * 1.4 + angle / TAU + age);
}
...
vec2 psyche_warp(vec2 uv, vec2 point, vec3 signal, float bass, float middle,
    ...
    uv += radial * signal.x * (0.025 + bass * 0.09) * depth;
    uv += tangent * pow(abs(cos(angle * 12.0)), 8.0) * treble * 0.03;
...
float layer_gate(vec2 point, vec3 signal, float progress, int index) {
    ...
    return 0.22 + 0.78 * pow(abs(cos(angle * (8.0 + slot))), 10.0);
}
