...
// Codex cache concept 031: moire chronograph.
...
vec3 palette(float phase) {
    return 0.5 + 0.5 * cos(TAU * (phase + 0.110 + vec3(0.04, 0.37, 0.69)));
}
...
vec2 concept_space(vec2 point, float age, vec3 old_audio,
    ...
    vec2 woven = q;
    woven.x += sin(q.y * 24.0 + time_f + age * 8.0) * (0.014 + old_high * 0.025);
    woven.y += sin(q.x * 19.0 - time_f * 0.7 - age * 6.0) * (0.014 + old_mid * 0.03);
    q = mix(q, woven.yx * vec2(1.0, -1.0), 0.08 + age * 0.1);
    return q;
}

float concept_field(vec2 q, float age, float bass, float middle, float high) {
    float bands = abs(sin((q.x + sin(q.y * 3.0)) * (18.0 + high * 9.0) + age * 4.0));
    return pow(1.0 - bands, 4.0);
}
