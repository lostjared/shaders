...
// psyche-bubble-cache-mobius-ribbon
...
vec3 psyche_field(vec2 point, float bass, float middle, float treble, float age) {
    ...
    float ribbon = sin(angle * 2.0 + sin(radius * 9.0 - time) * 2.0 + age * TAU);
    float half_twist = cos(angle + radius * 16.0 - time * 2.0);
    float strip = exp(-abs(ribbon) * (8.0 + treble * 6.0));
    float color_phase = 0.5 +
                        sin(angle + radius * 3.0 - time * 0.35) * 0.23 +
                        cos(angle * 2.0 - age * TAU) * 0.11;
    return vec3(half_twist, strip * (0.45 + 0.55 * abs(half_twist)),
                color_phase + half_twist * 0.2 + age);
}
...
vec2 psyche_warp(vec2 uv, vec2 point, vec3 signal, float bass, float middle,
    ...
    float ribbon_twist = sin(angle + age * TAU) * PI * 0.27 +
                         signal.x * PI * 0.12;
    vec2 strip = rotate_2d(ribbon_twist) * point;
    uv = strip / vec2(iResolution.x / max(iResolution.y, 1.0), 1.0) + 0.5;
    uv += tangent * signal.x * 0.025;
...
float layer_gate(vec2 point, vec3 signal, float progress, int index) {
    ...
    float traveling_fold = sin(angle + progress * TAU);
    return 0.22 + 0.78 * exp(-abs(traveling_fold) * 9.0);
}
