...
// hue-cache-code-purple-wormhole-prism
// Inversion fractal wormhole with prismatic orbit traps.
...
const float RIGHT_TURN = 3.80;
const float RIGHT_SCALE = 0.96;
const float HUE_OFFSET = 2.12;
...
void main() {
    ...
    vec2 fractal_point = point;
    float trap = 10.0;
    for (int iteration = 0; iteration < 5; ++iteration) {
        float distance_squared = max(dot(fractal_point, fractal_point), 0.035);
        fractal_point = abs(fractal_point) / distance_squared -
                        vec2(0.74 + bass * 0.08, 0.42);
        fractal_point = rotate_2d(0.35 + time_f * 0.025) * fractal_point;
        trap = min(trap, abs(length(fractal_point) - 0.8));
    }
    effect_point = fractal_point * 0.12;
    field = sin(trap * 70.0 - time_f * 4.0);
    feature = exp(-trap * 22.0);
