...
// Left Click: Center Control & Directional History Stretch
// Right Click: Zoom Control
// Palette: Strict #5D0774 Hex Targeting
...
// Procedural Palette locked to #5D0774
vec3 acid(float t) {
    vec3 hexTarget = vec3(0.3647, 0.0275, 0.4549);
    vec3 a = hexTarget; 
    vec3 b = hexTarget * 0.6; 
...
void main() {
    ...
    if (iMouse.z > 0.0) {
        ...
        // Calculate directional drag relative to the screen center
        // The further you drag, the stronger the stretching effect
    ...
    for (int i = 0; i < 8; i++) {
        ...
        // Force the older frames to bleed directly into the #5D0774 hex code
        vec3 targetPurple = vec3(0.3647, 0.0275, 0.4549);
        
        // Blend the recursive frames toward the target color, scaled by audio mids for reactivity
        cached.rgb = mix(cached.rgb, targetPurple * (1.0 + h_mid * 0.5), shift * 1.5);
        
        // Ensure the green channel never overtakes the target
        cached.g *= 0.5;
