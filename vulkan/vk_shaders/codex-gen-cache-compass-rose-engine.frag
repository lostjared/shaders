...
// Codex cache concept 008: compass rose engine.
...
vec3 palette(float phase) {
    return 0.5 + 0.5 * cos(TAU * (phase + 0.959 + vec3(0.04, 0.37, 0.69)));
}
...
vec2 concept_space(vec2 point, float age, vec3 old_audio,
    ...
    float slices = 7.0 + floor(high * 6.0);
    float sector_angle = TAU / slices;
    float local_angle = mod(angle + sector_angle * 0.5, sector_angle) -
                        sector_angle * 0.5;
    float sector_phase = local_angle / sector_angle * TAU;
    angle += sin(sector_phase) * age * (0.12 + old_mid * 0.2);
    q = vec2(cos(angle), sin(angle)) * radius * (1.0 - age * 0.05);
    return q;
}

float concept_field(vec2 q, float age, float bass, float middle, float high) {
    float spiral = abs(sin(atan(q.y, q.x) * 5.0 + log(length(q) + 0.04) * 11.0 - time_f));
    return pow(1.0 - spiral, 4.0);
}

vec3 blend_memory(vec3 memory, vec3 live, float field, float age, vec3 old_audio) {
    float luma = dot(memory, vec3(0.299, 0.587, 0.114));
    return mix(memory, palette(luma + age) * luma, 0.35 + field * 0.4);
}
...
void main() {
    ...
    for (int index = 0; index < SIZE; ++index) {
        ...
        float weight = pow(1.0 - age * 0.72, 2.0) * (0.48 + old_audio.y * 0.45);
        accumulated += treated * weight;
        total_weight += weight;
    }

    vec3 result = accumulated / max(total_weight, 0.001);
    result = (result - 0.5) * (1.05 + live_field * 0.12 + amp_smooth * 0.2) + 0.5;
