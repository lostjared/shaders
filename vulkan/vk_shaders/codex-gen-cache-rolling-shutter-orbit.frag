...
// Codex cache concept 087: rolling shutter orbit.
...
vec3 palette(float phase) {
    return 0.5 + 0.5 * cos(TAU * (phase + 0.782 + vec3(0.04, 0.37, 0.69)));
}
...
vec2 concept_space(vec2 point, float age, vec3 old_audio,
    ...
    float tube = 1.0 / (0.45 + length(q));
    q *= mix(1.0, tube, 0.18 + age * 0.22);
    q.x += sin(q.y * 18.0 + time_f + age * 7.0) * old_high * 0.018;
    return q;
}

float concept_field(vec2 q, float age, float bass, float middle, float high) {
    float scan = abs(fract((q.y + sin(q.x * 4.0) * 0.04) * (55.0 + high * 35.0) + age * 7.0) - 0.5);
    return exp(-scan * (16.0 + high * 12.0));
}
