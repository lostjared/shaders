...
void main() {
    ...
    // Right-click hue shift control
    vec3 userHueShift = vec3(0.0);
    if (iMouse.w > 0.0) {
        vec2 mouseUV = iMouse.xy / iResolution.xy;
        // Map mouse 0.0 to 1.0 range into an aggressive -1.0 to 1.0 push
        userHueShift.r = (mouseUV.x - 0.5) * 2.0;
        userHueShift.g = (mouseUV.y - 0.5) * 2.0;
        
        // Map blue to a blend of the two axes for maximum color variance
        userHueShift.b = (mouseUV.x - mouseUV.y);
    }
    ...
    for (int i = 0; i < 8; i++) {
        ...
        // AGGRESSIVE Acidic Hue drift per layer driven by history and mouse
        float shift = gen * 0.05;
        
        // Apply baseline audio shift plus the manual mouse drag, scaled by depth
