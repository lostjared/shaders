...
// hue-cache-code-lilac-shockwave-cache
// Angular shockwave fractures launch cache layers away from the cursor.
...
const float RIGHT_TURN = 0.70;
const float RIGHT_SCALE = 0.78;
const float HUE_OFFSET = 2.36;
...
void main() {
    ...
    float wave_radius = fract(time_f * 0.28 + bass * 0.18) * 1.25;
    float shock = exp(-abs(radius - wave_radius) * (28.0 + treble * 18.0));
    float fracture = floor((angle + PI) / TAU * 12.0);
    vec2 scatter = vec2(cos(fracture * 2.17), sin(fracture * 2.17));
    effect_point += scatter * shock * (0.08 + bass * 0.08);
    field = shock * 2.0 - 1.0;
    feature = shock;
