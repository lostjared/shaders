...
// Codex cache concept 080: newton basin radio.
...
vec3 palette(float phase) {
    return 0.5 + 0.5 * cos(TAU * (phase + 0.823 + vec3(0.04, 0.37, 0.69)));
}
...
float concept_field(vec2 q, float age, float bass, float middle, float high) {
    vec3 cells = voronoi_data(q * (7.0 + high * 4.0));
    return exp(-abs(cells.y - cells.x) * (35.0 + high * 25.0));
}
