...
// hue-cache-code-hexed-radial-memory
// Honeycomb memory cells pulse outward with historical bass energy.
...
const float RIGHT_TURN = 0.40;
const float RIGHT_SCALE = 0.50;
const float HUE_OFFSET = 1.52;
...
void main() {
    ...
    float cell_scale = 8.0 + treble * 6.0;
    vec2 cell = hex_cell(point * cell_scale);
    float edge = smoothstep(0.48, 0.39, length(cell));
    float pulse = sin(radius * 30.0 - time_f * 6.0);
    effect_point += cell / cell_scale * pulse * (0.8 + bass);
    field = pulse;
    feature = edge * pow(max(pulse, 0.0), 4.0);
