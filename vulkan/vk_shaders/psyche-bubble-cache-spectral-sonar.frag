...
// psyche-bubble-cache-spectral-sonar
...
vec3 psyche_field(vec2 point, float bass, float middle, float treble, float age) {
    ...
    float sonar = sin(radius * (42.0 + bass * 25.0) - time * 10.0 - age * 18.0);
    float sweep_angle = atan(sin(time * 0.8), cos(time * 0.8));
    float sweep = exp(-abs(sin(angle - sweep_angle - age * 1.7)) * radius * 24.0);
    return vec3(sonar, sweep + pow(max(sonar, 0.0), 10.0) * 0.4,
                radius * 1.8 - time * 0.1 + age);
}
...
vec2 psyche_warp(vec2 uv, vec2 point, vec3 signal, float bass, float middle,
    ...
    uv += radial * signal.x * (0.025 + bass * 0.08) * depth;
    uv += tangent * exp(-abs(sin(angle - time_f * 0.8)) * 12.0) * treble * 0.05;
...
float layer_gate(vec2 point, vec3 signal, float progress, int index) {
    ...
    return 0.2 + 0.8 * exp(-abs(radius - fract(time_f * 0.15 + progress) * 0.8) * 18.0);
}
