...
// psyche-bubble-cache-prism-jellyfish
...
vec3 psyche_field(vec2 point, float bass, float middle, float treble, float age) {
    ...
    vec2 bell = point;
    bell.x *= 1.0 + max(-point.y, 0.0) * (2.2 + bass);
    float dome = sin(length(bell - vec2(0.0, 0.14)) * 29.0 - time * 5.0 + age * 8.0);
    float tentacles = sin(point.x * 24.0 + sin(point.y * 9.0 + time) * 5.0);
    float canopy = exp(-abs(length(bell) - 0.31) * 14.0) * smoothstep(-0.4, 0.15, point.y);
    return vec3(dome + tentacles * 0.35, canopy + pow(max(tentacles, 0.0), 8.0) * 0.3,
                point.y - dome * 0.12 + age);
}
...
vec2 psyche_warp(vec2 uv, vec2 point, vec3 signal, float bass, float middle,
    ...
    uv.x += sin(point.y * 17.0 + age * 9.0) * (0.018 + treble * 0.025) * depth;
    uv.y += signal.x * 0.04 * smoothstep(-0.5, 0.3, point.y);
...
float layer_gate(vec2 point, vec3 signal, float progress, int index) {
    ...
    return 0.2 + 0.8 * smoothstep(-0.5, 0.35, point.y) * (0.35 + signal.y);
}
