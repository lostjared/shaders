...
vec4 getHistoryFrame(int index, vec2 uv) {
    ...
    if (safe_idx == 0) return texture(history, vec3(uv, float(CACHE_HISTORY_LAYER(0))));
    else if (safe_idx == 1) return texture(history, vec3(uv, float(CACHE_HISTORY_LAYER(1))));
    else if (safe_idx == 2) return texture(history, vec3(uv, float(CACHE_HISTORY_LAYER(2))));
    else if (safe_idx == 3) return texture(history, vec3(uv, float(CACHE_HISTORY_LAYER(3))));
    else if (safe_idx == 4) return texture(history, vec3(uv, float(CACHE_HISTORY_LAYER(4))));
    else if (safe_idx == 5) return texture(history, vec3(uv, float(CACHE_HISTORY_LAYER(5))));
    else if (safe_idx == 6) return texture(history, vec3(uv, float(CACHE_HISTORY_LAYER(6))));
    else return texture(history, vec3(uv, float(CACHE_HISTORY_LAYER(7))));
}
