...
// Texture samplers
layout(set = 0, binding = 0) uniform sampler2D samp;   // Base texture
...
void main(void)
{
    ...
    //--------------------------------------------------//
    {
        ...
        vec4 col1 = texture(history, vec3(swirlUV, float(CACHE_HISTORY_LAYER(0))));
    ...
    vec4 colorFractal2;
    {
        ...
        // Sample from samp2
        colorFractal2 = texture(history, vec3(newUV, float(CACHE_HISTORY_LAYER(1))));
    ...
    vec4 colorKaleido3;
    {
        ...
        colorKaleido3 = texture(history, vec3(kaleidoUV, float(CACHE_HISTORY_LAYER(2))));
        // Boost saturation
    ...
    vec4 colorFractal4;
    {
        ...
        // Sample the texture
        colorFractal4 = texture(history, vec3(swirl4UV, float(CACHE_HISTORY_LAYER(3))));
    ...
    vec4 colorSwirl    = texture(history, vec3(swirl(uv, vec2(0.0), 1.0, sin(time_f) * 5.0) * 0.5 + 0.5, float(CACHE_HISTORY_LAYER(0))));
