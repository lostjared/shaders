...
// Procedural Palette biased strictly for Prominent Purple
vec3 acid(float t) {
    vec3 a = vec3(0.6, 0.1, 0.8); 
    vec3 b = vec3(0.5, 0.3, 0.6);
    vec3 c = vec3(1.0, 1.0, 1.0);
    vec3 d = vec3(0.5, 0.2, 0.9); 
...
void main() {
    ...
    // Left Click: Direct Feedback Center Control
    if (iMouse.z > 0.0) {
        // Snaps to mouse position with just enough distortion to keep it moving
    ...
    for (int i = 0; i < 8; i++) {
        ...
        // Locked-in purple drift: aggressively crush green, boost red and blue
        cached.r *= 1.0 + shift * 0.8 + (h_mid * 0.25);
        cached.g *= 1.0 - shift * 1.5 - (h_bass * 0.15);
        cached.b *= 1.0 + shift * 1.2 + (h_air * 0.30);
