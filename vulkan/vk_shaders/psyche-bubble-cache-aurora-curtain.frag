...
// psyche-bubble-cache-aurora-curtain
...
vec3 psyche_field(vec2 point, float bass, float middle, float treble, float age) {
    ...
    float curtain = sin(point.y * 14.0 + fbm(vec2(point.x * 5.0 - time * 0.25,
                                                 point.y * 2.0)) * 8.0);
    float folds = cos(point.x * (10.0 + treble * 5.0) + curtain * 2.5 + age * 9.0);
    return vec3(curtain + folds, pow(max(folds, 0.0), 5.0) * (0.4 + 0.6 * bass),
                point.y + curtain * 0.13 - time * 0.06 + age);
}
...
vec2 psyche_warp(vec2 uv, vec2 point, vec3 signal, float bass, float middle,
    ...
    uv.x += signal.x * (0.025 + bass * 0.04) * depth;
    uv.y += fbm(vec2(point.x * 4.0, age * 5.0)) * 0.035 - 0.0175;
...
float layer_gate(vec2 point, vec3 signal, float progress, int index) {
    ...
    return 0.2 + 0.8 * pow(abs(sin(point.x * 8.0 + progress * 9.0)), 5.0);
}
