...
// psyche-bubble-cache-soap-kaleidoscope
...
vec3 psyche_field(vec2 point, float bass, float middle, float treble, float age) {
    ...
    vec2 crystal = kaleidoscope(rotate_2d(time * 0.07 + age) * point,
                                6.0 + floor(treble * 5.0));
    float film = sin(log(length(crystal) + 0.06) * 17.0 - time * 3.0 + age * 13.0);
    float edge = pow(abs(cos(atan(crystal.y, crystal.x) * 7.0)), 8.0);
    return vec3(film, edge * exp(-radius * 1.5), film * 0.16 + radius + age);
}
...
vec2 psyche_warp(vec2 uv, vec2 point, vec3 signal, float bass, float middle,
    ...
    vec2 folded = kaleidoscope(point, 6.0 + floor(treble * 5.0));
    uv = folded / vec2(max(iResolution.x / max(iResolution.y, 1.0), 0.001), 1.0) + 0.5;
    uv += tangent * signal.x * 0.025 * depth;
...
float layer_gate(vec2 point, vec3 signal, float progress, int index) {
    ...
    return 0.22 + 0.78 * pow(abs(cos(angle * (3.0 + slot) + progress * PI)), 7.0);
}
