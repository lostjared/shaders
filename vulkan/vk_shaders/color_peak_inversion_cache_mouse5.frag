...
void main() {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        // Standard acidic hue drift plus the time-driven left click shift
        float shift = gen * 0.05;
        cached.r *= 1.0 + shift + (h_mid * 0.25) + (userHueShift.r * gen * 0.1);
        cached.g *= 1.0 - shift * 0.8 - (h_bass * 0.15) + (userHueShift.g * gen * 0.1);
        cached.b *= 1.0 + shift * 0.4 + (h_air * 0.30) + (userHueShift.b * gen * 0.1);
