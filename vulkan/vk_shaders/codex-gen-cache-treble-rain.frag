...
// Codex cache concept 053: treble rain.
...
vec3 palette(float phase) {
    return 0.5 + 0.5 * cos(TAU * (phase + 0.124 + vec3(0.04, 0.37, 0.69)));
}
...
vec2 concept_space(vec2 point, float age, vec3 old_audio,
    ...
    q.x += sin(q.y * (8.0 + high * 6.0) + time_f * 0.9 + age * 4.0) * (0.035 + old_bass * 0.06);
    q.y += sin(q.x * (6.0 + middle * 5.0) - time_f * 0.7 - age * 3.0) * (0.025 + old_high * 0.04);
    return q;
}
