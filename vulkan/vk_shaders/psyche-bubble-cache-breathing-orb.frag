...
// psyche-bubble-cache-breathing-orb
...
vec3 psyche_field(vec2 point, float bass, float middle, float treble, float age) {
    ...
    float shell = sin((radius - 0.28 - bass * 0.05) * 42.0 - time * 4.0 - age * 8.0);
    float lens = exp(-abs(radius - 0.34) * (13.0 + middle * 8.0));
    return vec3(shell, lens * (0.55 + 0.45 * shell), radius - time * 0.08 + bass);
}
...
vec2 psyche_warp(vec2 uv, vec2 point, vec3 signal, float bass, float middle,
    ...
    uv += radial * signal.x * (0.035 + bass * 0.08) * depth;
...
float layer_gate(vec2 point, vec3 signal, float progress, int index) {
    ...
    return 0.25 + 0.75 * exp(-abs(radius - mix(0.12, 0.72, progress)) * 16.0);
}
