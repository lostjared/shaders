...
// psyche-bubble-cache-cosmic-reef
...
vec3 psyche_field(vec2 point, float bass, float middle, float treble, float age) {
    ...
    float reef = fbm(point * (5.0 + treble * 2.0) +
                     vec2(time * 0.11, -time * 0.07 + age * 3.0));
    float polyps = sin((reef + radius) * (27.0 + bass * 15.0) - time * 3.0);
    float branches = pow(max(1.0 - abs(polyps), 0.0), 8.0);
    return vec3(polyps, branches * (0.4 + reef), reef + angle / TAU + age);
}
...
vec2 psyche_warp(vec2 uv, vec2 point, vec3 signal, float bass, float middle,
    ...
    vec2 flow = vec2(fbm(point * 4.0 + time_f * 0.09),
                     fbm(point.yx * 4.0 - time_f * 0.07)) - 0.5;
    uv += rotate_2d(signal.x * 0.2) * flow * (0.04 + bass * 0.055) * depth;
...
float layer_gate(vec2 point, vec3 signal, float progress, int index) {
    ...
    return 0.24 + 0.76 * pow(max(signal.y, 0.0), 0.55);
}
