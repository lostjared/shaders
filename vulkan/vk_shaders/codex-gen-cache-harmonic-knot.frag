...
// Codex cache concept 059: harmonic knot.
...
vec3 palette(float phase) {
    return 0.5 + 0.5 * cos(TAU * (phase + 0.946 + vec3(0.04, 0.37, 0.69)));
}
...
vec2 concept_space(vec2 point, float age, vec3 old_audio,
    ...
    float phase = q.x * 6.0 + sin(q.y * 5.0 - time_f) + age * TAU;
    q += vec2(cos(phase), sin(phase * 0.7)) * (0.025 + old_mid * 0.045);
    q = rotate_2d(sin(phase) * 0.18 * age) * q;
    return q;
}
