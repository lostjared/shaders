...
// hue-cache-code-purple-sonar-bloom
// Offset sonar emitters producing crossing violet wavefronts.
...
const float RIGHT_TURN = 0.50;
const float RIGHT_SCALE = 0.48;
const float HUE_OFFSET = 0.68;
...
void main() {
    ...
    vec2 emitter_a = vec2(sin(time_f * 0.31), cos(time_f * 0.27)) * 0.24;
    vec2 emitter_b = -emitter_a.yx;
    float wave_a = sin(length(point - emitter_a) * 34.0 - time_f * 6.0);
    float wave_b = cos(length(point - emitter_b) * 29.0 - time_f * 5.0);
    field = (wave_a + wave_b) * 0.5;
    effect_point += vec2(wave_a, wave_b) * (0.025 + bass * 0.025);
