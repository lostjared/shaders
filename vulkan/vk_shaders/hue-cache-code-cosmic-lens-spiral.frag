...
// hue-cache-code-cosmic-lens-spiral
// Cosmic halftone lens with luminance-sized stars and bitwise color.
...
const float RIGHT_TURN = 1.00;
const float RIGHT_SCALE = 0.88;
const float HUE_OFFSET = 2.48;
const int FEEDBACK_STYLE = 4;
...
void main() {
    ...
    float cells = 54.0 + floor(treble * 36.0);
    vec2 grid = tc * vec2(cells * aspect, cells);
    vec2 cell_id = floor(grid);
    vec2 cell = fract(grid) - 0.5;
    vec2 cell_uv = (cell_id + 0.5) / vec2(cells * aspect, cells);
    float luminance = dot(texture(samp, cell_uv).rgb,
                          vec3(0.299, 0.587, 0.114));
    float dot_mask = smoothstep(mix(0.08, 0.48, luminance),
                                mix(0.04, 0.42, luminance), length(cell));
    effect_point = (cell_uv - center) * vec2(aspect, 1.0);
    field = dot_mask * 2.0 - 1.0;
    feature = dot_mask;
    ...
    vec3 bit_key = acid_palette(hash_21(floor(tc * 64.0)) + time_f * 0.03);
    live_color = mix(live_color * 0.12, xor_color(live_color, bit_key),
                     feature);
