...
// Codex cache concept 090: slingshot echoes.
...
vec3 palette(float phase) {
    return 0.5 + 0.5 * cos(TAU * (phase + 0.193 + vec3(0.04, 0.37, 0.69)));
}
...
vec3 blend_memory(vec3 memory, vec3 live, float field, float age, vec3 old_audio) {
    return max(memory * (0.65 + palette(age * 1.4) * 0.48),
               abs(memory - live.gbr) * (0.35 + field));
}
...
void main() {
    ...
    for (int index = 0; index < SIZE; ++index) {
        ...
        float weight = exp(-abs(age - (0.35 + bass * 0.2)) * 2.4) * 0.42;
        accumulated += treated * weight;
        total_weight += weight;
    }

    vec3 result = accumulated / max(total_weight, 0.001);
    result *= 0.88 + palette(live_field + middle) * (0.12 + live_field * 0.2);
