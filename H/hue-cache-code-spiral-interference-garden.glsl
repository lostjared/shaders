#version 330 core
// hue-cache-code-spiral-interference-garden
// Flowing ribbon garden built from crossed nonlinear wave fields.
...
const float RIGHT_TURN = 1.80;
const float RIGHT_SCALE = 0.30;
const float HUE_OFFSET = 1.40;
...
void main() {
    ...
    float ribbon_x = sin(point.x * (14.0 + bass * 8.0) +
                         sin(point.y * 7.0 - time_f) * 4.0);
    float ribbon_y = cos(point.y * (16.0 + middle * 7.0) +
                         cos(point.x * 8.0 + time_f * 1.2) * 4.0);
    field = (ribbon_x + ribbon_y) * 0.5;
    effect_point += vec2(ribbon_y, ribbon_x) * (0.025 + middle * 0.025);
    feature = pow(1.0 - abs(ribbon_x * ribbon_y), 6.0);
