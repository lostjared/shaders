#version 330 core
// hue-cache-code-ultraviolet-mandala
// A mirrored ultraviolet mandala with audio-variable petal count.
...
const float RIGHT_TURN = 3.20;
const float RIGHT_SCALE = 0.28;
const float HUE_OFFSET = 0.32;
...
void main() {
    ...
    float sides = 8.0 + floor(treble * 8.0);
    effect_point = kaleidoscope(rotate_2d(time_f * 0.18) * point, sides);
    float petal = cos(atan(effect_point.y, effect_point.x) * sides -
                      length(effect_point) * 16.0 + time_f * 2.0);
    effect_point.x *= 1.0 + petal * 0.12;
    field = petal;
    feature = pow(max(petal, 0.0), 7.0) * exp(-radius * 1.2);
