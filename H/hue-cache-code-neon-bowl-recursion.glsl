#version 330 core
// hue-cache-code-neon-bowl-recursion
// Deep neon bowl projection with curved scan bands and recursive zoom.
...
const float RIGHT_TURN = 2.80;
const float RIGHT_SCALE = 1.20;
const float HUE_OFFSET = 1.64;
...
void main() {
    ...
    float bowl = max(0.16, 1.0 - pow(clamp(radius / 1.15, 0.0, 1.0), 2.0));
    effect_point = point * bowl;
    effect_point.y += sin(point.x * 12.0 + time_f * 2.0) * 0.035;
    float bands = sin((radius / bowl) * 28.0 - time_f * 4.0);
    field = bands;
    feature = pow(max(bands, 0.0), 9.0) * (1.0 - bowl);
