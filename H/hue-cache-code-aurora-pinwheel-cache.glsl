#version 330 core
// hue-cache-code-aurora-pinwheel-cache
// Noise-warped aurora curtains twist into a rotating pinwheel cache.
...
const float RIGHT_TURN = 2.20;
const float RIGHT_SCALE = 0.46;
const float HUE_OFFSET = 1.76;
...
void main() {
    ...
    float flow = noise_21(point * 5.0 + vec2(0.0, time_f * 0.25));
    float curtain = sin(point.x * 16.0 + flow * 7.0 + time_f);
    effect_point = rotate_2d(radius * 5.0 + flow * 0.8) * point;
    effect_point.y += curtain * (0.025 + middle * 0.04);
    field = curtain;
    feature = pow(max(curtain, 0.0), 5.0) *
              smoothstep(0.9, 0.1, abs(point.y));
