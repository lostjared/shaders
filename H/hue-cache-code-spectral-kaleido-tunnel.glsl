#version 330 core
// hue-cache-code-spectral-kaleido-tunnel
// Log-polar kaleidoscope tunnel with a mouse-controlled vanishing point.
...
const float RIGHT_TURN = 2.00;
const float RIGHT_SCALE = 0.82;
const float HUE_OFFSET = 0.80;
...
void main() {
    ...
    float tunnel_radius = log(radius + 0.12);
    float tunnel_angle = angle + time_f * (0.7 + bass);
    vec2 tunnel = vec2(tunnel_angle / TAU, tunnel_radius * 0.32);
    tunnel.x = abs(fract(tunnel.x * 10.0) - 0.5);
    effect_point = (tunnel - vec2(0.25, -0.35)) * vec2(1.4, 1.0);
    field = sin(tunnel_radius * 18.0 - time_f * 5.0);
