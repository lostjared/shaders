#version 330 core
// hue-cache-code-feedback-event-horizon
// Event horizon using reciprocal radial compression and orbiting cache flow.
...
const float RIGHT_TURN = 3.50;
const float RIGHT_SCALE = 1.05;
const float HUE_OFFSET = 1.04;
...
void main() {
    ...
    float horizon = 0.24 + bass * 0.10;
    float bend = 1.0 + horizon / (abs(radius - horizon) + 0.08);
    effect_point = rotate_2d(time_f * 0.25 + bend * 0.18) * point / bend;
    field = sin(log(radius + 0.03) * 20.0 - time_f * 5.0);
    feature = exp(-abs(radius - horizon) * 35.0) +
              pow(max(field, 0.0), 8.0) * 0.35;
