#version 330 core
// hue-cache-code-violet-ripple-crown
// Falling diamond cells refract the feed into a violet crown.
...
const float RIGHT_TURN = 0.90;
const float RIGHT_SCALE = 0.40;
const float HUE_OFFSET = 1.16;
...
void main() {
    ...
    float grid_size = 7.0 + floor(middle * 7.0);
    vec2 grid = tc * grid_size;
    vec2 cell_id = floor(grid);
    vec2 cell = fract(grid) - 0.5;
    cell.y = fract(cell.y + time_f * (0.18 + hash_21(cell_id) * 0.35)) - 0.5;
    float diamond = abs(cell.x) + abs(cell.y);
    float shard = smoothstep(0.50, 0.43, diamond);
    vec2 shard_direction = vec2(cos(hash_21(cell_id) * TAU),
                                sin(hash_21(cell_id) * TAU));
    effect_point += shard_direction * shard * (0.04 + bass * 0.035);
    field = shard * 2.0 - 1.0;
    feature = smoothstep(0.48, 0.43, diamond) -
              smoothstep(0.42, 0.37, diamond);
