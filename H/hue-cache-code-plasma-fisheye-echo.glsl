#version 330 core
// hue-cache-code-plasma-fisheye-echo
// Layered plasma-ocean waves roll through a responsive fisheye.
...
const float RIGHT_TURN = 0.60;
const float RIGHT_SCALE = 1.10;
const float HUE_OFFSET = 1.28;
...
void main() {
    ...
    float wave = sin(point.x * 9.0 + time_f * 2.0 + bass * 4.0) * 0.35;
    wave += sin(point.x * 19.0 - time_f * 3.0 + point.y * 6.0) * 0.18;
    wave += (noise_21(point * 9.0 + time_f * 0.35) - 0.5) * 0.32;
    float lens = max(0.20, 1.0 - radius * radius * (0.42 + bass * 0.18));
    effect_point = point * lens + vec2(wave * 0.08, wave * 0.045);
    field = wave;
    feature = smoothstep(0.38, 0.55, wave);
