#version 330 core
// hue-cache-code-violet-frequency-rose
// Lissajous frequency rose woven from bass and treble oscillators.
...
const float RIGHT_TURN = 1.70;
const float RIGHT_SCALE = 0.44;
const float HUE_OFFSET = 2.60;
...
void main() {
    ...
    float rose = cos(angle * (9.0 + floor(treble * 7.0)) +
                     sin(radius * 8.0 - time_f));
    float lissajous = sin(point.x * (13.0 + bass * 7.0) + time_f * 2.0) *
                      cos(point.y * (17.0 + middle * 6.0) - time_f * 1.7);
    effect_point += vec2(rose, lissajous) * 0.035;
    field = rose * lissajous;
    feature = pow(abs(field), 7.0);
