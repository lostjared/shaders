#version 330 core
// hue-cache-code-chromatic-spiral-lens
// Barrel lens and spiral RGB dispersion controlled by both mouse axes.
...
const float RIGHT_TURN = 4.00;
const float RIGHT_SCALE = 0.90;
...
void main() {
    ...
    float lens = max(0.15, 1.0 - radius * radius *
                           (0.48 + bass * 0.25));
    float spiral = radius * 4.0 + time_f * 0.7;
    effect_point = rotate_2d(spiral) * point * lens;
    field = sin(radius * 24.0 - time_f * 5.0);
    feature = (1.0 - lens) * (0.5 + 0.5 * field);
