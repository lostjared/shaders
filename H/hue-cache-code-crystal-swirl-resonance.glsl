#version 330 core
// hue-cache-code-crystal-swirl-resonance
// Iterative quantum folds create a resonant crystalline probability cloud.
...
const float RIGHT_TURN = 1.40;
const float RIGHT_SCALE = 0.38;
const float HUE_OFFSET = 2.00;
...
void main() {
    ...
    vec2 folded = point * (2.2 + bass);
    float energy = 0.0;
    for (int fold = 0; fold < 6; ++fold) {
        folded = abs(folded);
        if (folded.x < folded.y)
            folded = folded.yx;
        folded -= 0.42 + middle * 0.08;
        folded = rotate_2d(time_f * 0.08 + float(fold) * 0.47) * folded;
        energy += exp(-dot(folded, folded) * 2.5);
    }
    effect_point = folded * 0.22;
    field = sin(energy * 8.0 - time_f * 3.0);
