#version 330 core
// hue-cache-code-recursive-iris
// Opening recursive iris blades with mid-band aperture control.
...
const float RIGHT_TURN = 1.50;
const float RIGHT_SCALE = 0.55;
const float HUE_OFFSET = 0.56;
...
void main() {
    ...
    float blades = 7.0 + floor(treble * 5.0);
    float iris = 0.55 + 0.30 * sin(angle * blades + time_f * 1.8);
    float aperture = smoothstep(iris + 0.08, iris - 0.08,
                                radius * (1.0 + middle * 0.4));
    effect_point = rotate_2d(aperture * 0.45 + radius * 0.7) * point;
    effect_point *= mix(1.18, 0.72, aperture);
    field = aperture * 2.0 - 1.0;
    feature = aperture * pow(abs(cos(angle * blades)), 8.0);
