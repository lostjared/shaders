#version 330 core
// hue-cache-code-magenta-tide-mirror
// Independent RGB water waves cross a moving magenta mirror seam.
...
const float RIGHT_TURN = 0.30;
const float RIGHT_SCALE = 0.62;
const float HUE_OFFSET = 1.88;
...
void main() {
    ...
    float tide = sin(point.x * 12.0 + time_f * 2.4) +
                 sin(point.y * 17.0 - time_f * 1.8);
    effect_point = point + vec2(tide, -tide) * (0.018 + bass * 0.018);
    effect_point.x = abs(effect_point.x) - 0.12 * sin(time_f * 0.4);
    field = tide * 0.5;
    feature = pow(max(field, 0.0), 6.0);
    ...
    float water_chroma = 0.012 + treble * 0.035;
    live_color.r = texture(samp, mirror_repeat(mapped +
                           vec2(sin(tc.y * 18.0 + time_f * 3.0), 0.0) *
                           water_chroma)).r;
    live_color.b = texture(samp, mirror_repeat(mapped +
                           vec2(0.0, cos(tc.x * 15.0 - time_f * 2.0)) *
                           water_chroma)).b;
