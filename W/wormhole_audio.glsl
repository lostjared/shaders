...
vec3 preBlendColor(vec2 uv) {
    ...
    // AUDIO: Smooth energy accelerates color cycle
    vec3 neon = neonPalette(t + r * 1.3 + (amp_smooth * 4.0));
...
vec2 wormholeUV(vec2 uv, vec2 c, float aspect, float t) {
    ...
    // AUDIO: Mids increase the swirl distortion
    float swirl = (0.22 + amp_mid * 0.4) / r;
    a += t * 0.35 + swirl;

    // AUDIO: Bass aggressively pushes the depth/zoom
    float z = 0.65 + 0.35 * sin(t * 0.6) + 0.15 * sin(r * 12.0 - t * 3.0) + (amp_low * 1.2);
...
void main(void) {
    ...
    // AUDIO: Bass increases the number of kaleido reflection segments
    float seg = 4.0 + 2.0 * sin(time_f * 0.33) + (amp_low * 6.0);
    vec2 kUV = reflectUV(tc, seg, m, aspect);
    kUV = diamondFold(kUV, m, aspect);

    // AUDIO: Mids push the fractal folding zoom
    float foldZoom = 1.45 + 0.55 * sin(time_f * 0.42) + (amp_mid * 0.6);
    ...
    // AUDIO: Smooth energy accelerates the spiral wrapping rotation
    float tz = time_f * 0.65 + (amp_smooth * 2.5);
    ...
    // AUDIO: Peak energy increases the overall pulse intensity
    float pulse = 0.5 + 0.5 * sin(time_f * 2.0 + rD * 28.0 + k * 12.0) + (amp_peak * 0.8);
    vec3 outCol = kaleidoRGB;
    outCol *= (0.75 + 0.25 * ring) * (0.85 + 0.15 * pulse) * vign;

    // AUDIO: RMS and Peak pump the bloom effect
    vec3 bloom = outCol * outCol * (0.18 + amp_rms * 0.6) + pow(max(outCol - 0.6, 0.0), vec3(2.0)) * (0.12 + amp_peak * 0.4);

    vec2 wh0 = wormholeUV(tc, m, aspect, pingPong((time_f * PI), 5.0));
    vec2 wh1 = wormholeUV(tc + vec2(0.0009, 0.0), m, aspect, time_f + 0.03);
    vec2 wh2 = wormholeUV(tc - vec2(0.0009, 0.0), m, aspect, time_f - 0.03);
    ...
    // AUDIO: Bass pumps the radius of the wormhole mask
    float holeR = mix(max(1.5 / max(iResolution.x, iResolution.y), 0.0008), cornerR, phase) + (amp_low * 0.25);

    // AUDIO: Treble widens the rim/blur of the wormhole transition
    float rimW = mix(0.02, 0.12, smoothstep(0.0, 1.0, phase)) * cornerR + (amp_high * 0.08);
