#version 330 core
// code-frac-x-astral-dream-engine: 4K+ derivative-filtered remix of the Acid Cam frac shader family.
// Domain engine: rational Mobius inversion. Detail engine: antialiased log-polar tunnel.
out vec4 color;
in vec2 tc;
...
const float FOLD_GAIN = 1.340;
const float SPIN_RATE = 0.300;
const float WARP_STRENGTH = 0.770;
const float TEXTURE_MIX = 0.770;
const float EXPOSURE = 1.056;
const float HUE_SHIFT = 0.741;
...
// Log-polar tunnel lattice: concentric recursion, angular spokes, and a
// moving helical interference layer, all filtered through screen derivatives.
vec3 remixDetail(vec2 p, vec2 original, float t, vec4 audio, vec4 orbit) {
    ...
    float depth = log(radius) * (2.1 + WARP_STRENGTH) - t * (0.16 + audio.x * 0.08);
    float rings = aaPulse(depth * 1.7 + 0.15 * sin(angle * TAU * 5.0), 0.105);
    float spokes = aaPulse(angle * (SEGMENTS + floor(audio.y * 5.0)) + depth * 0.13, 0.095);
    float helixPhase = depth * 2.4 - angle * 7.0 + orbit.w * 1.8 + t * 0.06;
    float helix = aaLine(sin(TAU * helixPhase), 0.12 + audio.z * 0.03);
    return vec3(rings, spokes, helix);
}
