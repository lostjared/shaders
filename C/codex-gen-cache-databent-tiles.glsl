#version 330 core
// Codex cache concept 061: databent tiles.
...
vec3 palette(float phase) {
    return 0.5 + 0.5 * cos(TAU * (phase + 0.220 + vec3(0.04, 0.37, 0.69)));
}
...
vec2 concept_space(vec2 point, float age, vec3 old_audio,
    ...
    vec2 block = floor((q + 1.0) * (8.0 + high * 12.0));
    q.x += (hash_21(block + floor(time_f * 4.0) + age * 31.0) - 0.5) * old_high * 0.12;
    q.y += (hash_21(block.yx + age * 17.0) - 0.5) * old_mid * 0.06;
    return q;
}

float concept_field(vec2 q, float age, float bass, float middle, float high) {
    vec2 tile = floor((q + 1.0) * (7.0 + high * 6.0));
    return mix(0.08, 1.0, mod(tile.x + tile.y + floor(age * 8.0), 2.0));
}
