#version 330 core
// Codex cache concept 047: pendulum chronicle.
...
vec3 palette(float phase) {
    return 0.5 + 0.5 * cos(TAU * (phase + 0.302 + vec3(0.04, 0.37, 0.69)));
}
...
vec2 concept_space(vec2 point, float age, vec3 old_audio,
    ...
    vec2 cell = floor((q + 0.8) * 5.0);
    vec2 pivot = (cell + 0.5) / 5.0 - 0.8;
    q = rotate_2d(time_f * 0.08 * (1.0 + bass) + hash_21(cell) * TAU + age * old_mid) *
        (q - pivot) + pivot;
    return q;
}

float concept_field(vec2 q, float age, float bass, float middle, float high) {
    float arc = abs(sin(atan(q.y, q.x) * (4.0 + floor(middle * 5.0)) + length(q) * 12.0 - time_f));
    return pow(1.0 - arc, 5.0);
}
