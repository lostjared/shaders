#version 330 core
// Codex cache concept 007: barcode metropolis.
...
vec3 palette(float phase) {
    return 0.5 + 0.5 * cos(TAU * (phase + 0.822 + vec3(0.04, 0.37, 0.69)));
}
...
float concept_field(vec2 q, float age, float bass, float middle, float high) {
    float lane = abs(fract((q.x + noise_2d(vec2(q.y * 3.0, age))) * (14.0 + high * 12.0)) - 0.5);
    return step(lane, 0.12 + middle * 0.08);
}
