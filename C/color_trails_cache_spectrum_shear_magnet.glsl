...
vec2 shearField(vec2 uv, float bass, float mid, float treble, float air, vec3 oldest, float layer) {
    vec2 dir = vec2(1.0, -0.45);
    dir += vec2(sin(uv.y * 16.0 - time_f * 2.4 - layer), cos(uv.x * 12.0 + time_f * 1.6 + layer)) * 0.5;
    dir += vec2(oldest.g - oldest.r, oldest.b - oldest.g) * 0.8;
    return dir * (0.010 + bass * 0.026 + treble * 0.014) + vec2(uv.y, -uv.x) * (0.004 + air * 0.018);
}
void main() {
    float aspect = iResolution.x / iResolution.y;
    vec2 uv = (tc - 0.5) * vec2(aspect, 1.0);
    float bass = texture(spectrum0, 0.02).r, mid = texture(spectrum0, 0.22).r, treble = texture(spectrum0, 0.50).r, air = texture(spectrum0, 0.80).r;
    float hist = 0.0;
    for (int i = 0; i < 8; i++)
        hist += specHist(i, 0.02);
    hist /= 8.0;
    vec3 oldest = texture(samp8, tc + vec2(sin(time_f * 0.30 + uv.y * 6.0), cos(time_f * 0.18 + uv.x * 5.0)) * (0.012 + hist * 0.030)).rgb;
    vec3 live = texture(samp, tc + shearField(uv, bass, mid, treble, air, oldest, 0.0)).rgb;
    live = mix(live, live.gbr, 0.20 + mid * 0.20) * acid(time_f * 0.07 + uv.x * 0.2 + bass);
    vec3 accum = live;
    float wsum = 1.0;
    for (int i = 0; i < 8; i++) {
        float layer = float(i + 1);
        float hB = specHist(i, 0.02), hM = specHist(i, 0.22), hT = specHist(i, 0.50), hA = specHist(i, 0.80);
        vec3 cached = cacheHist(i, tc + shearField(uv, hB, hM, hT, hA, oldest, layer)).rgb;
        float w = pow(0.79, layer) * (1.0 + hB * 1.3 + hM * 0.6);
        accum += cached * acid(layer * 0.12 + hB * 0.7 + oldest.g * 0.2) * w;
        wsum += w;
    }
    accum /= wsum;
    float streak = smoothstep(0.4, 1.0, abs(cos((uv.x + uv.y) * 22.0 - time_f * 2.5 + hist * 7.0)));
    accum += acid(time_f * 0.04 + uv.y * 0.35) * streak * (0.06 + amp_smooth * 0.20);
    color = vec4(clamp(mix(accum, vec3(1.0) - accum, smoothstep(0.90, 1.0, amp_peak) * 0.12), 0.0, 1.0), 1.0);
}