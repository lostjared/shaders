#version 330 core
// code-frac-x-oceanic-shatter: 4K+ derivative-filtered remix of the Acid Cam frac shader family.
// Domain engine: multi-octave liquid recursion. Detail engine: two-scale Voronoi fracture.
out vec4 color;
in vec2 tc;
...
const float FOLD_GAIN = 1.240;
const float SPIN_RATE = 0.274;
const float WARP_STRENGTH = 0.840;
const float TEXTURE_MIX = 0.710;
const float EXPOSURE = 1.056;
const float HUE_SHIFT = 0.244;
...
// Multi-octave liquid recursion: two vector-fBm flow passes feed eight
// sine/absolute folds before a soft kaleidoscopic reflection.
vec2 remixDomain(vec2 p, float t, vec4 audio) {
    vec2 q = p * (1.6 + audio.x * 0.18);
    q += flowNoise(q * 1.35 + t * vec2(0.07, -0.05)) * WARP_STRENGTH * 0.28;
    q += flowNoise(rot(0.9) * q * 2.1 - t * vec2(0.04, 0.06)) * WARP_STRENGTH * 0.13;
    for (int i = 0; i < 8; ++i) {
        float fi = float(i);
        q = sin(q.yx * (1.12 + fi * 0.025) + vec2(0.7, -0.4));
        q = abs(q) - vec2(0.36, 0.29);
        q = rot(0.24 + SPIN_RATE * 0.6 + 0.03 * sin(t + fi)) * q * 1.34;
    }
    return kaleido(q, max(3.0, SEGMENTS * 0.5 + floor(audio.y * 2.0)));
}
