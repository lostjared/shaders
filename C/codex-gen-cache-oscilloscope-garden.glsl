#version 330 core
// Codex cache concept 056: oscilloscope garden.
...
vec3 palette(float phase) {
    return 0.5 + 0.5 * cos(TAU * (phase + 0.535 + vec3(0.04, 0.37, 0.69)));
}
...
float concept_field(vec2 q, float age, float bass, float middle, float high) {
    float petals = abs(sin(atan(q.y, q.x) * (4.0 + floor(high * 5.0)))) * length(q);
    return exp(-abs(petals - 0.22 - sin(time_f + age * 4.0) * 0.04) * 28.0);
}

vec3 blend_memory(vec3 memory, vec3 live, float field, float age, vec3 old_audio) {
    return memory * mix(vec3(0.65), palette(old_audio.y + age), 0.72) +
           abs(memory - live) * field * 0.35;
}
...
void main() {
    ...
    result = mix(result, 1.0 - result.brg, live_field * amp_peak * 0.18);
