...
// EXTREME historical FFT flow - Intense Left Click Warp & Right Click Zoom
...
void main() {
    ...
    float intenseWarp = 1.0;
    
    if (iMouse.z > 0.0) {
        effectOrigin = (mouseUV - 0.5) * vec2(aspect, 1.0);
        // Massively overdrive the distortion multipliers when left click is held
        intenseWarp = 8.0; 
    }
    ...
    // UV distortion for the live feed, multiplied by our intense left-click warp
    vec2 distort = vec2(
    ...
    // Twist is multiplied by the left click intensity for extreme torque
    float spiralTwist = (15.0 - bass * 8.0) * (intenseWarp > 1.0 ? 3.0 : 1.0);
    float spiralSpeed = time_f * (4.0 + amp_smooth * 8.0) * intenseWarp;
    ...
    // Right-click zoom and rotation control
    float userZoomShift = 0.0;
    float userRotShift = 0.0;
    
    if (iMouse.w > 0.0) {
        // Vertical drag for massive zoom in and out
        userZoomShift = (0.5 - mouseUV.y) * 2.5; 
        // Horizontal drag for extreme rotation tearing
        userRotShift = (mouseUV.x - 0.5) * 2.0; 
    }
    ...
    for (int i = 0; i < 8; i++) {
        ...
        // Apply audio data and the right-click manual rotation override
        float h_rotPerLayer = 0.03 * sin(time_f * 0.3) + (h_treble * 0.15) + userRotShift;
