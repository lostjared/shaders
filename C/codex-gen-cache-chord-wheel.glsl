#version 330 core
// Codex cache concept 057: chord wheel.
...
vec3 palette(float phase) {
    return 0.5 + 0.5 * cos(TAU * (phase + 0.672 + vec3(0.04, 0.37, 0.69)));
}
...
vec2 concept_space(vec2 point, float age, vec3 old_audio,
    ...
    float angle = atan(q.y, q.x) + age * (0.6 + old_mid) + sin(radius * 9.0 - time_f) * 0.09;
    q = vec2(cos(angle), sin(angle)) * radius * (1.0 - age * 0.08 + old_bass * 0.04);
    return q;
}

float concept_field(vec2 q, float age, float bass, float middle, float high) {
    float spokes = abs(sin(atan(q.y, q.x) * (5.0 + floor(high * 5.0)) + time_f * 0.4));
    return pow(1.0 - spokes, 4.0);
}

vec3 blend_memory(vec3 memory, vec3 live, float field, float age, vec3 old_audio) {
    vec3 channels = vec3(memory.r, memory.g * (0.7 + field), memory.b * (1.2 - field * 0.3));
    return mix(channels, channels.gbr, old_audio.z * 0.35);
}
...
void main() {
    ...
    for (int index = 0; index < SIZE; ++index) {
        ...
        float weight = (0.32 + old_audio.y * 0.42) * (1.0 - age * 0.58) * (0.7 + field * 0.25);
        accumulated += treated * weight;
        total_weight += weight;
    }

    vec3 result = accumulated / max(total_weight, 0.001);
    result += abs(result - live) * live_field * (0.18 + bass * 0.2);
