#version 330 core
// Codex cache concept 088: kaleido elevator.
...
vec3 palette(float phase) {
    return 0.5 + 0.5 * cos(TAU * (phase + 0.919 + vec3(0.04, 0.37, 0.69)));
}
...
vec2 concept_space(vec2 point, float age, vec3 old_audio,
    ...
    float sector = TAU / (5.0 + floor(high * 4.0));
    float angle = abs(mod(atan(q.y, q.x) + sector * 0.5, sector) - sector * 0.5);
    q = rotate_2d(age * 0.22 + old_mid * 0.3) * vec2(cos(angle), sin(angle)) * radius;
    return q;
}

float concept_field(vec2 q, float age, float bass, float middle, float high) {
    vec2 grid = abs(fract(q * (9.0 + high * 5.0)) - 0.5);
    return exp(-min(grid.x, grid.y) * (28.0 + high * 22.0));
}
