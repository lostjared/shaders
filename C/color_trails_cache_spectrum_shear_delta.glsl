...
vec2 shearField(vec2 uv, float bass, float mid, float treble, float air, vec3 oldest, float layer) {
    float fan = abs(uv.x) * 1.6 + uv.y * 0.4;
    vec2 dir = vec2(sign(uv.x) * (0.3 + fan), -fan);
    dir += vec2(sin(uv.y * 18.0 + time_f * 1.8 + layer), cos(uv.x * 8.0 - time_f * 1.2 - layer)) * 0.4;
    dir += vec2(oldest.b - oldest.r, oldest.g - oldest.b) * 0.6;
    return dir * (0.011 + air * 0.030 + treble * 0.015);
}
void main() {
    float aspect = iResolution.x / iResolution.y;
    vec2 uv = (tc - 0.5) * vec2(aspect, 1.0);
    float bass = texture(spectrum0, 0.03).r, mid = texture(spectrum0, 0.25).r, treble = texture(spectrum0, 0.64).r, air = texture(spectrum0, 0.93).r;
    float hist = 0.0;
    for (int i = 0; i < 8; i++)
        hist += specHist(i, 0.93);
    hist /= 8.0;
    vec3 oldest = texture(samp8, tc + vec2(cos(time_f * 0.15 + uv.y * 7.0), sin(time_f * 0.17 + uv.x * 4.0)) * (0.012 + hist * 0.034)).rgb;
    vec3 live = texture(samp, tc + shearField(uv, bass, mid, treble, air, oldest, 0.0)).rgb * acid(time_f * 0.05 + uv.y * 0.28 + air);
    vec3 accum = live;
    float wsum = 1.0;
    for (int i = 0; i < 8; i++) {
        float layer = float(i + 1);
        float hB = specHist(i, 0.03), hM = specHist(i, 0.25), hT = specHist(i, 0.64), hA = specHist(i, 0.93);
        vec3 cached = cacheHist(i, tc + shearField(uv, hB, hM, hT, hA, oldest, layer)).rgb;
        float w = pow(0.82, layer) * (1.0 + hA * 1.25);
        accum += cached * acid(layer * 0.08 + hA * 0.8 + time_f * 0.02) * w;
        wsum += w;
    }
    accum /= wsum;
    float fanGlow = smoothstep(0.45, 1.0, abs(sin(abs(uv.x) * 20.0 + uv.y * 12.0 - time_f * 1.8 + hist * 8.0)));
    accum += acid(time_f * 0.03 + uv.x * 0.18) * fanGlow * (0.05 + amp_smooth * 0.18);
