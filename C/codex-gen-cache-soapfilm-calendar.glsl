#version 330 core
// Codex cache concept 037: soapfilm calendar.
...
vec3 palette(float phase) {
    return 0.5 + 0.5 * cos(TAU * (phase + 0.932 + vec3(0.04, 0.37, 0.69)));
}
...
float concept_field(vec2 q, float age, float bass, float middle, float high) {
    float first = abs(sin(q.x * 13.0 + sin(q.y * 9.0 + time_f)));
    float second = abs(sin(q.y * 15.0 + sin(q.x * 7.0 - time_f)));
    return pow(1.0 - min(first, second), 4.0);
}
