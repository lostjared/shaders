...
void main() {
    ...
    vec2 feedbackCenter;
    if (iMouse.z > 0.0) {
        // Left button held: map directly to cursor coordinates
        vec2 mouseUV = iMouse.xy / iResolution.xy;
        feedbackCenter = mouseUV + distort * 0.2; 
    } else {
        // Default floating center when nothing is pressed
        feedbackCenter = vec2(0.5) + distort * 2.5;
    }

    // Right-click hue shift and zoom control
    ...
    if (iMouse.w > 0.0) {
        ...
        // Map horizontal axis to color variance
        userHueShift.r = (mouseUV.x - 0.5) * 2.0;
        userHueShift.g = -(mouseUV.x - 0.5) * 1.5;
        userHueShift.b = (mouseUV.x - 0.5) * 1.0;
        
        // Map vertical axis to zoom scaling
        // Y goes from 0.0 (bottom) to 1.0 (top). This creates a +/- 0.15 zoom offset per layer.
        userZoomShift = (0.5 - mouseUV.y) * 0.15;
    }
    ...
    for (int i = 0; i < 8; i++) {
        ...
        // EXTREME multipliers for historical data, plus right-click manual zoom shift
        float h_zoomPerLayer = 0.95 + 0.02 * sin(time_f * 0.5) - (h_bass * 0.12) + userZoomShift; 
        ...
        // AGGRESSIVE Acidic Hue drift per layer driven by history and mouse X axis
        float shift = gen * 0.05;
        
        cached.r *= 1.0 + shift + (h_mid * 0.25) + (userHueShift.r * gen * 0.12);
        cached.g *= 1.0 - shift * 0.8 - (h_bass * 0.15) + (userHueShift.g * gen * 0.12);
        cached.b *= 1.0 + shift * 0.4 + (h_air * 0.30) + (userHueShift.b * gen * 0.12);
