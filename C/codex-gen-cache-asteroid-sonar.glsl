#version 330 core
// Codex cache concept 023: asteroid sonar.
...
vec3 palette(float phase) {
    return 0.5 + 0.5 * cos(TAU * (phase + 0.014 + vec3(0.04, 0.37, 0.69)));
}
...
vec2 concept_space(vec2 point, float age, vec3 old_audio,
    ...
    vec2 cell = floor((q + 1.0) * 4.0);
    vec2 local = fract((q + 1.0) * 4.0) - 0.5;
    local *= 1.0 + dot(local, local) * (0.8 + old_bass);
    q = (cell + local + 0.5) / 4.0 - 1.0;
    return q;
}

float concept_field(vec2 q, float age, float bass, float middle, float high) {
    vec2 diamond = abs(fract(q * (7.0 + middle * 4.0)) - 0.5);
    return exp(-abs(diamond.x + diamond.y - 0.33) * (36.0 + high * 18.0));
}
