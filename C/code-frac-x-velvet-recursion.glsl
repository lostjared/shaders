#version 330 core
// code-frac-x-velvet-recursion: 4K+ derivative-filtered remix of the Acid Cam frac shader family.
// Domain engine: rational Mobius inversion. Detail engine: three-scale aurora turbulence.
out vec4 color;
in vec2 tc;
...
const float FOLD_GAIN = 1.390;
const float SPIN_RATE = 0.235;
const float WARP_STRENGTH = 0.630;
const float TEXTURE_MIX = 0.800;
const float EXPOSURE = 1.128;
const float HUE_SHIFT = 0.670;
...
// Six-octave aurora turbulence layered at three scales, with analytic
// ribbon widths so its fine filaments survive UHD and 8K rendering.
vec3 remixDetail(vec2 p, vec2 original, float t, vec4 audio, vec4 orbit) {
    vec2 drift = vec2(t * 0.043, -t * 0.031);
    float broad = fbm6(p * 2.1 + drift);
    float medium = fbm6(rot(0.82) * p * 4.3 - drift * 1.7 + broad);
    float fine = fbm6(rot(-0.47) * p * 8.7 + vec2(medium, broad));
    float ribbonPhase = p.y * (3.8 + audio.y) + broad * 2.6 + medium * 1.3 - t * 0.17;
    float ribbon = aaLine(sin(ribbonPhase), 0.13 + audio.z * 0.04);
    float veil = smoothstep(0.30, 0.82, broad * 0.48 + medium * 0.34 + fine * 0.18);
    float sparks = aaPulse(fine * 3.2 + orbit.w + t * 0.025, 0.10);
    return vec3(ribbon, veil, sparks * smoothstep(0.45, 0.9, medium));
}
