#version 330 core
// Codex cache concept 098: neon tube map.
...
vec3 palette(float phase) {
    return 0.5 + 0.5 * cos(TAU * (phase + 0.289 + vec3(0.04, 0.37, 0.69)));
}
...
vec2 concept_space(vec2 point, float age, vec3 old_audio,
    ...
    q = abs(q) - vec2(0.22 + bass * 0.04);
    q = abs(rotate_2d(0.785 + age * 0.13) * q) - vec2(0.11);
    q = rotate_2d(old_mid * 0.4 - age * 0.25) * q;
    return q;
}

float concept_field(vec2 q, float age, float bass, float middle, float high) {
    vec2 cell = fract(q * (8.0 + middle * 4.0)) - 0.5;
    float trace = min(abs(cell.x), abs(cell.y));
    float node = length(cell) - 0.16;
    return max(exp(-trace * 45.0), exp(-abs(node) * 38.0));
}
