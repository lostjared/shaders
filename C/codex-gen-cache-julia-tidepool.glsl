#version 330 core
// Codex cache concept 071: julia tidepool.
...
vec3 palette(float phase) {
    return 0.5 + 0.5 * cos(TAU * (phase + 0.590 + vec3(0.04, 0.37, 0.69)));
}
...
vec2 concept_space(vec2 point, float age, vec3 old_audio,
    ...
    vec2 well_a = q - vec2(0.24, 0.0);
    vec2 well_b = q + vec2(0.24, 0.0);
    q += well_a / (dot(well_a, well_a) + 0.045) * (old_bass - 0.25) * 0.012;
    q -= well_b / (dot(well_b, well_b) + 0.045) * (old_high - 0.25) * 0.012;
    return q;
}

float concept_field(vec2 q, float age, float bass, float middle, float high) {
    float cloud = noise_2d(q * (5.0 + high * 3.0) + vec2(time_f * 0.08, age * 4.0));
    return smoothstep(0.42 - bass * 0.12, 0.72, cloud);
}

vec3 blend_memory(vec3 memory, vec3 live, float field, float age, vec3 old_audio) {
    return mix(memory, memory * palette(field + age + old_audio.y), 0.62) * (0.72 + field * 0.5);
}
...
void main() {
    ...
    for (int index = 0; index < SIZE; ++index) {
        ...
        float weight = exp(-age * 2.2) * (0.62 + field * 0.55 + old_audio.x * 0.25);
        accumulated += treated * weight;
        total_weight += weight;
    }

    vec3 result = accumulated / max(total_weight, 0.001);
    result += palette(live_field + time_f * 0.03) * live_field * (0.08 + amp_peak * 0.2);
