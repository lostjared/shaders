#version 330 core
// Codex cache concept 013: stormcell radar.
...
vec3 palette(float phase) {
    return 0.5 + 0.5 * cos(TAU * (phase + 0.644 + vec3(0.04, 0.37, 0.69)));
}
...
vec3 blend_memory(vec3 memory, vec3 live, float field, float age, vec3 old_audio) {
    vec3 split = vec3(memory.r, live.g, memory.b);
    return mix(memory, split * palette(field + age), 0.45 + field * 0.3);
}
...
void main() {
    ...
    for (int index = 0; index < SIZE; ++index) {
        ...
        float weight = pow(1.0 - age * 0.8, 1.5) * (0.35 + field * 0.75);
        accumulated += treated * weight;
        total_weight += weight;
    }

    vec3 result = accumulated / max(total_weight, 0.001);
    result += palette(length(live_q) + time_f * 0.04) * pow(live_field, 2.0) * 0.22;
