#version 330 core
// code-frac-x-fractured-singularity: 4K+ derivative-filtered remix of the Acid Cam frac shader family.
// Domain engine: log-polar singularity recursion. Detail engine: two-scale Voronoi fracture.
out vec4 color;
in vec2 tc;
...
const float FOLD_GAIN = 1.290;
const float SPIN_RATE = 0.196;
const float WARP_STRENGTH = 0.840;
const float TEXTURE_MIX = 0.740;
const float EXPOSURE = 1.020;
const float HUE_SHIFT = 0.534;
...
// Log-polar singularity: reciprocal tunnel projection followed by nine
// mirrored scale recursions, remixed from the zoom and wormhole frac shaders.
vec2 remixDomain(vec2 p, float t, vec4 audio) {
    float radius = max(length(p), 0.0015);
    float angle = atan(p.y, p.x) + WARP_STRENGTH / (0.18 + radius);
    p = vec2(log(radius) * 0.72 - t * (0.08 + audio.x * 0.04),
             angle / TAU * SEGMENTS);
    for (int i = 0; i < 9; ++i) {
        float fi = float(i);
        p = abs(fract(p * (1.36 + 0.018 * fi) + 0.5) - 0.5) * 2.0 - 0.48;
        p = rot(0.18 + SPIN_RATE * 0.5 + fi * 0.021) * p;
        p += 0.045 * vec2(sin(p.y * 5.0 + t + fi), cos(p.x * 4.0 - t));
    }
    return p * (1.25 + 0.15 * audio.w);
}
