#version 330 core
// Codex cache concept 100: charcoal phantom.
...
vec3 palette(float phase) {
    return 0.5 + 0.5 * cos(TAU * (phase + 0.563 + vec3(0.04, 0.37, 0.69)));
}
...
float concept_field(vec2 q, float age, float bass, float middle, float high) {
    float dune = abs(sin(q.x * (13.0 + high * 6.0) + q.y * q.y * 18.0 - time_f + age * 3.0));
    return pow(1.0 - dune, 3.0);
}
