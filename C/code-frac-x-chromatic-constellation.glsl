#version 330 core
// code-frac-x-chromatic-constellation: 4K+ derivative-filtered remix of the Acid Cam frac shader family.
// Domain engine: rational Mobius inversion. Detail engine: two-scale Voronoi fracture.
out vec4 color;
in vec2 tc;
...
const float FOLD_GAIN = 1.215;
const float SPIN_RATE = 0.170;
const float WARP_STRENGTH = 0.840;
const float TEXTURE_MIX = 0.695;
const float EXPOSURE = 1.074;
const float HUE_SHIFT = 0.599;
...
// Rational Mobius recursion: complex division, sphere inversion, orbiting
// poles, and nine bounded folds create a zero-gravity fractal manifold.
vec2 remixDomain(vec2 p, float t, vec4 audio) {
    vec2 pole = 0.32 * vec2(cos(t * 0.17), sin(t * 0.13));
    vec2 numerator = complexMul(p + vec2(0.18, -0.11), p - vec2(0.21, 0.16));
    p = complexDiv(numerator, p - pole);
    p *= 0.42 + 0.05 * audio.x;
    for (int i = 0; i < 9; ++i) {
        float fi = float(i);
        p = rot(SPIN_RATE + fi * 0.037 + t * 0.009) * p;
        p = abs(p) - vec2(0.41, 0.27 + 0.02 * sin(fi + t * 0.2));
        float radius2 = clamp(dot(p, p), 0.14, 1.32);
        p *= (1.06 + FOLD_GAIN * 0.23) / radius2;
        p -= vec2(0.47, 0.31);
    }
    return kaleido(p, SEGMENTS + floor(audio.y * 5.0));
}

vec3 voronoiMetrics(vec2 p) {
    vec2 cell = floor(p);
    vec2 local = fract(p);
    float nearest = 10.0;
    float second = 10.0;
    ...
    for (int y = -1; y <= 1; ++y) {
        for (int x = -1; x <= 1; ++x) {
            ...
            vec2 randomPoint = hash22(cell + neighbor);
            vec2 delta = neighbor + 0.5 + 0.43 * sin(TAU * randomPoint + time_f * 0.12) - local;
            float distance2 = dot(delta, delta);
            if (distance2 < nearest) {
                second = nearest;
                nearest = distance2;
                cellId = randomPoint.x;
            } else if (distance2 < second) {
                second = distance2;
            }
        }
    }
    return vec3(sqrt(nearest), sqrt(second), cellId);
}

// Two-scale Voronoi fracture with derivative-antialiased cell borders.
vec3 remixDetail(vec2 p, vec2 original, float t, vec4 audio, vec4 orbit) {
    vec2 warp = flowNoise(p * 1.7 + t * 0.035) * (0.11 + WARP_STRENGTH * 0.08);
    vec3 coarse = voronoiMetrics((p + warp) * (5.2 + audio.y * 1.4));
    vec3 fine = voronoiMetrics(rot(0.71) * (p - warp) * 11.0);
    float edgeDistance = coarse.y - coarse.x;
    float edge = aaLine(edgeDistance, 0.045 + audio.z * 0.018);
    float microEdge = aaLine(fine.y - fine.x, 0.028);
    float shard = aaPulse(coarse.z + fine.z * 0.37 + orbit.w - t * 0.018, 0.16);
    return vec3(edge, microEdge, shard);
}
