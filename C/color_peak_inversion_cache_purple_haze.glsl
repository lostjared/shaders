...
// Left Click: Intensity * Time Hue Shift
// Right Click: Zoom Control
// Palette: Deep Purple Energy
...
// Procedural Palette biased for Purple/Indigo
vec3 acid(float t) {
    vec3 a = vec3(0.5, 0.4, 0.6); // Base tint favors blue/purple
    vec3 b = vec3(0.5, 0.4, 0.6);
    vec3 c = vec3(1.0, 1.0, 0.5);
    vec3 d = vec3(0.4, 0.2, 0.8); // Shifted phase to bring out violet crests
...
void main() {
    ...
    if (iMouse.z > 0.0) {
        ...
        // Calculate intensity based on distance from the center of the screen
        float intensity = length(mouseUV - vec2(0.5)) * 2.0;
        
        // Multiply by time_f for a continuously cycling color drive
        float timeDrivenHue = intensity * time_f * 2.0;
        
        // Map this to a cascading RGB shift
        userHueShift.r = sin(timeDrivenHue) * 1.5;
        userHueShift.g = cos(timeDrivenHue * 0.8) * 1.5;
        userHueShift.b = sin(timeDrivenHue * 1.2) * 1.5;
    }
    ...
    for (int i = 0; i < 8; i++) {
        ...
        // Modified drift to favor heavy blues and moderate reds for a deep purple tunnel
        cached.r *= 1.0 + shift * 0.5 + (h_mid * 0.25) + (userHueShift.r * gen * 0.1);
        cached.g *= 1.0 - shift * 0.9 - (h_bass * 0.15) + (userHueShift.g * gen * 0.1);
        cached.b *= 1.0 + shift * 1.3 + (h_air * 0.30) + (userHueShift.b * gen * 0.1);
