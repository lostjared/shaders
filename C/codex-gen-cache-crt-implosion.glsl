#version 330 core
// Codex cache concept 067: crt implosion.
...
vec3 palette(float phase) {
    return 0.5 + 0.5 * cos(TAU * (phase + 0.042 + vec3(0.04, 0.37, 0.69)));
}
...
vec2 concept_space(vec2 point, float age, vec3 old_audio,
    ...
    q *= 1.0 + (0.28 + bass * 0.32) * radius2;
    q = rotate_2d((middle - old_mid) * age * 0.8) * q;
    return q;
}
