...
// EXTREME historical FFT flow - Dynamic Distortion Center & Global Hue Control
...
// Procedural Palette with added Hue Offset
vec3 acid(float t, float hueOffset) {
    vec3 a = vec3(0.5, 0.5, 0.5);
    vec3 b = vec3(0.5, 0.5, 0.5);
    vec3 c = vec3(1.0, 1.0, 0.5);
    // Adding the hueOffset to the phase vector 'd' rotates the entire color spectrum
...
void main() {
    ...
    // Right-click global hue shift calculation
    float globalHueShift = 0.0;
    if (iMouse.w > 0.0) {
        globalHueShift = (mouseUV.x - 0.5) * 3.0; // Horizontal drag shifts the acid palette
    }

    // Determine the mathematical origin of the distortion based on Left Click
    ...
    // The interference waves now orbit the dynamic effect origin
    ...
    // Add acid interference and crests, now utilizing the global hue offset
    float interference = combined * 0.5 + 0.5;
    current_col *= acid(interference + time_f * 0.1 + bass, globalHueShift) * 1.5;
    
    float crest = pow(max(combined, 0.0), 6.0);
    current_col += acid(r1 + time_f * 0.2, globalHueShift) * crest * (1.0 + air * 2.0);
    ...
    vec3 spiralColor = acid(r_center * 0.8 - time_f * 0.5 + mid * 0.5, globalHueShift);
    ...
    // Direct mouse control logic for feedback center
    vec2 feedbackCenter;
    if (iMouse.z > 0.0) {
        feedbackCenter = mouseUV + distort * 0.2; 
    } else {
        feedbackCenter = vec2(0.5) + distort * 2.5;
    }

    // Right-click secondary hue layer and zoom control
    vec3 userHueShift = vec3(0.0);
    float userZoomShift = 0.0;
    
    if (iMouse.w > 0.0) {
        userHueShift.r = (mouseUV.x - 0.5) * 8.0;
        userHueShift.g = -(mouseUV.x - 0.5) * 6.0;
        userHueShift.b = (mouseUV.x - 0.5) * 5.0;
    ...
    for (int i = 0; i < 8; i++) {
        ...
        cached.b *= 1.0 + shift * 0.4 + (h_air * 0.30) + (userHueShift.b * gen * 0.3);
