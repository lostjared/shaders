...
vec2 fractalFold(vec2 uv, float zoom, float t, vec2 c, float aspect) {
    vec2 p = uv;
    // Iterations = 3. This keeps the image chunks large and visible
    // rather than turning into noise.
    for (int i = 0; i < 3; i++) {
...
vec3 preBlendColor(vec2 uv) {
    // 1. Sample your texture at the distorted UVs
    vec3 tex = tentBlur3(samp, uv, iResolution);

    // 2. Generate Rainbow Spiral
    ...
    // 3. Mix them.
    // The texture IS the fractal now, colored by the rainbow.
...
void main(void) {
    // Note: We are NO LONGER sampling the flat background here.
    ...
    // --- Get the Texture (Distorted into Fractal) ---
    ...
    outCol *= (0.8 + 0.2 * pulse) * vign;
    outCol = clamp(outCol, 0.0, 1.0);

    // Final Output: The entire screen is now the fractal texture.
