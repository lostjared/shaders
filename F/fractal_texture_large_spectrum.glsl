...
void main(void) {
    ...
    float foldZoom = 1.15 + (0.15 + amp_low * 0.4) * sin(time_f * 0.42);
    ...
    // --- Spectrum Sampling ---
    // Sample the FFT 1D texture radially outward based on the diamond radius
    float specCoord = clamp(rD * 1.5, 0.0, 0.99);
    float specVal = texture(spectrum, specCoord).r;
    
    // Twist the fold angle when frequencies hit
    float ang = atan(q.y, q.x) + tz * 0.35 + 0.35 * sin(rD * 18.0 + time_f * 0.6);
    ang += specVal * (0.2 + amp_mid * 0.3); 
    ...
    // --- Spectrum Color Tinting ---
    // Inject a harsh color glitch based on the active spectrum frequency
    vec3 specTint = mix(vec3(1.0), vec3(1.0 + amp_low * 0.5, 0.5 + amp_mid, 1.2 + amp_high), specVal * 0.8);
    finalRGB *= specTint;
