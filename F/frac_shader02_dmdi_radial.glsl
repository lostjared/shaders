...
void main(void) {
    ...
    vec2 tcWave = tc + waveOffset;

    vec4 baseTex = texture(samp, tcWave);
    ...
    float seg = 4.0 + (2.0 + amp_mid * 2.0) * sin(time_f * 0.33 + uNorm * 0.5);
    vec2 kUV = reflectUV(tcWave, seg, m, aspect);
    kUV = diamondFold(kUV, m, aspect);
    float foldZoom = 1.35 + (0.65 + amp_low * 0.4) * sin(time_f * 0.42 + uNorm * 0.8);
    ...
    float pulse = 0.5 + (0.5 + amp_rms * 0.3) * sin(time_f * 2.0 + rD * 28.0 + k * 12.0 + aNorm * 1.5);
