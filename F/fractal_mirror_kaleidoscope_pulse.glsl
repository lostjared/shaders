...
void main(void) {
    ...
    float pulse = 1.0 + 0.4 * aLow * sin(t * 4.0);
    uv = (uv - 0.5) * pulse + 0.5;

    float seg = 6.0 + 4.0 * aMid;
    vec2 ctr = vec2(0.5);
    vec2 kUV = reflectUV(uv, seg, ctr, aspect);
    float zoom = 1.4 + 0.5 * sin(t * 0.35) + 0.3 * aLow;
    int iters = 6 + int(aPk * 4.0);
    kUV = fractalFold(kUV, zoom, t, ctr, aspect, iters);
    kUV = rotateUV(kUV, t * 0.2 + aHigh * 0.5, ctr, aspect);
    kUV = mirror(kUV);

    vec4 tex = texture(samp, kUV);
