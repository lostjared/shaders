...
void main(void){
    ...
    vec2 baseUV = spinWarp(tc, t);
    vec2 tcMix = mix(tc, baseUV, 0.75);
    ...
    vec3 base = textureGrad(samp, baseUV, dFdx(baseUV), dFdy(baseUV)).rgb;
    ...
    combined *= 1.0 + _ab * 0.6;
    combined = mix(combined, combined * vec3(1.0 + _abass * 0.3, 1.0 - _abass * 0.15, 1.0 + clamp(amp_high, 0.0, 1.0) * 0.25), _ab);
