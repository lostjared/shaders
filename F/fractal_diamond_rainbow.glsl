...
vec2 fractalFold(vec2 uv, float zoom, float t, vec2 c, float aspect) {
    vec2 p = uv;
    // REDUCED ITERATIONS: 6 -> 3
    // This ensures the "small fractals of the image" remain visible
    // rather than turning into pixel noise.
    for (int i = 0; i < 3; i++) {
...
// --- The Core Logic ---
// This function replaces the old neon/softTone logic.
// It applies the rainbow spiral to the texture *inside* the fractal shards.
