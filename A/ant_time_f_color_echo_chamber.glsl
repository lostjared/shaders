#version 330 core
// ant_time_f_color_echo_chamber
// Each cache frame placed at offset position - obvious 8-fold ghost echo trail
...
void main() {
    ...
    vec3 acc = cacheHist(0, tc).rgb;
    for (int i = 1; i < 9; i++) {
        float t = float(i) / 8.0;
        float ang = time_f * 0.3 + float(i) * 0.7;
        vec2 off = 0.04 * float(i) * vec2(cos(ang), sin(ang));
        float h = specHist(min(i, 7), 0.1 + t * 0.5);
        off *= 1.0 + h * 3.0;
        vec3 c = cacheHist(i, tc + off).rgb;
        vec3 tint = acid(float(i) * 0.13 + time_f * 0.05);
        acc += c * tint * (0.6 - t * 0.4) * (1.0 + h * 2.0);
    }
    acc /= 3.5;
