#version 330 core
// ant_texture_cache_spectrum_scale_starfield
...
vec2 styleDrift(vec2 uv, float layer, float age, float hB, float hM, float hT, float hA, vec3 oldest) {
    return uv * (age * 0.05 + hB * 0.04);

}

vec3 styleColor(vec3 cached, float layer, float age, float hB, float hT) {
    float spark = step(0.97, hash12(vec2(layer * 17.31, age * 23.71) + hT * 5.0));
    return cached + vec3(1.0) * spark * 0.25 * (0.5 + hT);
