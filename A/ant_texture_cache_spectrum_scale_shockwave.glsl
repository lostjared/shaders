#version 330 core
// ant_texture_cache_spectrum_scale_shockwave
...
vec2 styleDrift(vec2 uv, float layer, float age, float hB, float hM, float hT, float hA, vec3 oldest) {
    float r = length(uv);
    float pulse = sin(r * 12.0 - time_f * 4.0 - age * 6.0);
    vec2 n = uv / (r + 1e-3);
    return n * pulse * (0.006 + age * 0.020 + hB * 0.018);

}

vec3 styleColor(vec3 cached, float layer, float age, float hB, float hT) {
    return cached * (0.95 + 0.10 * cos(layer * 0.4 + hT * 5.0));
...
void main() {
    ...
    float r = length(uv);
    accum *= 0.95 + 0.10 * smoothstep(0.0, 1.0, 1.0 - r) * (0.5 + amp_peak);
