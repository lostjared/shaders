#version 330 core
// ant_peak_inversion_cache_spectrum_time_zebra_riot
// Zebra stripes warped through 9 history frames at 5x intensity
...
void main() {
    ...
    for (int i = 0; i < 9; i++) {
        ...
        vec2 p = uv;
        p.x += sin(p.y * (6.0 + h * 60.0) + iTime + float(i)) * (0.05 + h * 0.5);
        p.y += cos(p.x * (5.0 + h2 * 50.0) - iTime + float(i)) * (0.05 + h2 * 0.5);
        float stripe = step(0.5, fract(p.x * (4.0 + float(i)) + p.y * 2.0 + iTime));
        vec2 suv = p / vec2(aspect, 1.0) + 0.5;
        vec3 c = cacheHist(i, suv).rgb;
        vec3 tint = acid(float(i) * 0.15 + h * 0.4);
        vec3 layer = mix(c * 0.3, tint, stripe);
        acc += layer * pow(0.84, float(i)) * (0.7 + h * 5.0);
    }
    acc /= 3.0;
