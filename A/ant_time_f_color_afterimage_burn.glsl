#version 330 core
// ant_time_f_color_afterimage_burn
// Persistent afterimage: max-blend of all 9 cache frames - obvious 'last 8 frames burned in'
...
void main() {
    ...
    vec3 acc = vec3(0.0);
    for (int i = 0; i < 9; i++) {
        vec2 wob = 0.01 * float(i) * vec2(cos(time_f * 0.4 + float(i)), sin(time_f * 0.5 + float(i)));
        vec3 c = cacheHist(i, tc + wob).rgb;
        float h = specHist(min(i, 7), 0.2) * 5.0;
        vec3 tint = palette(float(i) * 0.13, vec3(0.0, 0.33, 0.66));
        c *= tint * (0.7 + h * 0.6);
        acc = max(acc, c * (1.0 - float(i) * 0.06));
    }
    // gentle decay
    acc = acc * 0.85 + cacheHist(0, tc).rgb * 0.3;
    acc *= 1.2 + amp_smooth * 0.8;
