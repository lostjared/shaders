#version 330 core
// ant_time_f_color_fft_aurora
// Aurora curtain - vertical bands where each band is one spectrum history slot painted floor-to-ceiling
...
void main() {
    ...
    for (int i = 0; i < 8; i++) {
        float bandX = (float(i) + 0.5) / 8.0;
        float dist = abs(tc.x - bandX);
        float h = specHist(i, tc.y) * 6.0;
        float curtain = exp(-dist * 30.0) * h;
        // sway
        float sway = sin(tc.y * 6.0 + time_f * 0.5 + float(i)) * 0.05;
        float curtain2 = exp(-abs(tc.x - bandX - sway) * 20.0) * h;
        vec3 tint = palette(float(i) * 0.13 + time_f * 0.1,
                            vec3(0.0, 0.33, 0.66));
        acc += tint * (curtain + curtain2) * 1.5;
    }
    acc *= 1.0 + amp_peak * 2.0;
