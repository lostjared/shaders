...
/* Spiral twist driven by luminance + audio */
...
/* Radial ripple (from your second shader), now centered at the movable sphere and content/audio aware */
vec2 rippleDisplace(vec2 uv, vec2 center, float aspect, float t, float drive) {
    ...
    float window = smoothstep(0.95, 0.15, dist); // stronger near center
    ripple *= window;
...
void main(void) {
    ...
    vec2 kUV = reflectUV(uvRipple, seg, center, aspect);

    vec3 echoCol = spiralEcho(kUV, center, aspect, t, drive);

    vec3 local = tentBlur3RGB(samp, uvRipple, iResolution);
