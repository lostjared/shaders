#version 330 core
// ant_time_f_color_timeline_swirl
// Time spiraling inward: angle codes age, radius codes FFT bin - both spectrum and cache rotate together
...
void main() {
    ...
    float spiralPhase = a + r * 6.0 - time_f * 0.5;
    float idxF = mod(spiralPhase / TAU * 8.0, 8.0);
    int idx = int(idxF);
    float h = specHist(idx, clamp(r * 1.5, 0.0, 1.0)) * 5.0;
    float rotA = a + time_f * 0.2 + float(idx) * 0.4;
    vec2 ruv = vec2(cos(rotA), sin(rotA)) * r;
    vec2 suv = ruv / vec2(aspect, 1.0) + 0.5;
    vec3 c = cacheHist(idx + 1, suv).rgb;
    vec3 tint = palette(float(idx) * 0.13 + r + time_f * 0.05,
                        vec3(0.0, 0.33, 0.66));
    c *= tint * (1.0 + h);
    // spiral arm separators
    float bord = smoothstep(0.0, 0.05, abs(fract(idxF) - 0.5) - 0.45);
    c *= 0.5 + 0.5 * bord;
    c *= 1.2 + amp_smooth * 1.2;
