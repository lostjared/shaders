#version 330 core
// ant_time_f_color_parallax_layers
// Parallax: each cache frame on its own depth plane scrolling at different speed
...
void main() {
    ...
    vec3 acc = vec3(0.0);
    for (int i = 0; i < 9; i++) {
        float depth = 1.0 - float(i) * 0.10;
        vec2 par = vec2(time_f * 0.05 * (1.0 + float(i) * 0.3),
                        sin(time_f * 0.3 + float(i)) * 0.05);
        vec2 suv = (tc - 0.5) / depth + 0.5 + par;
        float h = specHist(min(i, 7), 0.1) * 5.0;
        vec3 c = cacheHist(i, fract(suv)).rgb;
        vec3 tint = palette(float(i) * 0.13, vec3(0.0, 0.33, 0.66));
        acc = mix(acc, c * tint * (1.0 + h), 0.55 - float(i) * 0.04);
    }
    acc *= 1.2 + amp_smooth;
