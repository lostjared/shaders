#version 330 core
// ant_cache_spectrum8_strobe_tunnel
// Tunnel where every history slot strobes hard on its band peaks
...
void main() {
    ...
    vec3 acc = vec3(0.0);
    for (int i = 0; i < 8; i++) {
        float h = specHist(i, 0.05 + float(i) * 0.05);
        float h2 = specHist(i, 0.6);
        float strobe = step(0.5 - h * 0.3, h);
        float zoom = 1.0 - 0.08 * float(i) - h * 0.4;
        float rot = float(i) * 0.4 + h2 * 5.0 + iTime * 0.4;
        float cs = cos(rot), sn = sin(rot);
        vec2 p = uv / max(zoom, 0.05);
        p = vec2(p.x * cs - p.y * sn, p.x * sn + p.y * cs);
        vec2 suv = p / vec2(aspect, 1.0) + 0.5;
        vec3 c = cacheHist(i, suv).rgb;
        vec3 tint = palette(float(i) * 0.13 + r * 3.0, vec3(0.0, 0.33, 0.66));
        if (strobe > 0.5) c = 1.0 - c;
        acc += (c + tint * h * 1.5) * pow(0.83, float(i)) * (1.0 + h * 5.0);
    }
    acc /= 3.0;
    color = vec4(clamp(acc * (1.0 + amp_peak * 2.0), 0.0, 1.0), 1.0);
}
