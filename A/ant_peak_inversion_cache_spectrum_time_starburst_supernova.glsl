#version 330 core
// ant_peak_inversion_cache_spectrum_time_starburst_supernova
// Radial starburst where ray count and brightness scream from spectrum history
...
void main() {
    ...
    for (int i = 0; i < 8; i++) {
        float h = specHist(i, 0.05) * 5.0;
        float h2 = specHist(i, 0.6) * 5.0;
        float rays = 8.0 + float(i) * 4.0 + floor(h * 80.0);
        float ray = pow(max(cos(a * rays + iTime * (0.5 + h2 * 4.0) + float(i)), 0.0), 6.0);
        float falloff = exp(-r * (1.5 - h * 0.8));
        vec3 tint = acid(float(i) * 0.12 + h * 0.3);
        vec2 suv = uv * (0.95 - 0.05 * float(i)) / vec2(aspect, 1.0) + 0.5;
        vec3 c = cacheHist(i + 1, suv).rgb;
        acc += (tint * ray * falloff * 3.0 + c * 0.5) * (1.0 + h * 6.0) * pow(0.83, float(i));
    }
