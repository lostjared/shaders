#version 330 core
// ant_cache_spectrum8_oilslick
// Iridescent oil-slick with chromatic shift driven hard by spectrum history
...
void main() {
    ...
    vec3 acc = vec3(0.0);
    for (int i = 0; i < 8; i++) {
        float h = specHist(i, 0.1 + float(i) * 0.05);
        float h2 = specHist(i, 0.7);
        vec2 d = vec2(sin(uv.y * (6.0 + h * 20.0) + iTime + float(i)),
                      cos(uv.x * (8.0 + h2 * 18.0) - iTime + float(i))) * (0.04 + h * 0.2);
        vec2 suv = tc + d;
        float chroma = 0.02 + h * 0.15;
        vec3 c;
        c.r = cacheHist(i, suv + vec2(chroma, 0.0)).r;
        c.g = cacheHist(i, suv).g;
        c.b = cacheHist(i, suv - vec2(chroma, 0.0)).b;
        c *= palette(h * 5.0 + float(i) * 0.13 + iTime * 0.1, vec3(0.0, 0.33, 0.66)) * (1.0 + h * 6.0);
        acc += c * pow(0.82, float(i));
    }
    acc /= 3.0;
    acc *= 1.3 + amp_peak * 1.5;
