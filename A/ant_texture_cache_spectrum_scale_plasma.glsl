#version 330 core
// ant_texture_cache_spectrum_scale_plasma
...
vec2 styleDrift(vec2 uv, float layer, float age, float hB, float hM, float hT, float hA, vec3 oldest) {
    float a = sin(uv.x * 3.0 + uv.y * 2.0 + time_f * 0.8 + layer * 0.3);
    float b = cos(uv.y * 2.7 - uv.x * 1.6 - time_f * 0.5 + layer * 0.4);
    return vec2(a, b) * (0.010 + age * 0.040 + hB * 0.020);

}

vec3 styleColor(vec3 cached, float layer, float age, float hB, float hT) {
    vec3 p = palette(layer * 0.05 + hB * 0.8, vec3(0.5), vec3(0.5), vec3(1.0, 0.7, 0.4), vec3(0.0, 0.15, 0.25));
    return mix(cached, cached * p * 1.2, age);
