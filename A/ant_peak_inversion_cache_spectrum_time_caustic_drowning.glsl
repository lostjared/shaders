#version 330 core
// ant_peak_inversion_cache_spectrum_time_caustic_drowning
// Underwater caustics where each layer is warped 5x by an older spectrum
...
void main() {
    ...
    vec3 acc = vec3(0.0);
    for (int i = 0; i < 9; i++) {
        float h = specHist(min(i, 7), 0.1 + float(i) * 0.06) * 5.0;
        vec2 p = uv;
        for (int j = 0; j < 3; j++) {
            float fj = float(j) + float(i) * 0.3;
            p += 0.04 * vec2(sin(p.y * (5.0 + h * 40.0) + iTime + fj),
                             cos(p.x * (4.0 + h * 35.0) - iTime + fj));
        }
        float caustic = pow(max(sin(p.x * 10.0) + sin(p.y * 11.0) + sin((p.x + p.y) * 9.0), 0.0), 3.0);
        vec2 suv = p / vec2(aspect, 1.0) + 0.5;
        vec3 c = cacheHist(i, suv).rgb;
        vec3 tint = acid(float(i) * 0.13 + h * 0.4);
        acc += (tint * caustic * 1.5 + c * 0.6) * (1.0 + h * 5.0) * pow(0.83, float(i));
    }
    acc /= 3.0;
    acc *= 1.3 + amp_smooth * 1.5;
