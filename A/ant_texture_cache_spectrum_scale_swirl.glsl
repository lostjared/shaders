#version 330 core
// ant_texture_cache_spectrum_scale_swirl
...
vec2 styleDrift(vec2 uv, float layer, float age, float hB, float hM, float hT, float hA, vec3 oldest) {
    ...
    float ang = atan(uv.y, uv.x) + time_f * 0.4 + age * 1.2 + hM * 1.5;
    return (rot(uv, age * 0.8 + hT * 0.6) - uv) * 0.6 + vec2(cos(ang), sin(ang)) * (0.004 + hB * 0.020);

}

vec3 styleColor(vec3 cached, float layer, float age, float hB, float hT) {
    return cached * (0.9 + 0.2 * sin(layer * 0.6 + hT * 4.0));
