#version 330 core
// ant_cache_spectrum8_lava_flow
// Molten cells warped by stacked spectrum displacement, deep glowing palette
...
void main() {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        float h2 = specHist(i, 0.4);
        p += 0.05 * vec2(sin(p.y * 6.0 + iTime + float(i)), cos(p.x * 7.0 - iTime * 0.7 + float(i))) * (1.0 + h * 4.0);
        vec2 suv = p / vec2(aspect, 1.0) + 0.5;
        vec3 c = cacheHist(i, suv).rgb;
        vec3 lava = vec3(1.0, 0.3 + h * 0.5, 0.05) * pow(h, 1.5) * 6.0;
        lava += vec3(1.0, 0.7, 0.2) * pow(h2, 2.0) * 3.0;
        acc += (c * 0.3 + lava) * pow(0.85, float(i)) * (0.5 + h * 4.0);
    }
    acc *= 0.6 + amp_peak * 2.0;
