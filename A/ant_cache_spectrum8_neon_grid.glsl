#version 330 core
// ant_cache_spectrum8_neon_grid
// Pulsing neon grid where each grid-line layer is an older frame echo
...
void main() {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        float scale = 4.0 + float(i) * 2.0 + h * 20.0;
        vec2 g = uv * scale + iTime * vec2(0.3, 0.4) * (1.0 + h);
        vec2 gf = abs(fract(g) - 0.5);
        float line = exp(-min(gf.x, gf.y) * (20.0 - h * 18.0));
        float rot = float(i) * 0.3 + h * 2.0;
        float cs = cos(rot), sn = sin(rot);
        vec2 p = vec2(uv.x * cs - uv.y * sn, uv.x * sn + uv.y * cs);
        vec3 c = cacheHist(i, p / vec2(aspect, 1.0) + 0.5).rgb;
        vec3 tint = palette(float(i) * 0.15 + h * 2.0, vec3(0.0, 0.33, 0.66));
        acc += (tint * line * 2.0 + c * 0.6) * pow(0.85, float(i)) * (1.0 + h * 5.0);
    }
