#version 330 core
// ant_peak_inversion_cache_spectrum_time_vortex_singularity
// Spiral vortex: each cache layer twists by 5x a different historical FFT slot
...
void main() {
    ...
    for (int i = 0; i < 9; i++) {
        ...
        a += (3.0 + h * 30.0) / (r + 0.1) + iTime * (0.3 + h2 * 4.0) + float(i) * 0.5;
        vec2 p = vec2(cos(a), sin(a)) * r * (1.0 - h * 0.4);
        vec2 suv = p / vec2(aspect, 1.0) + 0.5;
        vec3 c = cacheHist(i, suv).rgb;
        c *= acid(a * 0.15 + float(i) * 0.1 + h * 0.3) * (0.7 + h * 5.0);
        float w = pow(0.83, float(i)) * (1.0 + h2);
        acc += c * w; wsum += w;
    }
    acc /= wsum;
