#version 330 core
// ant_peak_inversion_cache_spectrum_time_mandala_pulse
// Eight-petal mandala where each petal-ring is a history frame pulsing on its own band
...
void main() {
    ...
    for (int i = 0; i < 9; i++) {
        ...
        float petals = 4.0 + float(i) * 2.0;
        float ring = pow(max(sin(petals * a + iTime * (0.5 + h * 3.0)), 0.0), 4.0);
        float ringR = 0.1 + 0.07 * float(i) + h * 0.4;
        float rband = exp(-pow((r - ringR) * (12.0 - h * 4.0), 2.0));
        float rot = iTime * 0.3 + h * 6.0;
        ...
        acc += (c * 1.5 + tint * 2.5) * ring * rband * (1.0 + h * 6.0);
    }
    acc *= 1.5;
