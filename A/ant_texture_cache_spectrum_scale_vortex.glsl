#version 330 core
// ant_texture_cache_spectrum_scale_vortex
...
vec2 styleDrift(vec2 uv, float layer, float age, float hB, float hM, float hT, float hA, vec3 oldest) {
    ...
    float twist = age * 1.4 + hM * 1.2 + time_f * 0.4;
    vec2 q = vec2(cos(ang + twist), sin(ang + twist)) * r;
    return (q - uv) * 0.10;

}

vec3 styleColor(vec3 cached, float layer, float age, float hB, float hT) {
    return cached * (0.92 + 0.12 * sin(layer * 0.5 + hT * 4.0));
