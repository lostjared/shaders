#version 330 core
// ant_texture_cache_spectrum_scale_polar_echo
...
vec2 styleDrift(vec2 uv, float layer, float age, float hB, float hM, float hT, float hA, vec3 oldest) {
    ...
    return vec2(cos(a + age * TAU * 0.25), sin(a + age * TAU * 0.25)) * (age * 0.06) - uv * (age * 0.04 * (0.5 + hB));

}

vec3 styleColor(vec3 cached, float layer, float age, float hB, float hT) {
    return cached * (0.9 + 0.15 * cos(layer * 0.3 + hT * 5.0));
