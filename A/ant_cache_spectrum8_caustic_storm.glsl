#version 330 core
// ant_cache_spectrum8_caustic_storm
// Underwater caustics with each layer warped by an older spectrum
...
void main() {
    ...
    vec3 acc = vec3(0.0);
    for (int i = 0; i < 8; i++) {
        float h = specHist(i, 0.1 + float(i) * 0.06);
        vec2 p = uv;
        for (int j = 0; j < 3; j++) {
            float fj = float(j) + float(i) * 0.3;
            p += 0.04 * vec2(sin(p.y * (5.0 + h * 20.0) + iTime + fj),
                             cos(p.x * (4.0 + h * 18.0) - iTime + fj));
        }
        ...
        vec3 tint = palette(float(i) * 0.13 + h * 2.0, vec3(0.0, 0.4, 0.7));
        acc += (tint * caustic + c * 0.5) * (1.0 + h * 5.0) * pow(0.83, float(i));
    }
    acc /= 3.0;
