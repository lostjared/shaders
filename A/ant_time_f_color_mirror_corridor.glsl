#version 330 core
// ant_time_f_color_mirror_corridor
// Mirrored corridor: 8 mirror planes recede - each mirror shows older cache frame, FFT pulses depth
...
void main() {
    ...
    for (int i = 1; i < 9; i++) {
        float depth = float(i) * 0.12;
        float h = specHist(min(i, 7), 0.05) * 5.0;
        float scale = 1.0 - depth + h * 0.05;
        vec2 m = uv * scale;
        m = abs(m); // mirror into one quadrant repeatedly
        vec2 suv = m / vec2(aspect, 1.0) + 0.5;
        vec3 c = cacheHist(i, suv).rgb;
        vec3 tint = acid(float(i) * 0.13 + time_f * 0.05);
        float fade = exp(-depth * 2.0);
        acc += c * tint * fade * (1.0 + h);
    }
    acc *= 1.2 + amp_peak * 1.2;
