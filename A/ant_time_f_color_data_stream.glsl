#version 330 core
// ant_time_f_color_data_stream
// Vertical data columns: each column = one spectrum history slot scrolling, cache as background
...
void main() {
    ...
    vec3 bg = cacheHist(int(mod(floor(tc.x * 8.0), 9.0)), tc).rgb * 0.4;
    int col = int(clamp(floor(tc.x * 8.0), 0.0, 7.0));
    float scroll = fract(tc.y - time_f * 0.2);
    float s = specHist(col, scroll) * 5.0;
    float colFrac = fract(tc.x * 8.0);
    float colMask = exp(-pow((colFrac - 0.5) * 4.0, 2.0));
    vec3 stream = palette(scroll + float(col) * 0.13,
                          vec3(0.0, 0.33, 0.66)) * s * colMask * 2.0;
    vec3 acc = bg + stream;
    // column borders
    float bord = smoothstep(0.0, 0.02, min(colFrac, 1.0 - colFrac));
    acc *= 0.5 + 0.5 * bord;
    acc *= 1.0 + amp_peak;
