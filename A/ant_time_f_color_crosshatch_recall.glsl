#version 330 core
// ant_time_f_color_crosshatch_recall
// Crosshatch: 9 squares in a tight grid, each a cache frame with FFT history overlay bar
...
void main() {
    ...
    vec2 g = floor(tc * vec2(3.0, 3.0));
    int idx = int(g.y) * 3 + int(g.x);
    idx = clamp(idx, 0, 8);
    vec2 cell = fract(tc * 3.0);
    vec3 c = cacheHist(idx, cell).rgb;
    // FFT history overlay at top edge of each cell
    float h = specHist(min(idx, 7), cell.x) * 5.0;
    if (cell.y > 0.85) c = mix(c, acid(float(idx) * 0.13 + time_f * 0.1), h);
    // hatch lines
    float hatch = sin((cell.x + cell.y) * 80.0 + time_f * 2.0) * 0.5 + 0.5;
    c *= 0.7 + 0.3 * hatch;
    // border
    float gl = smoothstep(0.0, 0.015, min(cell.x, 1.0 - cell.x))
             * smoothstep(0.0, 0.015, min(cell.y, 1.0 - cell.y));
    c *= 0.4 + 0.6 * gl;
    c *= 1.1 + amp_peak;
