#version 330 core
// ant_texture_cache_spectrum_scale_lava
...
vec2 styleDrift(vec2 uv, float layer, float age, float hB, float hM, float hT, float hA, vec3 oldest) {
    float n = sin(uv.x * 4.0 + uv.y * 3.0 + time_f * 0.7 + layer * 0.4);
    float m = cos(uv.y * 5.0 - uv.x * 2.0 - time_f * 0.5 + layer * 0.3);
    return vec2(n, m) * (0.014 + age * 0.040 + hB * 0.030);

}

vec3 styleColor(vec3 cached, float layer, float age, float hB, float hT) {
    vec3 hot = vec3(1.0, 0.35, 0.12);
    return mix(cached, cached * hot * 1.3, age);
