...
// Centers on mouse only when left click is held down
...
void main(void) {
    ...
    // If left mouse button is held down (iMouse.z > 0.0), track the cursor
    if (iMouse.z > 0.0) {
        vec2 mouseUV = iMouse.xy / iResolution.xy; // 0.0 to 1.0
        mouseUV = mouseUV * 2.0 - 1.0;             // -1.0 to 1.0
        mouseUV.x *= iResolution.x / iResolution.y;
        targetCenter = mouseUV;
    }

    // Local coordinate system
    vec2 local_uv = uv - targetCenter;

    float r = length(local_uv);
    float angle = atan(local_uv.y, local_uv.x);
    ...
    vec2 dir = normalize(local_uv + 0.001); 
