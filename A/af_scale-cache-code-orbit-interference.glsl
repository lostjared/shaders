#version 330 core
// af_scale-cache-code-orbit-interference
// Three-source interference waves flowing through historical FFT orbits.
#define EFFECT_ID 1
...
vec3 acid_palette(float phase, float hue_offset) {
    vec3 base = vec3(0.5);
    vec3 amplitude = vec3(0.5);
    vec3 frequency = vec3(1.0, 1.0, 0.5);
    vec3 offset = vec3(0.30, 0.20, 0.20);
    offset += vec3(hue_offset, hue_offset * 1.2, hue_offset * 0.8);
    offset += float(EFFECT_ID) * vec3(0.031, 0.047, 0.067);
    return base + amplitude * cos(TAU * (frequency * phase + offset));
}

vec3 color_scale(vec3 video, vec3 acid_color, float energy) {
    float luminance = dot(video, vec3(0.299, 0.587, 0.114));
    vec3 channel_scale = 0.38 + acid_color * (0.78 + energy * 0.65);
    vec3 scaled = video * channel_scale;
    scaled += acid_color * luminance * (0.12 + energy * 0.18);
    return scaled;
}
...
vec2 cache_transform(vec2 uv, vec2 center, float age, float bass, float middle, float treble,
    ...
    float phase = radius * (13.0 + float(EFFECT_ID % 8)) - time_f * (2.0 + bass * 3.0) + age;
    float wave = sin(phase);
    float zoom = pow(max(0.958 - bass * 0.09, 0.35), age);

#if EFFECT_ID % 5 == 0
    point *= zoom;
    point += radial * wave * age * (0.004 + treble * 0.01);
    point += flow * age * (0.006 + middle * 0.012);
#elif EFFECT_ID % 5 == 1
    point.x += sin(point.y * (12.0 + middle * 8.0) + time_f + age) *
               age * (0.006 + treble * 0.012);
    point.y += cos(point.x * 10.0 - time_f * 0.7 - age) *
               age * (0.005 + bass * 0.01);
    point *= zoom;
#elif EFFECT_ID % 5 == 2
    point += radial * sin(phase + atan(point.y, point.x) * 4.0) *
             age * (0.008 + middle * 0.014);
    point *= zoom;
#elif EFFECT_ID % 5 == 3
    point.x *= 1.0 + age * (0.009 + treble * 0.012);
    point.y *= zoom;
    point += vec2(wave, -wave) * age * 0.004;
#else
    vec2 folded = abs(point);
    folded -= vec2(0.12 + bass * 0.08, 0.09 + middle * 0.07);
    point = mix(point, sign(point + vec2(0.0001)) * folded, 0.35 + treble * 0.25);
    point *= zoom;
    point += radial * wave * age * 0.006;
#endif
...
void main() {
    ...
    float global_hue = 0.0;
    if (iMouse.w > 0.0)
        global_hue = (mouse_uv.x - 0.5) * 3.0;
    ...
    float ripple_x =
        sin(point.x * (11.0 + float(EFFECT_ID % 7)) + time_f * (4.0 + bass * 3.0));
    float ripple_y =
        sin(point.y * (12.0 + float(EFFECT_ID % 6)) + time_f * (3.5 + middle * 2.0));
    float cross_ripple = sin((point.x + point.y) * (8.0 + float(EFFECT_ID % 5)) -
                             time_f * (2.5 + treble * 3.0));
    vec2 wave_distortion =
        vec2(ripple_x + cross_ripple * 0.6, ripple_y - cross_ripple * 0.6) *
        (0.008 + bass * 0.018);
    wave_distortion += surface_normal.xy * (0.012 + amp_low * 0.025);
    vec2 live_uv = mirror_repeat(tc + wave_distortion);
    ...
    vec3 live_palette = acid_palette(field + time_f * 0.025 + bass, global_hue);
    vec3 scaled_live = color_scale(live, live_palette, bass * 0.5 + middle * 0.3 + treble * 0.2);
    vec3 mapped = 0.5 + 0.5 * cos(clamp(scaled_live, 0.0, 1.0) * PI * 0.5 +
                                  live_palette);
    vec3 accumulated = mix(scaled_live, mapped * (0.45 + luminance), 0.44);
    accumulated += acid_palette(field * 0.55 + time_f * 0.04, global_hue) *
    ...
    for (int index = 0; index < 8; ++index) {
        ...
        float old_air = sample_history(index, 0.82);
        vec2 history_uv =
        ...
        float old_energy =
            old_bass * 0.45 + old_middle * 0.28 + old_treble * 0.17 + old_air * 0.10;
        vec3 history_tint =
            acid_palette(field * 0.12 + age * 0.061 + old_energy * 0.7, global_hue);
        vec3 cache_scale = vec3(1.0 + age * 0.035 + old_middle * 0.22,
                                1.0 - age * 0.021 + old_bass * 0.12,
                                1.0 + age * 0.016 + old_air * 0.28);
        history = color_scale(history * max(cache_scale, vec3(0.05)), history_tint, old_energy);
    ...
    accumulated += acid_palette(field + time_f * 0.09, global_hue) * crest *
                   (0.25 + air * 1.35);
