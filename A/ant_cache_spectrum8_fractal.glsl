#version 330 core
// ant_cache_spectrum8_fractal
// IFS-style fractal feedback where each iteration pulls from a different history
...
void main() {
    ...
    vec3 acc = vec3(0.0);
    for (int i = 0; i < 8; i++) {
        float h = specHist(i, 0.1 + float(i) * 0.08);
        p = abs(p) / dot(p, p) - vec2(0.7 + h * 0.6, 0.5 + bass * 0.4);
        float rot = iTime * 0.2 + h * 5.0;
        float cs = cos(rot), sn = sin(rot);
        p = vec2(p.x * cs - p.y * sn, p.x * sn + p.y * cs) * 0.8;
        vec2 suv = clamp(p * 0.3 + 0.5, 0.0, 1.0);
        vec3 c = cacheHist(i, suv).rgb;
        c *= palette(length(p) * 0.3 + float(i) * 0.1, vec3(0.0, 0.3, 0.6)) * (1.0 + h * 6.0);
        acc += c * pow(0.8, float(i));
    }
    acc /= 3.0;
