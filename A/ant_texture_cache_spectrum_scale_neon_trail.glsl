#version 330 core
// ant_texture_cache_spectrum_scale_neon_trail
...
vec2 styleDrift(vec2 uv, float layer, float age, float hB, float hM, float hT, float hA, vec3 oldest) {
    return vec2(cos(layer * 0.5 + time_f), sin(layer * 0.6 - time_f)) * (0.005 + age * 0.040 + hT * 0.015);

}

vec3 styleColor(vec3 cached, float layer, float age, float hB, float hT) {
    vec3 hot = palette(layer * 0.07 + hT * 0.4, vec3(0.5), vec3(0.5), vec3(1.0), vec3(0.0, 0.33, 0.66));
    return mix(cached, cached * hot * 1.4, age * 0.7);
...
void main() {
    ...
    accum += pow(accum, vec3(2.0)) * (0.18 + amp_peak * 0.4);
