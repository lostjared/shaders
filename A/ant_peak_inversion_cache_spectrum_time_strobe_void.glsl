#version 330 core
// ant_peak_inversion_cache_spectrum_time_strobe_void
// Tunnel where every history slot strobe-inverts on its band peaks at 5x
...
void main() {
    ...
    for (int i = 0; i < 9; i++) {
        ...
        float strobe = step(1.0, h);
        float zoom = max(0.05, 1.0 - 0.08 * float(i) - h * 0.5);
        ...
        vec3 tint = acid(float(i) * 0.13 + r * 3.0 + h * 0.3);
        if (strobe > 0.5) c = 1.0 - c;
        acc += (c + tint * h * 1.5) * pow(0.83, float(i)) * (1.0 + h * 5.0);
    }
    acc /= 3.0;
    color = vec4(clamp(acc * (1.0 + amp_peak * 2.5), 0.0, 1.0), 1.0);
}
