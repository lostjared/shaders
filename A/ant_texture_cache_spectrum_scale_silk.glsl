#version 330 core
// ant_texture_cache_spectrum_scale_silk
...
vec2 styleDrift(vec2 uv, float layer, float age, float hB, float hM, float hT, float hA, vec3 oldest) {
    vec2 d = vec2(
        sin(uv.y * 3.5 + time_f * 0.9 + layer * 0.25),
        cos(uv.x * 3.0 - time_f * 0.7 + layer * 0.20)
    );
    d *= 0.012 + age * 0.045 + hB * 0.030 + hT * 0.012;
    return d;

}

vec3 styleColor(vec3 cached, float layer, float age, float hB, float hT) {
    return cached * (0.95 + 0.05 * sin(layer * 0.4 + hT * 3.0));
...
void main() {
    ...
    float r = length(uv);
    accum *= 0.94 + 0.06 * (0.5 + 0.5 * sin(time_f * 1.2 - r * 2.5 + histTreble * 4.0)) + amp_smooth * 0.10;
