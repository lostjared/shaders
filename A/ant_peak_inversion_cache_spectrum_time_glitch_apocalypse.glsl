#version 330 core
// ant_peak_inversion_cache_spectrum_time_glitch_apocalypse
// Block-displacement glitch where each band offset is 5x a different FFT slot
...
void main() {
    ...
    for (int i = 0; i < 9; i++) {
        ...
        float h = specHist(min(i, 7), fract(band * 3.7 + float(i) * 0.13)) * 5.0;
        float jitter = (h - 0.6) * 1.2;
        if (h > 1.0)
            jitter *= 3.0;
