#version 330 core
// ant_time_f_color_light_painting
// Long-exposure light painting: additive blend of all cache frames with audio-driven trails
...
void main() {
    ...
    for (int i = 0; i < 9; i++) {
        float t = float(i);
        float h = specHist(min(i, 7), 0.1 + t * 0.05) * 5.0;
        vec2 dir = vec2(cos(time_f * 0.3 + t * 0.5),
                        sin(time_f * 0.4 + t * 0.6));
        vec2 off = dir * 0.04 * t * (1.0 + h);
