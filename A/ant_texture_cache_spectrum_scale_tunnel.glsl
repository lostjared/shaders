#version 330 core
// ant_texture_cache_spectrum_scale_tunnel
...
vec2 styleDrift(vec2 uv, float layer, float age, float hB, float hM, float hT, float hA, vec3 oldest) {
    float zoom = 1.0 + age * 0.18 + hB * 0.12;
    return uv * (zoom - 1.0) * (0.6) + vec2(0.0);

}

vec3 styleColor(vec3 cached, float layer, float age, float hB, float hT) {
    return cached * (0.88 + age * 0.18);
...
void main() {
    ...
    float r = length(uv);
    accum *= 0.92 + 0.10 * smoothstep(0.0, 1.0, 1.0 - r) + amp_smooth * 0.10;
