#version 330 core
// ant_texture_cache_spectrum_scale_inkdrop
...
vec2 styleDrift(vec2 uv, float layer, float age, float hB, float hM, float hT, float hA, vec3 oldest) {
    float r = length(uv);
    return vec2(uv.y, -uv.x) * (age * 0.05) + vec2(sin(r * 8.0 + time_f), cos(r * 9.0 - time_f)) * (0.005 + hM * 0.012);

}

vec3 styleColor(vec3 cached, float layer, float age, float hB, float hT) {
    vec3 ink = mix(vec3(0.05, 0.05, 0.10), cached, 1.0 - age * 0.5);
    return ink;
