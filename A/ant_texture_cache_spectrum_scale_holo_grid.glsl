#version 330 core
// ant_texture_cache_spectrum_scale_holo_grid
...
vec2 styleDrift(vec2 uv, float layer, float age, float hB, float hM, float hT, float hA, vec3 oldest) {
    return vec2(sin(uv.y * 22.0 + time_f * 3.0 + layer),
                cos(uv.x * 22.0 - time_f * 2.5 + layer)) * (0.003 + age * 0.012 + hT * 0.008);

}

vec3 styleColor(vec3 cached, float layer, float age, float hB, float hT) {
    return cached * (1.0 + 0.05 * sin(layer * 0.9 + hT * 6.0)) * mix(vec3(1.0), vec3(0.7, 1.0, 1.2), age);
...
void main() {
    ...
    // Subtle radial glint, no axis-aligned bands.
    float r = length(uv);
    accum += 0.04 * vec3(0.4, 0.7, 1.0) * smoothstep(0.7, 0.0, r) * (amp_smooth + 0.1);
