#version 330 core
// ant_time_f_color_ghost_train
// Eight ghost copies of past frames marching across screen, each with its FFT slot tint
...
void main() {
    ...
    for (int i = 1; i < 9; i++) {
        float t = float(i) / 8.0;
        float march = time_f * 0.2 + float(i) * 0.4;
        vec2 off = vec2(0.15 * float(i) * sin(march), 0.05 * cos(march * 0.7));
        float h = specHist(min(i, 7), 0.05 + t * 0.4) * 5.0;
        vec3 c = cacheHist(i, tc + off).rgb;
        vec3 tint = palette(float(i) * 0.13 + time_f * 0.05,
                            vec3(0.0, 0.33, 0.66));
        float alpha = (0.7 - t * 0.5) * (0.5 + h);
        acc += c * tint * alpha;
    }
