#version 330 core
// ant_cache_spectrum8_chromatic_pulse
// Three-channel chromatic split, each channel pulses to its own spectrum history
...
void main() {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        float zoom = 1.0 - 0.05 * float(i) - hR * 0.2;
        ...
        vec3 c;
        c.r = cacheHist(i, suv + vec2(hR * 0.15, 0.0)).r * (1.0 + hR * 5.0);
        c.g = cacheHist(i, suv + vec2(0.0, hG * 0.15)).g * (1.0 + hG * 5.0);
        c.b = cacheHist(i, suv - vec2(hB * 0.15, 0.0)).b * (1.0 + hB * 5.0);
