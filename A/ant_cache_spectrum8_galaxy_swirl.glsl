#version 330 core
// ant_cache_spectrum8_galaxy_swirl
// Spiral galaxy arms where each arm-history layer twists by its old spectrum
...
void main() {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        float arms = 2.0 + floor(h2 * 5.0);
        float twist = a * arms - log(r + 0.05) * (5.0 + h * 8.0) + iTime * (0.3 + h * 2.0) + float(i) * 0.5;
        ...
        acc += (tint * dust * falloff * 2.5 + c * 0.4) * (1.0 + h * 6.0) * pow(0.83, float(i));
    }
