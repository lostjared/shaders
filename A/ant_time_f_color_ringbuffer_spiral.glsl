#version 330 core
// ant_time_f_color_ringbuffer_spiral
// Spiral with 9 arms - each arm reads from a different cache frame; FFT history sets arm radius
...
void main() {
    ...
    float wsum = 0.0;
    for (int i = 0; i < 9; i++) {
        float h = specHist(min(i, 7), 0.05 + float(i) * 0.04);
        float armAng = float(i) * (TAU / 9.0) + time_f * 0.4;
        float armR = 0.15 + 0.05 * float(i) + h * 0.5;
        float d = abs(mod(a - armAng, TAU) - PI) * armR + abs(r - armR);
        float arm = exp(-d * (8.0 - h * 6.0));
        vec2 suv = (uv * (0.9 - float(i) * 0.05)) / vec2(aspect, 1.0) + 0.5;
        vec3 c = cacheHist(i, suv).rgb;
        vec3 tint = acid(float(i) * 0.12 + h * 0.6 + time_f * 0.05);
        acc += (c * 0.6 + tint * arm * 3.0) * (1.0 + h * 4.0);
        wsum += 1.0;
    }
    acc /= wsum;
    acc = mix(acc, 1.0 - acc, smoothstep(0.75, 1.0, amp_peak));
    color = vec4(clamp(acc * (1.0 + E_bass * 0.3), 0.0, 1.0), 1.0);
}
