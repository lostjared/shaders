#version 330 core
// ant_peak_inversion_cache_spectrum_time_fractal_inferno
// IFS fractal feedback: every iteration pulls from a deeper history with 5x folding
...
void main() {
    ...
    vec3 acc = vec3(0.0);
    for (int i = 0; i < 9; i++) {
        float h = specHist(min(i, 7), 0.1 + float(i) * 0.08) * 5.0;
        p = abs(p) / dot(p, p) - vec2(0.7 + h * 1.5, 0.5 + bass * 1.8);
        float rot = iTime * 0.2 + h * 6.0;
        float cs = cos(rot), sn = sin(rot);
        p = vec2(p.x * cs - p.y * sn, p.x * sn + p.y * cs) * 0.8;
        vec2 suv = clamp(p * 0.3 + 0.5, 0.0, 1.0);
        vec3 c = cacheHist(i, suv).rgb;
        c *= acid(length(p) * 0.3 + float(i) * 0.1 + h * 0.3) * (1.0 + h * 6.0);
        acc += c * pow(0.82, float(i));
    }
    acc /= 3.2;
    acc *= 1.3 + amp_smooth * 1.5 + E_treble * 0.3;
