#version 330 core
// ant_time_f_color_quantum_freeze
// Blocky time-freeze: pixels random-pick from one of 9 cache frames per tile
...
void main() {
    ...
    vec2 tile = floor(tc * 24.0);
    float pick = hash(tile + floor(time_f * 4.0));
    int idx = int(floor(pick * 9.0));
    idx = clamp(idx, 0, 8);
    vec2 cell = fract(tc * 24.0);
    vec3 c = cacheHist(idx, tc).rgb;
    float h = specHist(min(idx, 7), pick) * 5.0;
    vec3 tint = acid(float(idx) * 0.13 + time_f * 0.1 + h * 0.4);
    c *= tint * (1.0 + h);
    // tile separator
    float gl = smoothstep(0.0, 0.05, min(cell.x, 1.0 - cell.x)) * smoothstep(0.0, 0.05, min(cell.y, 1.0 - cell.y));
    c *= 0.5 + 0.5 * gl;
    c *= 1.0 + amp_peak * 1.5;
