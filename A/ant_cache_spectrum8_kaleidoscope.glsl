#version 330 core
// ant_cache_spectrum8_kaleidoscope
// Kaleidoscope where each slice samples a different cache+spectrum frame
...
void main() {
    ...
    int slices = 6 + int(floor(treble * 8.0));
    float seg = TAU / float(slices);
    float aseg = mod(a, seg);
    aseg = abs(aseg - seg * 0.5);
    vec2 kuv = vec2(cos(aseg), sin(aseg)) * r;

    vec3 acc = vec3(0.0);
    float w = 0.0;
    for (int i = 0; i < 8; i++) {
        float h = specHist(i, 0.03 + float(i) * 0.05);
        float rot = iTime * 0.4 + float(i) * 0.7 + h * 4.0;
        float cs = cos(rot), sn = sin(rot);
        vec2 r2 = vec2(kuv.x * cs - kuv.y * sn, kuv.x * sn + kuv.y * cs);
        vec2 suv = r2 / vec2(aspect, 1.0) + 0.5;
        vec3 c = cacheHist(i, suv).rgb;
        c *= palette(float(i) * 0.13 + h * 2.0, vec3(0.0, 0.33, 0.66)) * (1.0 + h * 5.0);
        float wi = pow(0.85, float(i));
        acc += c * wi;
        w += wi;
    }
    acc /= w;
