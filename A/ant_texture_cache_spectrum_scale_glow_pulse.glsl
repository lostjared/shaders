#version 330 core
// ant_texture_cache_spectrum_scale_glow_pulse
...
vec2 styleDrift(vec2 uv, float layer, float age, float hB, float hM, float hT, float hA, vec3 oldest) {
    return vec2(cos(layer * 0.4 + time_f), sin(layer * 0.5 - time_f)) * (0.005 + age * 0.020 + hT * 0.010);

}

vec3 styleColor(vec3 cached, float layer, float age, float hB, float hT) {
    return cached * (0.95 + 0.10 * sin(layer * 0.6 + hT * 4.0));
...
void main() {
    ...
    vec3 glow = pow(max(accum - 0.5, vec3(0.0)), vec3(2.0));
    accum += glow * (0.4 + amp_peak * 0.5);
    accum *= 0.96 + amp_smooth * 0.12;
