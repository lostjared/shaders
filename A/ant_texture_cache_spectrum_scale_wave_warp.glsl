#version 330 core
// ant_texture_cache_spectrum_scale_wave_warp
...
vec2 styleDrift(vec2 uv, float layer, float age, float hB, float hM, float hT, float hA, vec3 oldest) {
    return vec2(
        sin(uv.y * 6.0 + time_f * 1.3 + layer * 0.4),
        sin(uv.x * 5.0 - time_f * 1.1 + layer * 0.3)
    ) * (0.012 + age * 0.030 + hB * 0.020);

}

vec3 styleColor(vec3 cached, float layer, float age, float hB, float hT) {
    return cached * (0.96 + 0.06 * cos(layer * 0.5 + hT * 4.0));
