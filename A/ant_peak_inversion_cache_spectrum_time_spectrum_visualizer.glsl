#version 330 core
// ant_peak_inversion_cache_spectrum_time_spectrum_visualizer
// Literal animated FFT bars rendered as rings AROUND the cache feedback core
...
void main() {
    ...
    vec3 base = vec3(0.0);
    for (int i = 0; i < 9; i++) {
        float zoom = max(0.05, 1.0 - 0.06 * float(i));
    ...
    for (int i = 0; i < 8; i++) {
        ...
        float h = specHist(i, band) * 5.0;
        float bar = smoothstep(ringR + 0.005, ringR, r) - smoothstep(ringR + 0.04 + h * 0.3, ringR + 0.005 + h * 0.3, r);
        bar = max(bar, 0.0);
        vec3 tint = acid(float(i) * 0.13 + h * 0.3);
        bars += tint * bar * (1.0 + h * 8.0);
    }
    color = vec4(clamp(base + bars * 2.5, 0.0, 1.0), 1.0);
}
