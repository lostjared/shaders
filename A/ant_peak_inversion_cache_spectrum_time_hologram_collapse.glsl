#version 330 core
// ant_peak_inversion_cache_spectrum_time_hologram_collapse
// Holographic interference fringes shifting per FFT history slot at 5x
...
void main() {
    ...
    for (int i = 0; i < 9; i++) {
        ...
        float fringe = sin(dot(uv, dir) * (40.0 + h * 160.0) + iTime * (1.0 + h * 4.0));
        fringe = pow(fringe * 0.5 + 0.5, 4.0);
        float shift = fringe * 0.05 * (1.0 + h);
        vec2 suv = tc + dir * shift;
        vec3 c;
        c.r = cacheHist(i, suv + vec2(h * 0.2, 0.0)).r;
        c.g = cacheHist(i, suv).g;
        c.b = cacheHist(i, suv - vec2(h * 0.2, 0.0)).b;
        vec3 tint = acid(float(i) * 0.13 + h * 0.5);
        acc += (c * 1.2 + tint * fringe * 1.0) * (1.0 + h * 5.0) * pow(0.83, float(i));
    }
    acc /= 3.0;
