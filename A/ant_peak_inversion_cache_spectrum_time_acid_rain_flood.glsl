#version 330 core
// ant_peak_inversion_cache_spectrum_time_acid_rain_flood
// Vertical rain streaks - each column at 5x its FFT history bass
...
void main() {
    ...
    for (int i = 0; i < 8; i++) {
        float h = specHist(i, 0.04) * 5.0;
        float h2 = specHist(i, 0.5) * 5.0;
        float col = floor(tc.x * (40.0 + float(i) * 10.0));
        float speed = 0.3 + h * 5.0;
        float seed = hash(vec2(col, float(i)));
        float y = mod(tc.y - iTime * speed - seed, 1.0);
        float drop = exp(-y * (15.0 - h * 13.0)) * step(0.3, fract(seed * 9.0));
        vec3 tint = acid(seed + h2 * 0.4 + float(i) * 0.1);
        vec2 suv = tc + vec2(0.0, drop * 0.1);
        vec3 c = cacheHist(i, suv).rgb;
        acc += (tint * drop * 4.0 + c * 0.4) * (1.0 + h * 6.0) * pow(0.84, float(i));
    }
