#version 330 core
// ant_peak_inversion_cache_spectrum_time_plasma_furnace
// Stacked plasma fields, each layer warped 5x by its own historical FFT slot
...
void main() {
    ...
    vec3 acc = vec3(0.0);
    for (int i = 0; i < 9; i++) {
        float h = specHist(min(i, 7), 0.05 + float(i) * 0.04) * 5.0;
        float h2 = specHist(min(i, 7), 0.5 - float(i) * 0.05) * 5.0;
        float t = iTime * (0.5 + h * 2.0) + float(i);
        float v = sin(uv.x * (8.0 + h * 60.0) + t) + sin(uv.y * (10.0 + h2 * 50.0) - t * 1.1) + sin(length(uv * (4.0 + h * 35.0)) - t);
        vec2 d = vec2(v) * (0.04 + h * 0.5);
        vec2 suv = tc + d;
        vec3 c = cacheHist(i, suv).rgb;
        c *= acid(v * 0.2 + float(i) * 0.12 + h2 * 0.4) * (0.6 + h * 5.0 + h2 * 3.0);
        acc += c * pow(0.83, float(i));
    }
    acc /= 3.5;
    acc *= 1.0 + (E_bass + E_mid) * 0.4;
