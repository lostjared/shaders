#version 330 core
// ant_time_f_color_glitch_archive
// Glitched archive readout: scanlines snap to different cache frames at audio-driven intervals
...
void main() {
    ...
    float band = floor(tc.y * 32.0);
    float pick = hash(vec2(band, floor(time_f * 6.0))) * 9.0;
    int idx = clamp(int(pick), 0, 8);
    float h = specHist(min(idx, 7), tc.x) * 5.0;
    vec2 jitter = vec2(hash(vec2(band, time_f)) * 0.1 - 0.05, 0.0) * h;
    vec3 c = cacheHist(idx, tc + jitter).rgb;
    vec3 tint = acid(float(idx) * 0.13 + time_f * 0.05);
    c *= tint * (1.0 + h);
    // scanline darkening between bands
    float scan = smoothstep(0.0, 0.05, abs(fract(tc.y * 32.0) - 0.5));
    c *= 0.5 + 0.5 * scan;
    c *= 1.1 + amp_smooth;
