#version 330 core
// ant_peak_inversion_cache_spectrum_time_neon_grid_inferno
// Infinite neon grid where each gridline layer is an older frame inverted 5x by FFT
...
void main() {
    ...
    vec3 acc = vec3(0.0);
    for (int i = 0; i < 9; i++) {
        float h = specHist(min(i, 7), 0.05 + float(i) * 0.06) * 5.0;
        float scale = 4.0 + float(i) * 2.0 + h * 40.0;
        vec2 g = uv * scale + iTime * vec2(0.3, 0.4) * (1.0 + h);
        vec2 gf = abs(fract(g) - 0.5);
        float line = exp(-min(gf.x, gf.y) * (20.0 - h * 19.0));
        float rot = float(i) * 0.3 + h * 2.5;
        float cs = cos(rot), sn = sin(rot);
        vec2 p = vec2(uv.x * cs - uv.y * sn, uv.x * sn + uv.y * cs);
        vec3 c = cacheHist(i, p / vec2(aspect, 1.0) + 0.5).rgb;
        if (h > 0.5) c = 1.0 - c;
        vec3 tint = acid(float(i) * 0.15 + h * 0.5);
        acc += (tint * line * 3.0 + c * 0.7) * pow(0.85, float(i)) * (1.0 + h * 5.0);
    }
