#version 330 core
// ant_peak_inversion_cache_spectrum_time_lightning_god
// Massive lightning forks driven 5x by each history slot's mid/treble peaks
...
void main() {
    ...
    vec3 acc = cacheHist(0, tc).rgb * 0.3;
    for (int i = 0; i < 8; i++) {
        float h = specHist(i, 0.5 - float(i) * 0.05) * 5.0;
        float t = iTime * (1.5 + h * 5.0) + float(i);
        vec2 p = uv;
        p.x += sin(p.y * (10.0 + float(i) * 4.0) + t) * (0.05 + h * 0.6);
        p.x += sin(p.y * 35.0 + t * 2.0) * h * 0.3;
        float bolt = exp(-abs(p.x) * (40.0 - h * 35.0));
        bolt *= step(0.3, h);
        vec3 tint = palette(float(i) * 0.12 + 0.1, vec3(0.7, 0.8, 1.0));
        vec2 suv = (p + vec2(h * 0.15, 0.0)) / vec2(aspect, 1.0) + 0.5;
        vec3 c = cacheHist(i + 1, suv).rgb * (0.3 + h * 2.5);
        acc += (tint * bolt * 4.0 + c) * pow(0.78, float(i)) * (1.0 + h * 8.0);
    }
    acc = mix(acc, 1.0 - acc, smoothstep(0.6, 0.9, amp_peak));
