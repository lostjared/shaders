#version 330 core
// ant_texture_cache_spectrum_scale_kaleido
...
vec2 styleDrift(vec2 uv, float layer, float age, float hB, float hM, float hT, float hA, vec3 oldest) {
    vec2 p = rot(uv, layer * 0.5 + time_f * 0.2);
    p = abs(p) - 0.05 * age;
    return (p - uv) * 0.05 + vec2(sin(time_f + layer), cos(time_f - layer)) * (0.003 + hT * 0.012);

}

vec3 styleColor(vec3 cached, float layer, float age, float hB, float hT) {
    return cached * (0.95 + 0.10 * sin(layer * 0.7 + hT * 4.0));
...
void main() {
    ...
    accum = mix(accum, accum.brg, 0.10 * histTreble);
