#version 330 core
// ant_time_f_color_spectrum_waterfall
// Vertical waterfall: y axis IS time - top row spectrum0 (now), bottom row spectrum7 (oldest)
...
void main() {
    ...
    float y = tc.y;
    int hi = int(clamp(floor(y * 8.0), 0.0, 7.0));
    float frac = fract(y * 8.0);
    float s = mix(specHist(hi, tc.x), specHist(min(hi + 1, 7), tc.x), frac);
    s *= 6.0;
    vec3 wfall = palette(s + float(hi) * 0.15 + time_f * 0.1,
                         vec3(0.0, 0.33, 0.66));
    wfall *= s;
    // mix in cache frame matching same age
    vec2 cuv = vec2(tc.x, frac);
    vec3 cc = cacheHist(hi + 1, cuv).rgb;
    vec3 acc = wfall * 1.5 + cc * 0.5;
    // separator lines so the strips are obvious
    float sep = smoothstep(0.0, 0.02, abs(fract(y * 8.0) - 0.5) - 0.46);
    acc *= 0.4 + 0.6 * sep;
    acc *= 1.0 + amp_peak * 1.5;
