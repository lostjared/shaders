#version 330 core
// ant_time_f_color_spectrogram_paint
// Full-screen spectrogram: x = freq, y = age (0..7) - obvious 2D readout of all FFT history
...
void main() {
    ...
    int idx = int(clamp(floor(tc.y * 8.0), 0.0, 7.0));
    float frac = fract(tc.y * 8.0);
    float s1 = specHist(idx, tc.x);
    float s2 = specHist(min(idx + 1, 7), tc.x);
    float s = mix(s1, s2, frac) * 6.0;
    vec3 spec = palette(s + float(idx) * 0.13 + time_f * 0.1,
                        vec3(0.0, 0.33, 0.66)) * s;
    // tinted cache layer underneath, indexed by row
    vec3 cc = cacheHist(idx + 1, vec2(tc.x, frac)).rgb * 0.4;
    vec3 acc = cc + spec * 1.5;
    // row separator
    float bord = smoothstep(0.0, 0.015, abs(frac - 0.5) - 0.46);
    acc *= 0.5 + 0.5 * bord;
