#version 330 core
// ant_texture_cache_spectrum_scale_cosmic_web
...
vec2 styleDrift(vec2 uv, float layer, float age, float hB, float hM, float hT, float hA, vec3 oldest) {
    float n = hash12(floor(uv * (10.0 + age * 8.0)) + layer);
    return (vec2(n, fract(n * 7.13)) - 0.5) * (0.006 + age * 0.015 + hT * 0.012);

}

vec3 styleColor(vec3 cached, float layer, float age, float hB, float hT) {
    vec3 web = mix(cached, cached * vec3(0.6, 0.7, 1.4), age * 0.6);
    return web;
