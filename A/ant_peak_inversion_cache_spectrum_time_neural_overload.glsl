#version 330 core
// ant_peak_inversion_cache_spectrum_time_neural_overload
// Eight neuron nodes firing 5x with their FFT slot - dendrites = old cache frames
...
void main() {
    ...
    for (int i = 0; i < 8; i++) {
        float h = specHist(i, 0.05 + float(i) * 0.07) * 5.0;
        float h2 = specHist(i, 0.5) * 5.0;
        float ang = float(i) * (TAU / 8.0) + iTime * (0.3 + h * 2.0);
        vec2 node = (0.4 + h * 0.1) * vec2(cos(ang), sin(ang));
        float d = length(uv - node);
        float fire = exp(-d * (8.0 - h * 7.0)) * (0.5 + h * 6.0);
        vec2 dn = uv - node;
        ...
        acc += (tint * (fire + dend) * 3.0 + c * 0.5) * (1.0 + h * 5.0) * pow(0.85, float(i));
    }
    acc *= 1.2 + amp_peak * 2.5;
