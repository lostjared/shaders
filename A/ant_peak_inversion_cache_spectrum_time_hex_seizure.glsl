#version 330 core
// ant_peak_inversion_cache_spectrum_time_hex_seizure
// Hex tiles - each tile flashes hard from its assigned 5x history slot
...
void main() {
    ...
    float hf = specHist(slot, fract(idHash * 5.7)) * 5.0;
    ...
    if (hf > 1.5) c = 1.0 - c;
    vec3 tint = acid(idHash + hf * 0.4 + iTime * 0.2);
    vec3 acc = c * (0.4 + hf * 5.0) + tint * (1.0 - edge) * (0.5 + hf * 6.0);
    acc += vec3(1.0) * step(1.5, hf) * (1.0 - edge) * 0.8;
