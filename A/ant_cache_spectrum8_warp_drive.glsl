#version 330 core
// ant_cache_spectrum8_warp_drive
// Hyperspace streaks where streak length = older spectrum bass impulses
...
void main() {
    ...
    vec3 acc = vec3(0.0);
    for (int i = 0; i < 8; i++) {
        float h = specHist(i, 0.04);
        float h2 = specHist(i, 0.5);
        float r = length(uv);
        float a = atan(uv.y, uv.x);
        float streaks = pow(max(sin(a * (40.0 + float(i) * 8.0) + iTime * 4.0 + float(i)), 0.0), 8.0);
        float stretch = 1.0 + h * 3.0;
        float speed = iTime * (2.0 + h * 8.0);
        float band = mod(r * (3.0 + float(i) * 0.5) - speed, 1.0);
        float pulse = exp(-pow((band - 0.5) * (4.0 - h * 3.0), 2.0)) * stretch;
        vec3 tint = palette(float(i) * 0.13 + h2, vec3(0.0, 0.33, 0.66));
        vec2 suv = (uv * (1.0 - 0.05 * float(i))) / vec2(aspect, 1.0) + 0.5;
        vec3 c = cacheHist(i, suv).rgb;
        acc += (tint * streaks * pulse * 2.0 + c * 0.4) * (1.0 + h * 5.0) * pow(0.83, float(i));
    }
