#version 330 core
// ant_texture_cache_spectrum_scale_mosaic
...
vec2 styleDrift(vec2 uv, float layer, float age, float hB, float hM, float hT, float hA, vec3 oldest) {
    float cells = 28.0 + age * 12.0;
    vec2 g = floor(uv * cells) / cells - uv;
    return g * 0.4 + vec2(sin(layer + time_f), cos(layer - time_f)) * (0.002 + hM * 0.010);

}

vec3 styleColor(vec3 cached, float layer, float age, float hB, float hT) {
    return cached * (0.92 + 0.10 * sin(layer * 0.45 + hB * 4.0));
