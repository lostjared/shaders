#version 330 core
// ant_cache_spectrum8_lightning
// Forks of lightning whose branches are gated by older spectrum impulses
...
void main() {
    ...
    for (int i = 0; i < 8; i++) {
        float h = specHist(i, 0.5 - float(i) * 0.05);
        float t = iTime * (1.5 + h * 6.0) + float(i);
        vec2 p = uv;
        p.x += sin(p.y * (10.0 + float(i) * 4.0) + t) * (0.05 + h * 0.4);
        p.x += sin(p.y * 35.0 + t * 2.0) * h * 0.2;
        float bolt = exp(-abs(p.x) * (40.0 - h * 30.0));
        bolt *= step(0.4, h);
        vec3 tint = palette(float(i) * 0.12 + 0.1, vec3(0.7, 0.8, 1.0));
        vec2 suv = (p + vec2(h * 0.1, 0.0)) / vec2(aspect, 1.0) + 0.5;
        vec3 c = cacheHist(i + 1, suv).rgb * (0.3 + h * 2.0);
        acc += (tint * bolt * 3.0 + c) * pow(0.78, float(i)) * (1.0 + h * 8.0);
    }
    acc = mix(acc, 1.0 - acc, smoothstep(0.7, 0.95, amp_peak) * 0.5);
