#version 330 core
// ant_peak_inversion_cache_spectrum_time_magma_eruption
// Molten flow with each older frame layered as a hotter, faster-warped lava sheet
...
void main() {
    ...
    for (int i = 0; i < 9; i++) {
        ...
        float h2 = specHist(min(i, 7), 0.4) * 5.0;
        p += 0.05 * vec2(sin(p.y * 6.0 + iTime + float(i)), cos(p.x * 7.0 - iTime * 0.7 + float(i))) * (1.0 + h * 5.0);
        vec2 suv = p / vec2(aspect, 1.0) + 0.5;
        vec3 c = cacheHist(i, suv).rgb;
        vec3 lava = vec3(1.0, 0.3 + h * 0.5, 0.05) * pow(min(h, 2.0), 1.5) * 4.0;
        lava += vec3(1.0, 0.7, 0.2) * pow(min(h2, 2.0), 2.0) * 2.5;
        acc += (c * 0.4 + lava) * pow(0.85, float(i)) * (0.5 + h * 4.0);
    }
    acc *= 0.6 + amp_peak * 2.5;
