#version 330 core
// ant_time_f_color_trail_blaze
// Motion blur trail: average of all 9 cache frames shifted along audio-driven path
...
void main() {
    ...
    vec2 dir = vec2(cos(time_f * 0.5), sin(time_f * 0.4));
    for (int i = 0; i < 9; i++) {
        float t = float(i) / 8.0;
        float h = specHist(min(i, 7), 0.1) * 5.0;
        vec2 off = dir * (t * 0.25) * (1.0 + h * 2.0);
        vec3 c = cacheHist(i, tc + off).rgb;
        vec3 tint = mix(vec3(1.0, 0.3, 0.1), vec3(0.1, 0.5, 1.0), t);
        float w = pow(0.78, float(i)) * (1.0 + h);
        acc += c * tint * w;
        wsum += w;
    }
    acc /= wsum;
    acc *= 1.5 + amp_peak * 1.5;
