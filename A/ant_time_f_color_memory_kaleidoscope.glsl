#version 330 core
// ant_time_f_color_memory_kaleidoscope
// Kaleidoscope where each pie slice samples a different cache frame and FFT slot
...
void main() {
    ...
    int idx = int(mod(floor((a + PI) / seg), float(slices)));
    float aa = mod(a + PI, seg) - seg * 0.5;
    vec2 kuv = vec2(cos(aa), sin(aa)) * r;
    vec2 suv = kuv / vec2(aspect, 1.0) + 0.5;
    float h = specHist(min(idx, 7), 0.1 + float(idx) * 0.06) * 5.0;
    vec3 c = cacheHist(idx, suv).rgb;
    vec3 tint = acid(float(idx) * 0.13 + time_f * 0.1 + h * 0.3);
    c = c * tint * (1.0 + h * 1.5);
    // boundary lines
    float bord = smoothstep(0.0, 0.01, abs(aa) - seg * 0.45);
    c *= 0.4 + 0.6 * bord;
    c *= 1.2 + amp_peak * 1.2;
