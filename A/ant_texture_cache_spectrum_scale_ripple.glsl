#version 330 core
// ant_texture_cache_spectrum_scale_ripple
...
vec2 styleDrift(vec2 uv, float layer, float age, float hB, float hM, float hT, float hA, vec3 oldest) {
    float r = length(uv);
    float w = sin(r * (12.0 + hB * 8.0) - time_f * 2.0 - layer * 0.7);
    vec2 n = vec2(uv.x, uv.y) / (r + 1e-3);
    return n * w * (0.006 + age * 0.020);

}

vec3 styleColor(vec3 cached, float layer, float age, float hB, float hT) {
    return cached * (0.95 + 0.08 * sin(layer * 0.5 + hT * 4.0));
