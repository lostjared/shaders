...
// A crown of sharpened acid rays and scale-pulsed reflections.
#define EFFECT_ID 24
...
#if EFFECT_ID % 5 == 0
    point = rotate_2d(ripple * 0.025) * point * scale;
    point += flow * age * (0.007 + middle * 0.012);
#elif EFFECT_ID % 5 == 1
    point = kaleidoscope(point, 5.0 + float(EFFECT_ID % 7));
    point *= scale;
...
#else
    float radius = length(point);
    point = rotate_2d(sin(radius * 16.0 - time_f) * 0.035 * age) * point * scale;
