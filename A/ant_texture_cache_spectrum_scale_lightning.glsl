#version 330 core
// ant_texture_cache_spectrum_scale_lightning
...
vec2 styleDrift(vec2 uv, float layer, float age, float hB, float hM, float hT, float hA, vec3 oldest) {
    float jitter = (hash12(vec2(layer, floor(time_f * 30.0))) - 0.5) * 2.0;
    return vec2(jitter * 0.012, sin(uv.y * 9.0 + time_f * 5.0 + layer) * 0.008) * (0.4 + age * 1.2 + hT * 1.5);

}

vec3 styleColor(vec3 cached, float layer, float age, float hB, float hT) {
    float spark = step(0.93, hash12(vec2(layer, floor(time_f * 12.0))));
    return cached + vec3(0.6, 0.8, 1.0) * spark * (0.3 * age * (0.5 + hT));
