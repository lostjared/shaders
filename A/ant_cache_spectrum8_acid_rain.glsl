#version 330 core
// ant_cache_spectrum8_acid_rain
// Vertical rain streaks each falling at speed = spectrum-history bass
...
void main() {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        float speed = 0.3 + h * 3.0;
        float seed = hash(vec2(col, float(i)));
        float y = mod(tc.y - iTime * speed - seed, 1.0);
        float drop = exp(-y * (15.0 - h * 12.0)) * step(0.3, fract(seed * 9.0));
        vec3 tint = palette(seed + h2 * 3.0 + float(i) * 0.1, vec3(0.0, 0.33, 0.66));
        vec2 suv = tc + vec2(0.0, drop * 0.05);
        vec3 c = cacheHist(i, suv).rgb;
        acc += (tint * drop * 2.5 + c * 0.3) * (1.0 + h * 6.0) * pow(0.84, float(i));
    }
