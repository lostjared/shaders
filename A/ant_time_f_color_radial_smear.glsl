#version 330 core
// ant_time_f_color_radial_smear
// Radial smear: concentric rings of cache frames blended outward, FFT history sets ring brightness
...
void main() {
    ...
    float wsum = 0.0;
    for (int i = 0; i < 9; i++) {
        float ringR = 0.05 + float(i) * 0.08;
        float band = exp(-pow((r - ringR) * 12.0, 2.0));
        float h = specHist(min(i, 7), 0.1 + float(i) * 0.05) * 5.0;
        vec2 suv = (uv * (1.0 - float(i) * 0.04)) / vec2(aspect, 1.0) + 0.5;
        vec3 c = cacheHist(i, suv).rgb;
        vec3 tint = acid(float(i) * 0.13 + time_f * 0.05);
        acc += c * tint * band * (0.5 + h * 2.0);
        wsum += band;
    }
    acc /= max(wsum, 0.4);
    acc *= 1.5 + amp_smooth * 1.2;
