#version 330 core
// ant_time_f_color_shutter_burst
// Frame stutter: screen tiled into a 3x3 grid where each tile shows a different cache frame
...
void main() {
    ...
    vec3 tint = acid(float(idx) * 0.13 + time_f * 0.1);
    c *= tint * (1.0 + h);
    // FFT bar at bottom of each cell
    if (cell.y < specHist(min(idx, 7), cell.x) * 0.5)
        c += acid(time_f * 0.1 + float(idx) * 0.13);
    // grid lines
    float gl = smoothstep(0.0, 0.01, min(cell.x, 1.0 - cell.x)) * smoothstep(0.0, 0.01, min(cell.y, 1.0 - cell.y));
    c *= 0.3 + 0.7 * gl;
    c *= 1.0 + amp_smooth;
